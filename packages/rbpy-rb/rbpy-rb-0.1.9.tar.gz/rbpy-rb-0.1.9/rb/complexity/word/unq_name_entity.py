...
class NoUnqNamedEntity(ComplexityIndex):
    ...
    def __init__(self, lang: Lang, named_ent_type: NamedEntityONEnum,
        ...
        ComplexityIndex.__init__(self, lang=lang, category=IndexCategory.MORPHOLOGY,
                                 abbr="NoUnqNamedEntity", reduce_depth=reduce_depth,
    ...
    def compute_below(self, element: TextElement) -> Set[str]:
        if element.is_word() == True:
            if element.token.ent_type_ != 0 and element.token.ent_type_ == self.named_ent_type.name:
                return set([element.token.text])
            else:
                return set()
