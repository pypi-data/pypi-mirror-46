...
class ParseDepTree(ComplexityIndex):
    ...
    def compute_below(self, element: TextElement) -> float:
        if element.is_sentence() == True:
            res = self.compute_height(list(element.doc.sents)[0].root)
            return res
    ...
    def compute_height(self, token: Token) -> int:
        if not list(token.children):
            return 1
        else:
            return 1 + max(self.compute_height(x) for x in token.children)