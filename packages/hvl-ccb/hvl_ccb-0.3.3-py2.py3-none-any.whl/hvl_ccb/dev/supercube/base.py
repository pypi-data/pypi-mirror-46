...
class SupercubeBase(SingleCommDevice):
    ...
    def start(self) -> None:
        ...
        self.com.init_monitored_nodes(
            map(str, constants.GeneralSockets), self.config.namespace_index)
        self.com.init_monitored_nodes(
            map(str, constants.GeneralSupport), self.config.namespace_index)
        self.com.init_monitored_nodes(
            map(str, constants.Safety), self.config.namespace_index)
        self.com.init_monitored_nodes(
        ...
        self.com.init_monitored_nodes(
            map(str, constants.EarthingStick), self.config.namespace_index)
    ...
    def read(self, node_id: str):
        ...
        self.logger.debug('Read from node ID {} ...'.format(node_id))
        result = self.com.read(
            str(node_id),
            self.config.namespace_index
        )
        self.logger.debug('Read from node ID {}: {}'.format(node_id, result))
        return result
    ...
    def write(self, node_id, value) -> None:
        ...
        self.logger.debug('Write to node ID {}: {}'.format(node_id, value))
        self.com.write(
            str(node_id),
            self.config.namespace_index,
            value
    ...
    def get_support_input(self, port: int, contact: int) -> bool:
        ...
        return bool(self.read(
            constants.GeneralSupport.input(port, contact)
        ))
    ...
    def get_support_output(self, port: int, contact: int) -> bool:
        ...
        return bool(self.read(
            constants.GeneralSupport.output(port, contact)
        ))
    ...
    def get_t13_socket(self, port: int) -> bool:
        ...
        :param port: is the socket number (1..3)
        :return: on-state of the power socket
        """

        if port not in (1, 2, 3):
            raise ValueError('port not in (1, 2, 3): {}'.format(port))

        return bool(self.read(
            getattr(constants.GeneralSockets, 't13_{}'.format(port))
        ))
    ...
    def set_t13_socket(self, port: int, state: bool) -> None:
        ...
        if not isinstance(state, bool):
            raise ValueError('state is not <bool>: {}'.format(state))
    ...
    def get_status(self) -> constants.SafetyStatus:
        ...
        return constants.SafetyStatus(
            self.read(constants.Safety.status)
        )
    ...
    def get_door_status(self, door: int) -> constants.DoorStatus:
        ...
        return constants.DoorStatus(
            self.read(constants.Door.get(door))
        )
