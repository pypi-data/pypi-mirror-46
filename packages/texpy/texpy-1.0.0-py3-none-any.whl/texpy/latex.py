...
def principale(n,nrif=None,nult=None,unit=None):
	...
	if nult==None:
		if absolute(nrif)==absolute(n):
			if temp==None:nult=n/100 #ns(456,456)=4.56 x 10^2
			else: nult=n
		if absolute(nrif)>absolute(n): nult=n    #ns(6,572)=0.06 x 10^2
		if absolute(nrif)<absolute(n): nult=nrif #ns(572,6)=572
...
nes=vectorize(numero_con_errore_separato)
