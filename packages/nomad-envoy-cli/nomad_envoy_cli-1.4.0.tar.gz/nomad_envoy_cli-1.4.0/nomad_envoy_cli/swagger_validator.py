...
class SwaggerValidator(object):
    ...
    def _validate_recursively(self, obj: Any,
        ...
        types = getattr(field_types_container, self.KEY_OPENAPI_TYPES)
    ...
    def _load_str_types_mapping(cls, any_swagger_model) -> Dict[str, Type]:
        if len(cls.MAPPING_STR_TO_REAL_TYPES) > 0:
            return cls.MAPPING_STR_TO_REAL_TYPES  # means already loaded.
        ...
        for field in dir(models_module):
            ...
            if hasattr(potential_class, cls.KEY_OPENAPI_TYPES):
                cls.MAPPING_STR_TO_REAL_TYPES[field] = potential_class

        # 2. Also load primitive types.
        cls.MAPPING_STR_TO_REAL_TYPES["str"] = str
        cls.MAPPING_STR_TO_REAL_TYPES["int"] = int
        cls.MAPPING_STR_TO_REAL_TYPES["float"] = float
        cls.MAPPING_STR_TO_REAL_TYPES["bool"] = bool

        # 3. Others
        cls.MAPPING_STR_TO_REAL_TYPES["list"] = List
        cls.MAPPING_STR_TO_REAL_TYPES["dict"] = Dict  # TODO need check
        cls.MAPPING_STR_TO_REAL_TYPES["date"] = date  # TODO need check
        cls.MAPPING_STR_TO_REAL_TYPES["datetime"] = datetime  # TODO need check
