...
def superscalar(symbol_in, training_symbol, block_length, pilot_number, constl):
    ...
    divided_symbols = first_order_pll(divided_symbols,(constl),0.1)

    divided_symbols[1::2,] = divided_symbols[1::2,::-1]
...
def first_order_pll(divided_symbols, constl,g):
    ...
    error = np.zeros((divided_symbols.shape[0], divided_symbols.shape[1]))
    for i in prange(divided_symbols.shape[0]):
        signal = divided_symbols[i, :]
        each_error = error[i, :]
        for point_index, point in enumerate(signal):
            if point_index == 0:
                point = point * np.exp(-1j * each_error[point_index])
            else:
            ...
            if point_index > 0:
                ...
                each_error[point_index] = angle_difference * g + each_error[0]
    return divided_symbols
    ...
    for j in range(niter):
        for i in range(samplex.shape[0]):
            ...
            if i>0:
                ...
                xout_cpr = xout * np.exp(-1j * pll_phase[0, 0])
                yout_cpr = yout * np.exp(-1j * pll_phase[1, 0])
            ...
            angle_difference_x = (xout_cpr * np.conj(xout_cpr_decision)).imag
            angle_difference_y = (yout_cpr * np.conj(yout_cpr_decision)).imag
            if i==0:
                pll_phase[0, i] = g * angle_difference_x + pll_phase[0, 0]
                pll_phase[1, i] = g * angle_difference_y + pll_phase[1, 0]
            else:
