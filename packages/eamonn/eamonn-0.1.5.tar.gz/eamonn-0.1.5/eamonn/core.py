...
def get(url,timetry=3,headers="",ip='',timeout=60,ex="",data='',cache=False,cookie='',cachedur=2):
    import requests
    parm=struct_parm({'url':url,'data':data,'headers':headers,'cookie':cookie})
    if headers!="":
        headers=f2h(headers)
    ...
    while 1:
        try:
            if data=='':
                response=requests.get(parm['url'],headers=parm['headers'],proxies=ip,timeout=timeout)
            else:
                response=requests.post(parm['url'],headers=parm['headers'],data=parm['data'],proxies=ip,timeout=timeout)
...
class save_mongo():
    def __init__(self, ip="127.0.0.1",fname='',dname="Default_name",time=True):
        import pymongo
        if fname == '':
            if time: self.cname = 'Default_name'+"_" + time(2)
