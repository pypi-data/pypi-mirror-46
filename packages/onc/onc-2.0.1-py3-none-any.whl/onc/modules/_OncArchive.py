import requests
from modules._util import saveAsFile
import json
import puremagic
import humanize
import time
...
class _OncArchive:
    ...
    def getListByLocation(self, filters: dict=None):
        """
    ...
    def getListByDevice(self, filters: dict=None):
        """
    ...
    def getFile(self, filename: str='', overwrite: bool=True):
        url = '{:s}api/archivefiles'.format(self.baseUrl)
        filters = {
            'token'   : self.token,
            'method'  : 'getFile',
            'filename': filename,
        ...
        try:
            ...
            if response.ok:
                ...
                saveAsFile(response, self.outPath, filename, overwrite)
                ...
                filePath = '{:s}/{:s}'.format(self.outPath, filename)
