...
class AppsApi(object):
    ...
    def regenerate_api_key(self, app_id, body, async=False, **kwargs):
        """
        ...
        kwargs['_return_http_data_only'] = True
        if async:
            return self.regenerate_api_key_with_http_info(app_id, body, async=async, **kwargs)
        else:
            (data) = self.regenerate_api_key_with_http_info(app_id, body, async=async, **kwargs)
            return data

    def regenerate_api_key_with_http_info(self, app_id, body, async=False, **kwargs):
        """
        Create a new API key for an application. An application may only have one valid API key at a time, so performing this action will invalidate all old API keys. This does not invalidate existing sessions, so any applications with an existing open session will be able to continue operating with their old session until those sessions expire. 
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async=True::
            >>> thread = api.regenerate_api_key_with_http_info(app_id, body, async=True)
            >>> result = thread.get()
