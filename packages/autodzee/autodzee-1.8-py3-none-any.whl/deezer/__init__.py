...
class Login:
    ...
    def download(self, track_id, title, artist_name,info_download={}, output=localdir+"/track"):
        track_id=str(track_id)
        artist_name=artist_name.translate({ord(c): " " for c in "!@#$%^&*()[]{};:,./<>?\|`~-=_+"}).strip()
        title=title.translate({ord(c): " " for c in "!@#$%^&*()[]{};:,./<>?\|`~-=_+"}).strip()
        dir = output + "/" + artist_name.replace(" ","_") + "/"
        try:
            os.makedirs(dir)
        ...
        def get_audio(infos, quality, id,title,extension):
            try:
            ...
            path_name = dir + uuid.uuid4().hex + "_"+quality+ "_" + base64.b64encode(track_id.encode('utf-8')).decode('utf-8') + "_" + base64.b64encode(
                "97206058".encode('utf-8')).decode('utf-8')+"_"+base64.b64encode(infos['ISRC'].encode('utf-8')).decode('utf-8')  + extension
        ...
        if int(infos['FILESIZE_FLAC']) > 0:
            ...
            extension = ".flac"
            try:
                info_download['url_flac']=get_audio(infos,quality,track_id,title,extension)
        ...
        if int(infos['FILESIZE_MP3_320']) > 0:
            ...
            extension = ".mp3"
            try:
                info_download['url_320'] = get_audio(infos, quality, track_id,title,extension)
            except TrackNotFound:
        ...
        if int(infos['FILESIZE_MP3_128']) > 0:
            ...
            extension = ".mp3"
            try:
                info_download['url_128'] = get_audio(infos, quality, track_id,title,extension)
            except TrackNotFound:
