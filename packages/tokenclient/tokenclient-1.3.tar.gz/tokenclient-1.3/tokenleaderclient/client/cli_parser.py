...
parent_parser.add_argument('--emailid', action = "store", dest = "emailid", required = False)
...
list_parser.add_argument('-u', '--username', 
                  ...
                  required = False,
                  help = "please provide the username to access the list entity, its mandatory"
                  )
list_parser.add_argument('-o', '--otp', 
                  action = "store", dest = "otp",
                  required = False,
                  help = "please provide the otp send to your mail id, only valid for external user"
                  )
...
delete_parser.add_argument('-o', '--otp', 
                  action = "store", dest = "otp",
                  required = False,
                  help = "please provide the otp send to your mail id, only valid for external user"
                  )
...
addwfc_parser.add_argument('-u', '--username', 
                  action = "store", dest = "username",
                  required = False,
                  help = "please provide the username to access the list entity, its mandatory"
                  )
addwfc_parser.add_argument('-o', '--otp', 
                  action = "store", dest = "otp",
                  required = False,
                  help = "please provide the otp send to your mail id, only valid for external user"
                  )
...
adduser_parser.add_argument('-u', '--username', 
                  action = "store", dest = "username",
                  required = False,
                  help = "please provide the username to access the list entity, its mandatory"
                  )
adduser_parser.add_argument('-o', '--otp', 
                  action = "store", dest = "otp",
                  required = False,
                  help = "please provide the otp send to your mail id, only valid for external user"
                  )
...
addservice_parser.add_argument('-u', '--username', 
                  action = "store", dest = "username",
                  required = False,
                  help = "please provide the username to access the list entity, its mandatory"
                  )
addservice_parser.add_argument('-o', '--otp', 
                  action = "store", dest = "otp",
                  required = False,
                  help = "please provide the otp send to your mail id, only valid for external user"
                  )
...
deletservice_parser.add_argument('-u', '--username', 
                  action = "store", dest = "username",
                  required = False,
                  help = "please provide the username to access the list entity, its mandatory"
                  )
deletservice_parser.add_argument('-o', '--otp', 
                  action = "store", dest = "otp",
                  required = False,
                  help = "please provide the otp send to your mail id, only valid for external user"
                  )
...
listservice_parser.add_argument('-u', '--username', 
                  action = "store", dest = "username",
                  required = False,
                  help = "please provide the username to access the list entity, its mandatory"
                  )
listservice_parser.add_argument('-o', '--otp', 
                  action = "store", dest = "otp",
                  required = False,
                  help = "please provide the otp send to your mail id, only valid for external user"
                  )
...
def main():     
    ...
    if options.authuser and options.authpwd:
        if options.domain and options.otp:
            # print('domain and otp found')
            auth_config = Configs(tlusr=options.authuser, tlpwd=options.authpwd, domain=options.domain, otp=options.otp)
        if options.domain:
            # print('only domain found')
            auth_config = Configs(tlusr=options.authuser, tlpwd=options.authpwd, domain=options.domain)
        if options.otp:
            # print('only otp found')
            auth_config = Configs(tlusr=options.authuser, tlpwd=options.authpwd, otp=options.otp)
        else:
            # print('only username and password found')
            auth_config = Configs(tlusr=options.authuser, tlpwd=options.authpwd)
    elif options.emailid and options.otp:
        # print('emailid and otp found')
        auth_config = Configs(email=options.emailid, otp=options.otp)
    else:
        # print('default')
        auth_config = Configs()
    ...
    if  sys.argv[1] == 'list':
        if options.entity == 'user':
         if options.name and options.otp:
            print(c.list_user_byname(options.name, options.username, options.otp))
         else:
            print(c.list_users(options.username, options.otp))
                
    if  sys.argv[1] == 'list':
        if options.entity == 'dept':
         if options.name and options.otp:
            print(c.list_dept_byname(options.name, options.username, options.otp))
         else:
            print(c.list_dept(options.username, options.otp)) 
            
    if  sys.argv[1] == 'list':
        if options.entity == 'org':
         if options.name and options.otp:
           print(c.list_org_byname(options.name, options.username, options.otp))
         else:
           print(c.list_org(options.username, options.otp))

    if  sys.argv[1] == 'list':
        if options.entity == 'role':
          if options.name and options.otp:
            print(c.list_role_byname(options.name, options.username, options.otp))
          else:
            print(c.list_role(options.username, options.otp))

    if  sys.argv[1] == 'list':
        if options.entity == 'ou':
          if options.name and options.otp:
            print(c.list_ou_byname(options.name, options.username, options.otp))
          else:
            print(c.list_ou(options.username, options.otp))

    if  sys.argv[1] == 'list':
        if options.entity == 'wfc':
          if options.name and options.otp:
            print(c.list_wfc_byname(options.name, options.username, options.otp))
          else:
            print(c.list_wfc(options.username, options.otp))

    if  sys.argv[1] == 'adduser':
        r = c.add_user(options.username, options.otp)
        print(r)
    ...
    if  sys.argv[1] == 'add':
        ...
        if options.entity == 'org':      
            af.register_org(options.name, options.username, options.otp)
               
        
        if options.entity == 'ou':      
            r = c.add_orgunit(options.name, options.username, options.otp)
        print(r)
                
                
        if options.entity == 'dept':      
            af.register_dept(options.name, options.username, options.otp)
          
                
        if options.entity == 'role':
            af.register_role(options.name, options.username, options.otp)
