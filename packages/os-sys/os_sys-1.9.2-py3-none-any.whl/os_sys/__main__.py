...
def copytree(src, dst, symlinks=False, ignore=None):
    ...
    for item in items:
        ...
        if os.path.isdir(s):
            shutil.copytree(s, d, symlinks, ignore=True)
        else:
            shutil.copy2(s, d)
    items.close()
    return fi
...
def install(path, verbose, quiet):
    ...
    for small in package_info:
        try:
            ...
            if 'name' in key:
                ...
                continue
    from distutils.sysconfig import get_python_lib as gpl
    mapsdel =all_dict(os.path.join(gpl(), info_dict['name']), maps=True, files=False)
    for i in mapsdel:
        try:
            shutil.rmtree(i)
        except:
    ...
    base = os.path.join(gpl(), info_dict['name'])
    ...
    if time != None:
        for i in pbar:
            ...
            try:
                path, data = i.split('>>>>>>>>>>>>>>>')
                if verbose:
                    pbar.set_description('processing file: {path}...')
                a = path
            except Exception:
                pass
            else:
...
command line console")
    parser.add_argument("-u", "--usage", help=f"show this help message: {help_msg}")
...
stored if you select install default is working dictory", action="store", default=os.path.abspath(''))
    ...
    if "make_doc" == arg:
        ...
        if auto_install_name == 'server':
            ...
            import time
            time.sleep(1)
            os.rmdir(os.path.join(os.path.abspath(''), 'server_dist'))
    elif "upload" == arg:
