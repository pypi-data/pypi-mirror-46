...
#     :param commands: List of Alpine Linux OS commands to be run to set up the container
#     :return:
#     """
...
#
#     optional = util.optional(project=project, name=function, commands=commands)
...
class DatabaseClient(object):
    ...
    def post_json(self, url_part: str, raw_data: Dict):
        ...
        a_url = self.url() + url_part
        try:
            result = requests.post(a_url, headers=headers, data=json.dumps(raw_data), verify=False)
            js = result.json()
            result.raise_for_status()
            return js
...
def get_database_client(url: str = "https://app.15rock.com/gateway/compute",
                        credentials: Dict = None,
                        credentials_path: str = None) -> DatabaseClient:
    credentials = credentials or helper.get_credentials(credentials, credentials_path)
