...
def make_requirements_file_notebook(working_dir: Path) -> Path:
    from sys import stderr
    from subprocess import run, PIPE
    cmd = ['pip', 'freeze']
    # out = run(cmd, env=env, stdout=PIPE, stderr=PIPE)
    ...
    blacklisted = ['Automat', 'conda', 'pycosat', 'pycurl', 'PySocks']
    b_result = []
...
if __name__ == '__main__':
    ...
    req_path = make_requirements_file_notebook(wdir)
    print(req_path)
    # remove_dir(wdir)
    # print('Req Path')
