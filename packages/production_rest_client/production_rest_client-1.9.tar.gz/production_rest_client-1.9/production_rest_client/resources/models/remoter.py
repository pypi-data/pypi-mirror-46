...
class RemoteCtrl(object):
    def __init__(self, host, ini='../config/comConfig.yaml', output=None):
        self.ini = ini
        self.output = output
        self.srv_port = 5000
        self._read_cfg()
    ...
    def _read_cfg(self):
        if not os.path.isfile(self.ini):
            raise StandardError("Config file not exist!")
        filep = open(self.ini)
        self.config = yaml.load(filep)
    def reset_remote(self):
        cmd = 'reset'
        #print "[vRemoteCtrl] cmd: %s, IP: %s"%(cmd,ip)
        self._remote_ctrl(cmd, self.host)
    def power_off(self):
        cmd = 'off'
        self._remote_ctrl(cmd, self.host)
    def power_on(self):
        ...
        for srv in self.config:
            ...
            for client in clist:
                if ip_addr in client['ip']:
                    return sip
        raise StandardError("Server not found!")
