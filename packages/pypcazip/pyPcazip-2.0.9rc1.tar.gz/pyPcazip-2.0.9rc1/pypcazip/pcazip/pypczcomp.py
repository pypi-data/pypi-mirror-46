...
from mdio.rmsd_utils import kabsch_rmsd, kabsch
...
def pczcomp(args):
    ...
    rms = kabsch_rmsd(px.refxyz, py.refxyz)
...
#
    ...
    R, T, rmse = kabsch(px.refxyz, py.refxyz)

    for i in range(args.nvecs):
        evx[i] = px.evecs[i]
        evy[i] = py.evecs[i]
        evy[i] = np.dot(evy[i].reshape(n_atoms,3),R).flatten()
