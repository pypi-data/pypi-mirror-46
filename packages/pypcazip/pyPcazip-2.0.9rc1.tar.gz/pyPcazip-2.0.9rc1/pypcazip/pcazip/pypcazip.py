...
def pcazip(args):
    ...
    tlist = []
    traj_parts = []
    for i, t in enumerate(trajfiles):
        if tops[i] == top and selections[i] == sel:
            tlist.append(t)
        else:
            traj_parts.append(mdio.mpi_load(tlist, top=top, selection=sel, comm=comm))
            top = tops[i]
            sel = selections[i]
            tlist = []
