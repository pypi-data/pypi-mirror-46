...
def DayToday():
  ...
  """
  return calendar.day_abbr[datetime.date.today().weekday()]
...
def LatestUpdateTime(path,HDF_landing=False):
  ...
  for file in files:
      ...
      x = datetime.fromtimestamp(status.getModificationTime()/1000.0)
...
def datetimediff(d2_ts,d1_ts):
  ...
  try:
    fmt = '%Y-%m-%d %H:%M:%S'
    d1_ts = datetime.strptime(d1_ts, fmt)
    d2_ts = datetime.strptime(d2_ts, fmt)
...
def GetDayFromDate(dt,formats=DateFormatswithDashes()):
  """
  Get Day from Date
...
def get_eom(date):
  ...
  date = date + datetime.timedelta(32)
  date = date.replace(day=1)
...
def GetMin(dt,formats=DateFormatswithDashes()):
  """
...
def utc_to_local(utc_dt,tz):
  ...
  try:
    ...
    local_dt = datetime.strftime(local_dt,'%Y-%m-%d %H:%M:%S')
    return local_dt  
  except:
    return utc_dt
...
udf_EDWDateTime = F.udf(lambda dt,tm:ConcatenateEDWDateTime(dt,tm), TimestampType())
udf_day_from_date = F.udf(lambda dt:GetDayFromDate(dt), StringType())
udf_eom = F.udf(lambda x:get_eom(x), DateType())
udf_som = F.udf(lambda x:get_som(x), DateType())
udf_get_hour_from_datetime = F.udf(lambda dt:GetHour(dt), IntegerType())
udf_get_min_from_datetime = F.udf(lambda dt:GetMin(dt), IntegerType())
