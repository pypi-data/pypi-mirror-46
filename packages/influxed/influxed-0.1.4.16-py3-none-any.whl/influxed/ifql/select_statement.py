...
from influxed.ifql.functions import Func, Count, Sum, Max, Mean, Min, Median, Distinct, Percentile, Derivative, Stddev, First, Last, Difference
        ...
        if self._group_by:
            ...
            for i in range(len(group_by_strs)):
                if(str_needs_escape(group_by_strs[i])):
                    group_by_strs[i] = f'"{group_by_strs[i]}"'
