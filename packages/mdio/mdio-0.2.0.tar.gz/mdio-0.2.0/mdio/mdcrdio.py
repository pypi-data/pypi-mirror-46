...
def mdcrd_open(filename, mode='r', atom_indices=None, top=None, selection=None):
    """
    Open an MDCRD  file.
    """
    if not mode in ["r", "w"]:
    ...
    if mode == 'r':
        return MDCRDFileReader(filename, atom_indices=atom_indices, top=top, selection=selection)
    else:
