...
class RstFileReader(object):
    def __init__(self, filename, atom_indices=None, top=None, selection=None):
        ...
        self.top = top
        if selection is not None and top is None:
            raise IOError('Error - cannot have selection without a topology.')
        if top is not None:
...
def rst_open(filename, mode='r', atom_indices=None, top=None, selection=None):
    """
    Open a Rst file.
    """
    if not mode in ["r", "w"]:
    ...
    if mode == 'r':
        return RstFileReader(filename, atom_indices=atom_indices, top=top, selection=selection)
    else:
