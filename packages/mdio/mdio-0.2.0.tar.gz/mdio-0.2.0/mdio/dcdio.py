...
class DCDFileReader(object):
    def __init__(self, f, atom_indices=None, top=None, selection=None):
        self.f  = f
        self.atom_indices = atom_indices
        if top is not None:
            self.top = mdio.base.Topology(top)
            if selection is not None:
                self.atom_indices = self.top.select(selection)
            if self.atom_indices is not None:
                self.top = self.top.subset(self.atom_indices)
        else:
            self.top = None
            if selection is not None:
                raise RuntimeError('Error - no topology for selection.')
    ...
    def read_frame(self):
        ...
        frame = mdio.base.Frame(crds, box, time=time, timestep=timestep, units='angstroms')
        return frame
...
def dcd_open(filename, mode='r', atom_indices=None, top=None, selection=None):
    if not mode in ['r', 'w']:
        raise ValueError('Error - mode must be "r" or "w".')
    if mode == 'r':
        f = open(filename, 'rb')
        filehandler = DCDFileReader(f, atom_indices=atom_indices, top=top, selection=selection)
    elif mode == 'w':
        f = open(filename, 'wb')
