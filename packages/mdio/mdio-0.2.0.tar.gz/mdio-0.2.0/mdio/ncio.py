...
class NCFileReader(object):
    def __init__(self, filename, atom_indices=None, top=None, selection=None):
        ...
        if top is not None:
            ...
            self.top = None
            if selection is not None:
                raise RuntimeError('Error - no topology for the selection.')
...
def nc_open(filename, mode='r', atom_indices=None, top=None, selection=None):
    """
    ...
    if mode == 'r':
        return NCFileReader(filename, atom_indices=atom_indices, top=top, selection=selection)
    else:
