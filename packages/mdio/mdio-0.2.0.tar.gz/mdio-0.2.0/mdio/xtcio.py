...
def read_frame(up, atom_indices=None):
    ...
    if atom_indices is None:
        ...
        natoms = np.array(atom_indices).max()
    if natoms > maxatoms:
...
def xtc_open(filename, mode='r', atom_indices=None, top=None, selection=None):
    if not mode in ['r', 'w']:
        raise ValueError('Error: mode must be "r" or "w"')
    if mode == 'r':
        filehandler = XtcfileReader(filename, atom_indices=atom_indices, top=top, selection=selection)
    else:
