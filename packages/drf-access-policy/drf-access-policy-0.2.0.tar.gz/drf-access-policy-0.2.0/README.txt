...
+-----------------+-----------------------------------------+-------+---------------+
| Name            | Description                             | Speci | Examples      |
|                 |                                         | al    |               |
|                 |                                         | Value |               |
|                 |                                         | s     |               |
+=================+=========================================+=======+===============+
| principal       | Identifies the user of the current      | ``"*" | ``["group:adm |
|                 | request by the name of a group they     | ``    | ins", "id:932 |
|                 | belong to or their user id. Can be      | (any  | 2"]``         |
|                 | formatted as a string or list of        | user) | ``["id:5352"] |
|                 | strings.                                | ``"au | ``            |
|                 |                                         | thent | ``["anonymous |
|                 |                                         | icate | "]``          |
|                 |                                         | d"``  |               |
...
|                 |                                         | user) |               |
+-----------------+-----------------------------------------+-------+---------------+
| action          | The action or actions that the          | ``"*" | ``["list", "d |
|                 | statement applies to. Can be formatted  | ``    | elete", "crea |
|                 | as a string or list of strings. The     | (any  | te]``         |
|                 | value should match the name of a view   | actio | ``["*"]``     |
|                 | set method or the name of the view      | n)    |               |
|                 | function.                               |       |               |
+-----------------+-----------------------------------------+-------+---------------+
| effect          | Whether the statement, if it is in      | n/a   | ``"allow"``   |
...
+-----------------+-----------------------------------------+-------+---------------+
| condition       | The name of a method on the policy that | n/a   | ``"is_manager |
|                 | returns a boolean. The method signature |       | _for_account" |
|                 | is                                      |       | ``            |
|                 | ``condition(request, view, action: str) |       | ``"is_author_ |
|                 | ``.                                     |       | of_post"``    |
|                 | If true, the policy will be in effect.  |       | ``["balance_i |
|                 | Useful for enforcing object-level       |       | s_positive",  |
|                 | permissions. If list of strings, all    |       | "account_is_n |
|                 | conditions must evaluate to ``True``.   |       | ot_frozen"]`` |
+-----------------+-----------------------------------------+-------+---------------+
