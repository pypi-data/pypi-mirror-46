...
class AsyncTranslator(Translator):
    def __init__(self, service_urls=None, user_agent=DEFAULT_USER_AGENT,
        ...
        self.session = TranslateSession()

        if proxies is not None:
            self.session.proxy = proxies
        headers = {
            'User-Agent': user_agent,
        }
    ...
    async def _translate(self, text, dest, src):
        ...
        params = utils.build_params(query=text, src=src, dest=dest,
                                    token=token)
        url = '?'.join([urls.TRANSLATE.format(host=self._pick_service_url()), urlencode(params, doseq=True)])
        r = await self.session.get(url)

        data = utils.format_json(await r.text())
        return data
