...
_DEFAULT_LOGGING_FORMAT = ('[%(levelname)1.1s '
                           '%(asctime)s '
                           'P%(process)d '
                           '%(threadName)s '
                           '%(module)s:%(lineno)d] '
...
class _SlackLoggerThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            backoff_time = (2 ** self._flush_cnt)  \
    ...
    def flush(self):
        ...
        msg_encoded = urllib.parse.urlencode(msg).encode('utf-8')
        urllib.request.urlopen(self.slack_url, data=msg_encoded)  # nosec
    ...
    def prepare(self, record):
        ...
        if record.exc_info is not None:
            ...
            if 100 < len(traceback_lines):
                traceback_lines = traceback_lines[:50]  \
                    + ['...(truncated)...'] \
                    + traceback_lines[-50:]
...
def init_logger(
    ...
    if slack_url is not None:
        ...
        slack_handler = _SlackLoggerHandler(product_name,
                                            slack_logger_thread.queue)
    ...
    interested_loggers: List[logging.Logger] = [
        app_logger,
        *additional_loggers
    ]
