...
WorkerConfig = Dict[str, ProcessConfig]


class _ProcessInfo(NamedTuple):
    pname: str
    popen_obj: subprocess.Popen
    p_monitor: threading.Thread


def run(worker_config: WorkerConfig):
    ...
    max_name_length = 0
    for process_name, process_config in worker_config.items():
        for idx in range(process_config.num_workers):
            pname = '%s_%d' % (process_name, idx + 1)
    ...
    p_info_list: List[_ProcessInfo] = []
    for process_name, process_config in worker_config.items():
        for idx in range(process_config.num_workers):
            pname = '%s_%d' % (process_name, idx + 1)

            popen_obj = subprocess.Popen(process_config.cmd,
            ...
            p_info_list.append(_ProcessInfo(
                pname=pname,
                popen_obj=popen_obj,
                p_monitor=p_monitor
            ))
