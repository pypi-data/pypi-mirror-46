...
def _register_device(end_point, cert, payload):
    ...
    try:
        ret = requests.post(end_point, data=payload,
...
def main(board, register_host, register_port, cert):
...
def run_cli():
    ...
    if board:
        status = main(board, args.register_host,
                      args.register_port, args.cert)
