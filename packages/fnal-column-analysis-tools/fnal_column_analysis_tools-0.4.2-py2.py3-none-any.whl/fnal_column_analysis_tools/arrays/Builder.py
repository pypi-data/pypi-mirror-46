...
class Dangerousness(object):
    ...
    def __getattr__(self,what):
        if what in self.columns:
            return self[what]
        thewhat = getattr(super(SaiyanCommonMethods, self), what)
        return self.fromjagged(thewhat)


class SaiyanCommonMethods(Dangerousness):
...
def Initialize(items):
    ...
    if 'p4' in argkeys:
        ...
        p4 = SaiyanArray.from_ptetaphim(items['pt'], items['eta'],
    ...
    if isinstance(out, awkward.JaggedArray):
        out.content.__class__ = type("Object",
                                     (out.content.__class__,
                                      SaiyanCommonMethods),
                                     {})
        out.__class__ = type("Collection",
                             (out.__class__,
                              SaiyanCommonMethods),
                             {})
