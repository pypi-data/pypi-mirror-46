...
def spring_month(year):
    ...
    jan_m, jan_d = get_ludar_date(jan)[1:]
    feb_m, feb_d = get_ludar_date(feb)[1:]
    mar_m, mar_d = get_ludar_date(mar)[1:]
    apr_m, apr_d = get_ludar_date(apr)[1:]
    # jan_end_m, jan_end_d = get_ludar_date(jan)[1:]
    # feb_end_m, feb_end_d = get_ludar_date(feb)[1:]
    # mar_end_m, mar_end_d = get_ludar_date(mar)[1:]
    if jan_m == 12:
        if feb_d > 1:
            return jan.month
        else:
...
if __name__ == "__main__":
    y, m, d = 2018, 9, 13
    print("Sun calendar 2010-9-13 == Lunar calendar ", get_lunar_date(y, m, d))
    # this_month()
    spring_month(2003)
