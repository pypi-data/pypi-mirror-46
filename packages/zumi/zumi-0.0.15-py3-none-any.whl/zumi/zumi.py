...
class MPU:
    def __init__(self, bus):
        ...
        #Gyro Range (°/sec), Gyro Sensitivity (LSB/°/sec), Gyro Rate Noise (dps/√H$
        #±250   131    0.005
        #±500   65.5   0.005
        #±1000  32.8   0.005
        #±2000  16.4   0.005
        ...
        self.bus.write_byte_data(Device.MPU, DIV, 1)
        #self.bus.write_byte_data(Device.MPU, PWR_MGMT_1, 1)
        self.bus.write_byte_data(Device.MPU, PWR_MGMT_1, 0x00)
        self.bus.write_byte_data(Device.MPU, CONFIG, DISABLE_FSYNC)
        #set the Gyro sensitivity to 2000 degrees
        self.bus.write_byte_data(Device.MPU, GYRO_CONFIG, 0x00)#zero it out first
        self.bus.write_byte_data(Device.MPU, GYRO_CONFIG, GYRO_RANGE_2000DEG)
        #set the accelerometer sensitivity
        self.bus.write_byte_data(Device.MPU, ACCEL_CONFIG, ACCEL_RANGE_2G)
        #self.bus.write_byte_data(Device.MPU, INT_EN, 1)

        time.sleep(0.5)

        """
        Old code created a new object which inherits Axis object
        New code just calls Axis object with the correct scalar parameter
        """
        #self.calibrate_gyro = GyroCalibrate()
        #self.calibrate_accel = AccelerometerCalibrate()
        self.calibrate_gyro = Axis(scalar=GYRO_SCALE_MODIFIER_250DEG)
...
class Zumi:
    def __init__(self):
        ...
        #grab the byte that arduino sends over
        #speed_data = self.bus.read_i2c_block_data(Device.Arduino, 0, 7)
        speed_data = self.bus.read_i2c_block_data(Device.Arduino, 192, 7)
        #parse the speed data on index 6 of the byte
        self.speed = speed_data[6]

        self.speed_left, self.speed_right = _get_config_speed()
        print(" left ",self.speed_left," right ", self.speed_right)
    ...
    def get_tilt(self, x_acc, y_acc, z_acc):
        ...
        this force is usually gravity
            ...
            orientation state
         -1 = accelerating faster than 1 g
        ...
        r_x = int(2 * 180.0 / math.pi * math.atan(x_perc))
        r_y = int(2 * 180.0 / math.pi * math.atan(y_perc))
        r_z = int(2 * 180.0 / math.pi * math.atan(z_perc))
    ...
    def update_angles(self, gyro_multiplier=8.2):
        ...
        angular_speed_x = mpu_list[3] * gyro_multiplier
        angular_speed_y = mpu_list[4] * gyro_multiplier
        angular_speed_z = mpu_list[5] * gyro_multiplier
    ...
    def turn(self,desired_angle, duration=1, max_speed=40, accuracy=1):
        '''
        ...
        self.reset_PID()
        i = 0
        while time.time() - start_time < abs(duration*0.5):
            if(i < max_speed):
                i = i + 1
            self.drive_at_angle(i, 0, desired_angle, k_p, k_d, k_i, accuracy)
    ...
    def go_straight(self, speed, desired_angle,max_speed=127):
        '''
        :WARNING This method only works inside of loops!
    ...
    #the following methods will need to reset the gyroscope angle or change the heading
    def u_turn(self, speed=60,step=1):
        # update the angles and grab the Z angle
        angles_now_list = self.update_angles()
        angle_z = angles_now_list[2]
    ...
    def circle(self, speed=60,step=1):
        # update the angles and grab the Z angle
        angles_now_list = self.update_angles()
        angle_z = angles_now_list[2]
    ...
    def forward(self, speed, duration, desired_angle):
        '''
        :param speed: the forward speed you want Zumi to drive at. Should only input a positive number
        :param duration: number of seconds you want Zumi to try to drive forward
        :param desired_angle: the desired angle
    ...
    def reverse(self, speed, duration, desired_angle):
        '''
        :param speed: the forward speed you want Zumi to drive at. Should only input a positive number
        :param duration: number of seconds you want Zumi to try to drive forward
        :param desired_angle: the desired angle
