...
class Screen:
    ...
    def draw_text(self, string, display=0, image=0, font_size=16, clear=True):
        if display == 0:
            display = self.disp
        if image == 0:
        ...
        current_y = (self.height - 4 - sum(text_heights))  / 2

        for text in texts:
            text_width, text_height = draw.textsize(text, font=font)
            # print(text + ": height-" + str(text_height) + ", width-" + str(text_width))
