            ...
            if form.lower() in self._lexicon:
                ...
                if form in self._lexicon:
                    if len([tag for lemma, weighted_tags in self._lexicon[form].items()
                            ...
                            if self._tag_number_converter[tag][0] not in
                               NOUN + ADJECTIVE + VERB + ADVERB]) > 0:
                        form = original_form
                        prefix = u''
                            ...
                            if post_context_tags[:left_context] == fragment_tags[:left_context]:
                                ...
                                for post_fragment_tags, post_weighting in post_data.items():
                                    ...
                                    if post_fragment_tags[:right_context] ==\
                                            context_tags[left_context:][:len(post_fragment_tags[:right_context])]:
...
if __name__ == "__main__":
    text = 'Su estancia acabó bruscamente al comienzo de la primavera. ' \
           ...
           'Protegía con el brazo el plato de la comida, y se enojaba si yo trataba de poner al otro enano ' \
           'su propio plato. ' \
           ...
           'Cuando no era presa de sus cambios de humor, paseaba por el salón y en su camino elástico era ' \
           'como si pronto fueran a llegar noticias con el resultado de una batalla. ' \
           ...
           'El otro enano desmejoraba, se quedaba flaco y amarillo; no participaba de las conversaciones ' \
           'y sonreía débilmente cuando nos dirigíamos a él. ' \
           ...
           'Tenemos que irnos. ' \
           'No sé cuánto tiempo me llevará convencerle, pero tiene que pensar que lo hace por mí, porque ' \
           'no entiende que está desquiciado.'
    '''
    path_corpus = dirname(realpath(__file__)) + \
                  '/archivos_de_datos/corpus_corpes/corpus_corpes.lemas_revisados.msgpack'
    with open(path_corpus, 'rb') as archivo_corpus:
        corpus = msgpack.unpack(archivo_corpus, use_list=False, encoding="utf-8")
    text = '\n'.join([' '.join(frase) for frase in zip(*corpus)[0]])
    '''
    tagger = SyntacticTagger()
    tags = tagger.tag_text(text)
    print('\n'.join([token + ': ' + tag + ' (' + lemma + ')' for sentence in tags for token, tag, lemma in sentence]))
    pass
