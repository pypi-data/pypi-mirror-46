...
class Region_Selector(object):
    ...
    def onselect1(self, xmin, xmax):
        self.rect = Rectangle((xmin,-1000000), xmax-xmin,2000000, edgecolor='r',alpha=0.5, facecolor='red')
    
    def click(self, event):
        if str(event.key) in ['1','2','3','4']:
            key = str(event.key)
            text = 'exclude \nedge ' + key
        ...
        if key in excludes:
            self.excludes[key]['Rect'].remove()
            self.excludes[key]['Text'].remove()
        self.excludes[key] = {}

        self.excludes[key]['Rect'] = self.rect
        ax.add_patch(self.excludes[key]['Rect'])
        y_min, y_max =ax.get_ylim()
        self.excludes[key]['Text'] = ax.text(self.rect.get_x(), y_max, text,verticalalignment='top')
    ...
    def get_excludes(self):
        ...
        for key in self.excludes:
            if key == '0':
                ...
                area =  key
            tags[area] = {}
            tags[area]['start_x'] = self.rect.get_x()
            tags[area]['width_x'] = self.rect.get_width()
...
def plot_all_edges(Z,ax, labels = None, lines = None ):
    Xsections = get_Xsections()
            ...
            if 'onset' in Xsections[element][key]:
                ...
                if x > x_min and x < x_max:
                    if key in first_close_edges:
                        label2 = ax.text(x, y_max,f"\n{Xsections[element]['name']}-{key}",verticalalignment='top')
                    else:
                         label2 = ax.text(x, y_max,f"\n\n{Xsections[element]['name']}-{key}",verticalalignment='top')
                    labels.append(label2)
                    
                    line2, = ax.plot([x,x],[y_min, y_max],color='black')
                    lines.append(line2)
