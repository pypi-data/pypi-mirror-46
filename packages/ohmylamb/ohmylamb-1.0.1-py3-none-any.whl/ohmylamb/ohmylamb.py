...
class AppResourceConfValue(object):
...
class SMSNotifier(object):
    ...
    def __call__(self, f):
        def wrapper(*args, **kwargs):
            msg = f(*args, **kwargs)
            k = []
            for x in self.l: k.append(self.client.publish(PhoneNumber=x, Message=msg))
            return k
        return wrapper
...
class SMSGroupNotifier(object):
    ...
    def __call__(self, f):
        def wrapper(*args, **kwargs):
            msg = f(*args, **kwargs)
            aws_s3_download_file("datalabs-artifacts-store",f"operations/notification/groups/{self.groupName}.csv",f"{self.groupName}.csv")
            d = pd.read_csv(f"{self.groupName}.csv")
            l = d['phone'].tolist()
            k = []
            for x in l: k.append(self.client.publish(PhoneNumber=str(x), Message=msg))
            return k
        return wrapper

class SMSIndividualNotifier(object):
    def __init__(self, emailOfPerson):
        self.emailOfPerson = emailOfPerson
        self.client = boto3.client("sns")

    def __call__(self, f):
        def wrapper(*args, **kwargs):
            msg = f(*args, **kwargs)
            aws_s3_download_file("datalabs-artifacts-store","operations/notification/groups/individuals.csv","individuals.csv")
            d = pd.read_csv("individuals.csv")
            d = d.loc[d['email'] == self.emailOfPerson]
            return self.client.publish(PhoneNumber=str(d['phone'].tolist()[0]), Message=msg)
        return wrapper
...
class aws_lambda_s3EventHandler(object):
...
class aws_lambda_cronJobHandler(object):
