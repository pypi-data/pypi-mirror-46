...
def get_dim(tensor, dim):
    if tensor.get_shape()[dim].value is not None:
        return tensor.get_shape()[dim].value
    else:
...
def rasterize_clip_space(clip_space_vertices, attributes, triangles,
  ...
  batch_size = get_dim(clip_space_vertices, 0)
  vertex_count = get_dim(clip_space_vertices, 1)
  ...
  def mapper(im):
    barycentric_coords, triangle_ids, _ = (
        rasterize_triangles_module.rasterize_triangles(
            ...
            image_height))
    print("barycentric_coords", barycentric_coords)
    ...
    reindexed_ids = tf.add(vertex_ids, im * get_dim(clip_space_vertices, 1))
    print("reindexed_ids", reindexed_ids)
    return tf.reshape(barycentric_coords, [-1, 3]), reindexed_ids

  barycentric_coordinates, vertex_ids = tf.map_fn(
    mapper, tf.range(get_dim(clip_space_vertices, 0)),
    dtype=(tf.float32, tf.int32))
  barycentric_coordinates = tf.reshape(barycentric_coordinates, [-1, get_dim(barycentric_coordinates, 2)])
  vertex_ids = tf.reshape(vertex_ids, [-1, get_dim(vertex_ids, 2)])
