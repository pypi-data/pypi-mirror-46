...
import pv_prompt.helpers as hlp
...
class MainMenu(BasePrompt):
    ...
    async def _toggle_verbose(self, *args, **kwargs):
        pv_prompt.settings.GLOBALS[
            pv_prompt.settings.ATTR_VERBOSITY
        ] = not pv_prompt.settings.GLOBALS[pv_prompt.settings.ATTR_VERBOSITY]

        print_key_values(
            "Verbosity",
            "on"
            if pv_prompt.settings.GLOBALS[pv_prompt.settings.ATTR_VERBOSITY]
            else "off",
        )
