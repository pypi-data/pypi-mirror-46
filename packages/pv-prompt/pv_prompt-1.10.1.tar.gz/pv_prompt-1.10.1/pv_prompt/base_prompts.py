...
class BasePrompt:
    def __init__(self, commands=None, loop=None):
        ...
        self._commands = {
            "q": Command(
                function_=self.quit, label="(q)uit", show_only_label=True
            ),
            "b": Command(
                autoreturn=True, label="(b)ack", show_only_label=True
            ),
        }
        if commands:
    ...
    def _toolbar_string(self):
        def command_name(key, value: Command):
            if value.show_only_label:
                return value.name
            else:
                return "({}) {}".format(key, value.name)

        _str = " | ".join(
            (command_name(key, value) for key, value in self._commands.items())
        )
        return _str
