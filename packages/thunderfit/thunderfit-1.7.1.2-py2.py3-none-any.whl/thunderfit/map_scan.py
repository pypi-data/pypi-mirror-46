...
def main():
    ...
    if (arguments.get('clip_data',
                      False) and not arguments.get('clips',
                                                   False)) or arguments.get('bg_first_only',
                                                                            False) or arguments.get('peakf_first_only',
                                                                                                    False) or arguments.get('bounds_first_only',
                                                                                                                            False):  # then we want the user to decide which thunder object to use
    ...
    utili.save_fit_report(
        getattr(
            ...
            'stats'),
        path=dirname,
        filename=f"{file_name}_report.json")
    utili.save_fit_report(
        getattr(
            bag,
            'fit_params'),
        path=dirname,
        filename=f"{file_name}_peak_info.json")
    ...
    utili.save_fit_report(
        arguments,
        path=dirname,
        filename=f"{file_name}_inpargs.json")
