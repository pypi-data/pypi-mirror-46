...
void Worker(const Contractor& contractor, const Config& config,
  ...
  std::vector<Box::IntervalVector> local_stack;
  local_stack.reserve(1000);
  ...
  while ((*found_delta_sat == -1) &&
    ...
    // if (cnt % 10000 == id * 300) {
    //   DREAL_LOG_CRITICAL(
    //       "Thread #{:>3}\t Local Stack: {}\t Global Stack: {}\t Num Boxes:
    //       {}", id, local_stack.size(), *global_stack_size, *number_of_boxes);
    // }
    ...
    if (need_to_pop) {
      if (!local_stack.empty()) {
        ...
        // std::cerr << "L+ ";
      } else if (global_stack_size->load(std::memory_order_acquire) == 0 ||
                 !global_stack->pop(current_iv)) {
    ...
    while (!local_stack.empty() &&
           global_stack_size->load(std::memory_order_acquire) == 0) {
      global_stack_size->fetch_add(1, std::memory_order_relaxed);
    ...
    // Populating Global...
    if (global_stack_size->load(std::memory_order_acquire) == 0 &&
        !filling->test_and_set()) {
      // std::cerr << "F" << id;
