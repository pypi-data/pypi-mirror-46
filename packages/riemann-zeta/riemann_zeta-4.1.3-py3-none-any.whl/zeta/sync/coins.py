...
async def _get_address_unspents(
    ...
    # start a new task checking on recently spent prevouts
...
async def check_history_item_for_spent_prevouts(
    ...
    outpoints_spent = [txin.outpoint.hex()
                       for txin in historical_tx['object'].tx_ins]

    # NB: check those against our recently spent prevouts
    recently_spent_by_this_tx = filter(
        lambda p: p['outpoint'] in outpoints_spent,
        recently_spent)
