...
class RemoteFile(Object):
    ...
    Args:
        id (:obj:`str`):
            Remote file identifier; may be emptyCan be used across application restarts or even from other devices for the current userIf the ID starts with "http://" or "https://", it represents the HTTP URL of the fileTDLib is currently unable to download files if only their URL is knownIf downloadFile is called on such a file or if it is sent to a secret chat, TDLib starts a file generation process by sending updateFileGenerationStart to the client with the HTTP URL in the original_path and "#url#" as the conversion stringClients should generate the file by downloading it to the specified location
    ...
    def __init__(self, id, is_uploading_active, is_uploading_completed, uploaded_size, **kwargs):
    ...
    def read(q: dict, *args) -> "RemoteFile":
        ...
        return RemoteFile(id, is_uploading_active, is_uploading_completed, uploaded_size)
