...
class AverageMetric(object):
    ...
    def __eq__(self, other):  # Used only for testing
        rel_tol = 1e-09
        abs_tol = 0.0
...
class BaseCallback(BaseCallbackImages):
    ...
    def epoch_end(self, epoch, metric_data=None, **kwargs):
        ...
        computed_values = self._update_computed_metrics(metric_data)
    ...
    def _update_computed_metrics(self, metric_data):
        computed_values = {}
        for func_name, func in self._COMPUTED_METRICS.items():
            for metric_name, metric_value in metric_data.items():
                self._update_computed_metric(computed_values, func, func_name, metric_name, metric_value)
        self._computed_metrics.update(computed_values)
        return computed_values

    def _update_computed_metric(self, computed_values, func, func_name, metric_name, metric_value):
        computed_name = self._build_computed_metric_name(func_name, metric_name)
        try:
            if computed_name in self._computed_metrics:
                computed_values[computed_name] = func(metric_value, self._computed_metrics[computed_name])
