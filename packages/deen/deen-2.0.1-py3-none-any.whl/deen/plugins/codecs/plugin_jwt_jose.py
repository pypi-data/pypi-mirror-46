...
class DeenPluginJwtJose(DeenPlugin):
    name = 'jwt-jose'
    display_name = 'JWT-JOSE'
    cmd_name = 'jwt-jose'
    cmd_help='Encode/Decode JSON Web Tokens (RFC 7519)'

    def __init__(self):
        super(DeenPluginJwtJose, self).__init__()
        self.secret = None
        self.key = None
    ...
    def process(self, data, secret=b'', algo='HS256', key=None):
        super(DeenPluginJwtJose, self).process(data)
        if not JOSE:
    ...
    def unprocess(self, data, secret=b'', verify=False, algo='HS256',
                  key=None):
        super(DeenPluginJwtJose, self).unprocess(data)
        if not JOSE:
            self.error = MissingDependencyException('python-jose module missing')
            return data
