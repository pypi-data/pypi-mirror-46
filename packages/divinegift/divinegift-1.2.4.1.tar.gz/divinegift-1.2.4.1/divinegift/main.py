...
def get_list_files(path: str, filter=None, filter_not_contain: str = None, add_path: bool = False, path2: str = ''):
    ...
    if filter_not_contain:
        filter = r'^((?!{filter_not_contain}).)*$'.f
        ormat(filter_not_contain=filter_not_contain)
    if filter:
        if type(filter) == list:
            for f in filter:
...
class Yaml(BaseMl):
    ...
    def create(self):
        ...
        with open(os.path.join(self.fpath, self.fname), 'w', encoding=self.encoding) as outfile:
            response = yaml.dump(self.data, outfile, default_flow_style=False, indent=4, allow_unicode=True)
