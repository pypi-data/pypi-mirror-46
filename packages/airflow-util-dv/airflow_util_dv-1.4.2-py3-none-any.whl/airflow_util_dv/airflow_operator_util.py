...
class FileOracleOperator(BaseOperator):
    ...
    def execute(self, context):
        try:
            self.sql = return_sql(self.log, self.sql_file)
...
def return_sql(log, sql_name, need_chinese=True):
    r"""
    return sql list
    :param sql_path:
    :param sql_name:
    ...
    try:
        ...
        log.info("exec throw a exception " + str(e))


class FileSensor(BaseSensorOperator):
    r"""
        examine file flag's existence.
        file can be one single file or a file list
    ...
    def __init__(self, file, *args, **kwargs):
        ...
        # override poke interval default 60
        self.poke_interval = 30
        try:
            self.ssh_host = kwargs['ssh_host']
        except Exception:
            print('Warning! do not need ssh!')
            self.ssh_host = None

    def poke(self, context):
        try:
            self.log.info('Poking: %s', self.file)
            if self.ssh_host:
                return self.ssh_file_is_exists(self.file)
            else:
    ...
    def ssh_file_is_exists(file):
        if type(file) == str:
            resp = subprocess.call('test -e ' + pipes.quote(file), shell=True)
            if resp == 0:
                return True
            else:
                return False
            ...
            for f in file:
                resp = subprocess.call('test -e ' + pipes.quote(f), shell=True)
                if resp != 0:
                    return False
            return True