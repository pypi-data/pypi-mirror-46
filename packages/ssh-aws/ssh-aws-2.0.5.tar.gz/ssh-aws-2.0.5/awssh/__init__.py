...
def select_instance(args, instances, parser):
    try:
        server_selection = int(input("Enter server number: "))  # User selection
        if instances[server_selection].public_ip_address is None:
            for instance in instances:
                if instance.key_name == instances[server_selection].key_name and instance.public_ip_address is not None:
                    # ssh_instance, jump_server, args
                    if jump_connect(instances[server_selection], instance, args):
                        break
        else:
