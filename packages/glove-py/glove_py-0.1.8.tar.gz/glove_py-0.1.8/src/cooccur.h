...
public:
    CoMat(unsigned long long vocab_size, std::string input_file,
          std::string cofile, unsigned long long window, bool symmetric,int memory_limit,string log_dir,std::string temp_file);

    void write_chunk(std::list<CoRec> &co, std::ofstream &out);

    void merge_files(int fidCounter, std::string cooccurFileName, std::string temp_file);
    ...
    void write_high_cooccur(std::string &temp_file, std::vector<double> &bigram_table);
