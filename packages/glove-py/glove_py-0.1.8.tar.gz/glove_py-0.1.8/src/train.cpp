...
Train::Train(unsigned long embed_size, unsigned long long vocab_size, int threads, 
             unsigned long epoch, std::string cofile, double lr, string log_dir, string vocab_file,
             string embd_file) : embed_size(embed_size),
...
void Train::train() {
    ...
    for (unsigned long _epoch = 0; _epoch != epoch; ++_epoch) {
        ...
        for (uint32_t threadId = 0; threadId != threads; ++threadId) {
            vec_threads.emplace_back(&Train::single_thread, this, cofile, threadId,
...
double Train::single_thread(const std::string &cofile, uint32_t threadId, double &loss, double lr) {
    std::ifstream in(log_dir + cofile);
    assert(in);
...
void Train::to_txt(const Vocabulary &v) const {
    std::ofstream os;
    file::open(os, log_dir + embd_file + ".meta");
    os << embed_size << std::endl;
    os.close();

    file::open(os, log_dir + embd_file);
    ...
    file::open(os, log_dir + vocab_file);
    for (std::uint32_t i = 0; i != W1->rows(); ++i) {
        os << v[i] << endl;
    }
    os.close();
