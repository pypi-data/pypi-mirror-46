...
//    file::open(input, input_file);
    ...
    std::ofstream foverflow(log_dir + temp_file + std::to_string(fidCounter));
    while (!input.eof()) {
        if (ind >= overflow_length) { // If overflow buffer is (almost) full, sort it and write it to temporary file
            std::ofstream foverflow(log_dir + temp_file + std::to_string(fidCounter),
...
//                如果为共现次数较少的三元组
    ...
    write_high_cooccur(temp_file, bigram_table);

    merge_files(fidCounter + 1, cofile, temp_file);
...
void CoMat::write_high_cooccur(std::string &temp_file, std::vector<double> &bigram_table) {
    //如果是共现矩阵左上角高频共现词
...
//            std::cout << "bigram_table..." << bigram_table.size() << std::endl;
    ...
    std::ofstream out(log_dir + temp_file + std::to_string(fidCounter), std::ios::out | std::ios::binary);
    assert(out);
...
void CoMat::merge_files(int num, std::string cofile, std::string temp_file) {
...
//    std::vector<std::ifstream> fid;
    std::ofstream fout(log_dir + cofile, std::ios::out | std::ios::binary);

    /* Open all files and add first entry of each to priority queue */

    std::ifstream fid[num];
    for (i = 0; i < num; i++) {
        fid[i].open(log_dir + temp_file + std::to_string(i), std::ios::out | std::ios::binary);
    }
...
CoMat::CoMat(unsigned long long vocab_size, std::string input_file,
             std::string cofile,
             unsigned long long window, bool symmetric, int memory_limit, string log_dir,std::string temp_file
