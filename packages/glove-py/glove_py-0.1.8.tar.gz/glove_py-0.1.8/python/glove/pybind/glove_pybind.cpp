...
PYBIND11_MODULE(glove_pybind, m) {
    ...
    py::class_<Glove>(m, "glove")
            ...
            .def_readwrite("log_dir", &Glove::log_dir)
            .def_readwrite("cofile", &Glove::cofile)
            .def_readwrite("vocab_file", &Glove::vocab_file)
            .def_readwrite("temp_file", &Glove::temp_file)
            .def_readwrite("embd_file", &Glove::embd_file)
            ...
            .def_readwrite("keep_case", &Glove::keep_case)
            .def_readwrite("symmetric", &Glove::symmetric)
            ...
            .def(py::init<std::string, std::string, std::string, std::string, std::string, std::string,
                    ...
                    unsigned long, unsigned long, int,  double, bool, bool>())

            .def("to_txt", [](Glove &m) { m.to_txt(); })

            .def("train", [](Glove &m, string input_file) { m.train(input_file); })

            .def("most_similary", [](Glove &m, string word, int num) {
                return m.most_similary(word, num);
            })

            .def("load", [](Glove &m, std::string vocab_file, std::string wordvec_file, std::string meta_filel) {
                m.load(vocab_file, wordvec_file, meta_filel);
            });
