...
def generate_data_files(*args):
    ...
    for path in args:
        if os.path.isfile(path):
            data_files.append( ( os.path.dirname(os.path.join(tempfile.gettempdir(),NAME)), [path]) )
        elif os.path.isdir(path):
            for p in package_files(path):
                data_files.append( ( os.path.dirname(os.path.join(tempfile.gettempdir(),NAME,p)), [p]) )
