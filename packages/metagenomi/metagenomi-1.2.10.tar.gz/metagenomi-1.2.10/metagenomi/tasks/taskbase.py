...
class MgTask(MgObj):
    ...
    # def _delete_from_list_DEPRECATE(self, key):
    #     response = self.db.table.query(
    #         KeyConditionExpression=Key('mgid').eq(self.mgid))
    #
    #     if len(response['Items']) < 1:
    ...
    #                 print('No unique_id in ', item)
    #                 i_list.append(list_to_search.index(item))
    #
    #     for i in i_list:
    #         print(f'would delete at idex {i}')
    #         result = self.db.table.update_item(
    #             Key={
    #                 'mgid': self.mgid
    #             },
    #             UpdateExpression=f"REMOVE #mg[0]",
