...
def f1(driver, num):
    ...
    for tr in trs:
        ...
        if re.findall(r'^[\[【](\w+)[\]】]', title):
            ...
            info = json.dumps({'gglx': diqu}, ensure_ascii=False)
        else:
            info = None
...
data = [
    ["zfcg_gqita_cgxx_gg",
     "http://zfcg.xjcz.gov.cn/djl/cmsPublishAction.do?method=selectCmsInfoPublishList&channelId=1231",
     ["name", "ggstart_time", "href", "info"], add_info(f1, {'lx': '采购信息'}), f2],
    # #
    ["zfcg_zhaobiao_gg",
     "http://zfcg.xjcz.gov.cn/djl/cmsPublishAction.do?method=selectCmsInfoPublishList&channelId=1232",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    #
    ["zfcg_zhongbiao_gg",
     "http://zfcg.xjcz.gov.cn/djl/cmsPublishAction.do?method=selectCmsInfoPublishList&channelId=1233",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    #
    ["zfcg_gqita_bian_liu_gg",
     "http://zfcg.xjcz.gov.cn/djl/cmsPublishAction.do?method=selectCmsInfoPublishList&channelId=1235",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    # # #
    ["zfcg_zhaobiao_fscg_gg",
     "http://zfcg.xjcz.gov.cn/djl/cmsPublishAction.do?method=selectCmsInfoPublishList&channelId=1234",
...
def work(conp, **args):
    est_meta(conp, data=data, diqu="新疆省克拉玛依市", **args)
    est_html(conp, f=f3, **args)


if __name__ == '__main__':
    work(conp=["postgres", "since2015", "192.168.3.171", "guoziqiang", "kelamayi"])

    # driver=webdriver.Chrome()
    # url = "http://zfcg.xjcz.gov.cn/djl/cmsPublishAction.do?method=selectCmsInfoPublishList&channelId=1233"
    # driver.get(url)
    # for i in range(3, 5):
    #     df=f1(driver, i)
    #     print(df.values)
