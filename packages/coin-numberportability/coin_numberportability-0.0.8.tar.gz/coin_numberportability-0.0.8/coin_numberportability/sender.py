...
class Sender:
    ...
    def send_request(self, request, url, json):
        method = request.__name__
        headers = self.security_service.generate_headers(url, method, json)
        cookie = self.security_service.generate_jwt()
...
def send_message(message):
    ...
    return sender.send_request(requests.post, url, message_dict)
