...
class SecurityService:
    ...
    def load_private_key(self, filename=None):
        filename = filename or config.privateKeyFile
    ...
    def load_shared_key(self, private_key,
                        filename=None):
        filename = filename or config.encryptedSharedKeyFile
        encrypted_key = None
        with open(filename, 'rb') as f:
    ...
    def generate_headers(self, url, method='get', request=None):
        ...
        authorization = f'hmac username="{config.consumerName}", algorithm="hmac-sha256", ' \
