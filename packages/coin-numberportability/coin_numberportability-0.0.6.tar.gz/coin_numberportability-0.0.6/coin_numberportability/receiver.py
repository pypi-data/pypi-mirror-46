...
from coin_numberportability.domain import MessageType, ConfirmationStatus, Config
...
class Receiver(ABC):
    ...
    def _get_stream(self, url, offset: int, confirmation_status: ConfirmationStatus, message_types: [MessageType]):
        ...
        headers = self._security_service.generate_headers(url)
    ...
    def start_stream(self, offset: int = None, confirmation_status: ConfirmationStatus = None, message_types: [MessageType] = None):
        response = self._get_stream(self._config.sseUrl, offset, confirmation_status, message_types)
