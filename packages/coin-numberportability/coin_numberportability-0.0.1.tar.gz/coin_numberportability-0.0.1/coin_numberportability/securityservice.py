...
import config
from config import consumerName
...
class SecurityService:
    ...
    def load_private_key(self, filename=config.privateKeyFile):
    ...
    def load_shared_key(self, private_key,
                        filename=config.encryptedSharedKeyFile):
        encrypted_key = None
        with open(filename, 'rb') as f:
    ...
    def generate_headers(self, url, method='get', request=None):
        ...
        authorization = f'hmac username="{self.issuer}", algorithm="hmac-sha256", ' \
