...
class WorkspaceManager(object):
    ...
    def display_status(self, configuration, entity='sample', submission_id=None, filter_active=True, num_threads=12):
        """
        Display summary of task statuses
        """
        # workflow status for each sample (from latest/current run)
        status_df = self.get_entity_status(entity, configuration)
        print(status_df['status'].value_counts())
        if filter_active:
            status_df = status_df[status_df['status']!='Succeeded']
        if submission_id is not None:
            status_df = status_df[status_df['submission_id']==submission_id]

        status_df = status_df[status_df['status']=='Running']

        # get metadata for all submissions
        metadata = {}
        for k,i in enumerate(status_df.index, 1):
            print('\rFetching metadata for sample {}/{}'.format(k, status_df.shape[0]), end='')
            try:
                metadata[i] = self.get_workflow_metadata(status_df.loc[i, 'submission_id'], status_df.loc[i, 'workflow_id'])
            except:
                pass

        # metadata = {}
        # with mp.Pool(processes=num_threads) as pool:
        #     for k,(res,i) in enumerate(zip(pool.starmap(
        #         self.get_workflow_metadata,
        #         [(status_df.loc[j, 'submission_id'], status_df.loc[j, 'workflow_id'], ) for j in status_df.index]
        #     ), status_df.index), 1):
        #         metadata[i] = res
        #         print('\rFetching metadata for sample {}/{}'.format(k, status_df.shape[0]), end='')
        # def get_workflow_metadata(self, submission_id, workflow_id):
        ...
        # get task list
        workflow_tasks = np.unique([j for k in metadata for j in metadata[k]['calls']])
        state_df = pd.DataFrame(
            {i:{k.split('.')[-1]:metadata[i]['calls'][k][-1]['executionStatus'] for k in metadata[i]['calls']} for i in status_df.index}
        ).T

        # state_df

        summary_df = pd.concat([state_df[c].value_counts() for c in state_df], axis=1, sort=True).fillna(0).astype(int)
        print(summary_df)
        print(status_df['status'].value_counts())

        state_df[['workflow_id', 'submission_id']] = status_df.loc[state_df.index, ['workflow_id', 'submission_id']]
