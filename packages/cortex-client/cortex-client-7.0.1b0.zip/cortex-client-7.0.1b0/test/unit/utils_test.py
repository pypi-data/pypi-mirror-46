...
class TestUtils(unittest.TestCase):
    ...
    def test_bad_token_throws(self):
        with self.assertRaisesRegexp(BadTokenException, 'Your Cortex Token is invalid.  Refer to https://docs.cortex.insights.ai/docs/cortex-tools/access/ for more information.'):
            decode_JWT('nonsensicaltoken', verify=False)
    @mocketize
    def test_nonsense_tooken_throws(self):
        with self.assertRaisesRegexp(BadTokenException, 'Your Cortex Token is invalid.  Refer to https://docs.cortex.insights.ai/docs/cortex-tools/access/ for more information.'):
            decode_JWT('nonsensicaltoken', verify=False)
    @mocketize
    ...
    def test_cortex_token_expired_throws(self):
        ...
        """
        with self.assertRaisesRegexp(BadTokenException, 'Your Cortex Token has expired.  Refer to https://docs.cortex.insights.ai/docs/cortex-tools/access/ for more information.'):
