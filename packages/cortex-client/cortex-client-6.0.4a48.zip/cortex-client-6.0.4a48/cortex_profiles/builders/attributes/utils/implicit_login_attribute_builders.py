...
def derive_statistical_summary_for_daily_login_counts(sessions_df:pd.DataFrame, df_timeframe:str="eternally") -> List[ObservedProfileAttribute]:

    groups = implicit_attribute_builder_utils.group_daily_login_counts(sessions_df)

    if isinstance(groups, pd.DataFrame) and pd.DataFrame.empty:
        return []
...
def derive_statistical_summary_for_daily_login_durations(sessions_df:pd.DataFrame, df_timeframe:str="eternally") -> List[ObservedProfileAttribute]:
