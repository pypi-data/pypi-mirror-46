...
class Gateway(Item):
    ...
    def save_to_db(self, db=None):
        try:
            if db is None:
                ...
                Database.insert("""INSERT OR IGNORE INTO GATEWAYS (NAME, IP, PORT, KEY1, KEY2, TYPE) VALUES
                            (:name, :ip, :port, :key1, :key2, :type)""",
                            {'name': self.id, 'ip': self.ip, 'port': self.port,
                             'key1': self.key1, 'key2': self.key2, 'type': self.type})

                Database.update("""UPDATE OR IGNORE GATEWAYS SET IP = :ip, PORT = :port, KEY1 = :key1,
    ...
    def delete(self, db=None):
        try:
            Database.delete("DELETE FROM GATEWAYS WHERE NAME == :key", {'key': self.id}, db)
            return True
        except:
            return False
    ...
    def load_all_from_db(query, params, db=None):
        gateways = []
        for item in Database.select_all(query, params, db):
