...
from Homevee.Utils import NotificationManager
...
class ChatMessage(Item):
    ...
    def send(self):
        ...
        results = Database.select_all("SELECT USERNAME FROM USERDATA WHERE USERNAME != :user", {'user': self.username})
        users = []
        ...
        NotificationManager().send_notification_to_users(users, "Neue Nachricht von " + self.username,
                                                       json.loads(self.data)['msg'], Database.get_database_con(),
    ...
    def save_to_db(self, db=None):
        try:
            if (self.id is None or self.id == ""):
                Database.insert("""INSERT INTO CHAT_DATA (USERNAME, DATA) VALUES (:username, :data, :timestamp)""",
                            {'username': self.username, 'data': self.data, 'timestamp': time.time()}, db)
        except:
            if(Logger.IS_DEBUG):
                traceback.print_exc()
    ...
    @staticmethod
    def load_all_by_time(time, limit, db=None):
        where_clause = ""
        params = {'limit': limit}
