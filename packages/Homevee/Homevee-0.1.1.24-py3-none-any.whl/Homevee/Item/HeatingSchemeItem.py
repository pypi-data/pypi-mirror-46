...
class HeatingSchemeItem(Item):
    ...
    def delete(self, db=None):
        try:
            Database.delete("DELETE FROM HEATING_SCHEME WHERE ID == :id", {'id': self.id}, db)
            Database.delete("DELETE FROM HEATING_SCHEME_DAYS WHERE HEATING_SCHEME_ID == :id", {'id': self.id}, db)
            Database.delete("DELETE FROM HEATING_SCHEME_DEVICES WHERE ID == :id", {'id': self.id}, db)
            return True
        except:
            return False
    ...
    def load_all(db=None):
        return HeatingSchemeItem.load_all_from_db('SELECT * FROM EVENTS', {}, db)

    @staticmethod
    def load_all_by_day(day, db=None):
