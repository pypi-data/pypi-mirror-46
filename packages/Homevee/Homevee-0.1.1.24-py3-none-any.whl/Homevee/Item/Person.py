...
class Person(Item):
    ...
    def delete(self, db=None):
        try:
            Database.delete("DELETE FROM PEOPLE_DATA WHERE ID == :id", {'id': self.id}, db)
            return True
        except:
            return False
    ...
    def save_to_db(self, db=None):
        try:
            if (self.id is None or self.id == ""):
                new_id = Database.insert("""INSERT INTO PEOPLE_DATA (NAME, NICKNAME, PHONE_NUMBER, ADDRESS, BIRTHDATE, LONGITUDE, 
                LATITUDE) VALUES (:name, :nickname, :phone_number, :address, :birthdate, :longitude, :latitude)""",
                            {'name': self.name, 'nickname': self.nickname, 'phone_number': self.phone_number,
                             ...
                             'latitude': self.latitude}, db)
                self.id = new_id

            # update
            else:
                Database.update("""UPDATE PEOPLE_DATA SET NAME = :name, NICKNAME = :nickname, PHONE_NUMBER = :phone_number,
                 ADDRESS = :address, BIRHTDATE = :birthdate, LONGITUDE = :longitude, LATITUDE = :latitude WHERE ID = :id""",
                                {'name': self.name, 'nickname': self.nickname, 'phone_number': self.phone_number,
                                 'address': self.address, 'birthdate': self.birthdate, 'longitude': self.longitude,
                                 'latitude': self.latitude, 'id': self.id}, db)
