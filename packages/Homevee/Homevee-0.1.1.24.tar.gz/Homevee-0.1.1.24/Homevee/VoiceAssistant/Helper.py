...
def set_context(user, context_key, context_data, db):
    ...
    Database.insert("INSERT INTO VOICE_COMMAND_USER (USERNAME, CONTEXT_TIME, CONTEXT_KEY, CONTEXT_DATA) VALUES (:username, :time, :key, :data)",
                {'username': user.username, 'time': int(time.time()), 'key': context_key, 'data': context_data}, db)

def get_context(user, db):
    data = Database.select_one("SELECT * FROM 'VOICE_COMMAND_USER' WHERE USERNAME == :user",
                {'user': user.username}, db)
...
def replace_voice_commands(text, user, db):
