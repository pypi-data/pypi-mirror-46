    ...
    def save_to_db(self, db=None):
        try:
            ...
            if(self.id is None or self.id == ""):
                Database.insert("INSERT INTO ROOMS (NAME, ICON) VALUES (:name, :icon)",
                            {'name': self.name, 'icon': self.icon}, db)
            #update
            else:
                Database.update("UPDATE ROOMS SET NAME = :name, ICON = :icon WHERE LOCATION = :id",
                            {'name': self.name, 'icon': self.icon, 'id': self.id}, db)
