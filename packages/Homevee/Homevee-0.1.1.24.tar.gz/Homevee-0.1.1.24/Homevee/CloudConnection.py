...
class CloudConnection():
    ...
    def save_cloud_cert(self, cloud_ip):
        cloud_cert = HomeveeCloudWrapper().get_cloud_cert(cloud_ip)

        open(Constants.LOCAL_SSL_CERT, "wt").write(cloud_cert)
    ...
    def do_connection(self):
        ...
        cloud_to_use = HomeveeCloudWrapper().get_cloud_to_use()
        Logger.log("Cloud to use: "+str(cloud_to_use))
        ...
        try:
            ...
            #Neues SSL-Verfahren (Instabil)
            #self.save_cloud_cert(cloud_to_use)
            #conn = ssl.wrap_socket(conn, ca_certs=Constants.CLOUD_SSL_CERT)#,cert_reqs=ssl.CERT_REQUIRED, ssl_version=ssl.PROTOCOL_SSLv23)


            #Altes SSL-Verfahren (Stabil)
