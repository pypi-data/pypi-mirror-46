...
class SaveSensorDataCronjob(FixedTimeCronjob):
    ...
    def save_to_db(self, sensor, db=None):
        time = datetime.datetime.now().strftime('%Y-%m-%d %H:00')

        Database.insert("INSERT INTO SENSOR_DATA (DEVICE_ID, DEVICE_TYPE, DATETIME, VALUE) \
                        VALUES (:id, :type, :time, :value)",
                        {'id': sensor.id, 'type': sensor.get_device_type(), 'time': time, 'value': sensor.value}, db)