...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    results = db.select_all("SELECT DISTINCT(CONTROL_KEY) FROM (SELECT * FROM AR_CONTROL_LEARNING_DATA WHERE CONTROL_KEY IS NOT NULL)", {})
    ...
    for label in labels:
        results = db.select_all("SELECT * FROM AR_CONTROL_LEARNING_DATA WHERE CONTROL_KEY = :key", {"key": label})
...
def ar_training(username):
    ...
    training_steps = db.get_server_data('AR_TF_TRAINING_STEPS')
    ...
    model_size = db.get_server_data('AR_TF_MODEL_SIZE')
    image_size = db.get_server_data('AR_TF_MODEL_IMAGE_SIZE')
    try:
    ...
    NotificationManager().send_notification_to_users(username, "AR-Steuerung", "Das Training der AR-Steuerung ist abgeschlossen.")
