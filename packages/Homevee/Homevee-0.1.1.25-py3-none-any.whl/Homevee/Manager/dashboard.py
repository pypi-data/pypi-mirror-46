...
def get_user_dashboard(user, db: Database = None):
    if db is None:
        db = Database()
    ...
    if dashboard_data is not None and dashboard_data != "":
        ...
        for item in data['dynamic']:
            ...
            if not found:
                dashboard_data['dynamic'].append(get_device_info(user, item['type'], item['id']))

        return dashboard_data

    return {'result': 'unknownuser'}

def get_user_dashboard_items(user, db: Database = None):
    if db is None:
        db = Database()
    dashboard_array = []
...
def edit_user_dashboard(username, dashboard_data, db: Database = None):
    if db is None:
        db = Database()
...
def get_user_favourite_devices(user, db: Database = None):
    if db is None:
        db = Database()
    ...
    fav_array = get_dynamic_items(user, dynamic_items)

    return fav_array

def get_dynamic_items(user, dynamic_items, db: Database = None):
    if db is None:
        db = Database()
    ...
    for type in types:
        ...
        if len(ids) > 0:
            for id in ids:
                item = get_device_info(user, type, id)
...
def get_device_info(user, type, id, db: Database = None):
    if db is None:
        db = Database()
    Logger.log(type + " - " + id)
