...
def automatic(config_content: str, *, include: List[str] = None) -> "ConfigBase":
    """
    ...
    :param only: Only check the given types. Types are "asa", "ios", "nxos",
        "fortios", "hp", and "junos". By default all options will be checked.
    ...
    # IOS probably has version as the first real item
    if "ios" in include and re.match(
        "[\s!]*version \d+.\d+$", config_content, re.MULTILINE
    ):
