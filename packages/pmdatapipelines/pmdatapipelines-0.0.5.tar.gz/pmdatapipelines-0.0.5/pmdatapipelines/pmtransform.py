...
def convert_to_object(content_string):

	content_object = []
	for i, j in stream_chunk(content_string):
		event = flatten(json.loads(content_string[_i:i+1]))
		content_object.append(event)
		_i = j-1

	event = flatten_json(json.loads(content[_i:]))
	content_object.append(event)
