...
from kidx_core.constants import INTENT_MESSAGE_PREFIX
...
class NaturalLanguageInterpreter(object):
    ...
    def create(obj, endpoint=None):
        ...
        if not endpoint:
            return KidxNLUInterpreter(model_directory=obj)

        name_parts = os.path.split(obj)

        if len(name_parts) == 1:
            # using the default project name
            return KidxNLUHttpInterpreter(name_parts[0],
                                          endpoint)
        elif len(name_parts) == 2:
            return KidxNLUHttpInterpreter(name_parts[1],
                                          endpoint,
                                          name_parts[0])
        else:
            raise Exception(
...
class KidxNLUHttpInterpreter(NaturalLanguageInterpreter):
    def __init__(self,
    ...
    def parse(self, text, message_id=None):
        ...
        default_return = {"intent": {"name": "", "confidence": 0.0},
                          "entities": [], "text": ""}
        result = self._kidx_http_parse(text, message_id)

        return result if result is not None else default_return

    def _kidx_http_parse(self, text, message_id=None):
        """Send a text message to a running kidx NLU http server.

        Return `None` on failure."""

        if not self.endpoint:
            logger.error(
                "Failed to parse text '{}' using kidx NLU over http. "
                "No kidx NLU server specified!".format(text))
            return None
...
class KidxNLUInterpreter(NaturalLanguageInterpreter):
