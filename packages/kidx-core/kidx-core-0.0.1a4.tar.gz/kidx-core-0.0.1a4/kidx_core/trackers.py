...
class DialogueStateTracker(object):
    ...
    def set_slot(self, key: Text, value: Any):
    ...
    def set_slots(self, slots_: Dict[Text, Any]):
        """Batch update the value of a slot."""
        if slots_ is None or not isinstance(slots_, dict):
            logger.error("The slots arguments are invalid")
            return False

        for key, value in slots_.items():
            if key in self.slots:
                # bug here check the validation of arguments
                self.slots[key].value = value
            else:
                logger.info("Tried to access non existent "
                            "slot '{}'".format(key))
        return True
