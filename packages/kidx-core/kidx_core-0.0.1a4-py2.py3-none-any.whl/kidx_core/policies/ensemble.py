...
import kidx_core
from kidx_core import utils, training, constants
from kidx_core.actions.action import ACTION_LISTEN_NAME
from kidx_core.domain import Domain
from kidx_core.events import SlotSet, ActionExecuted, ActionExecutionRejected
from kidx_core.exceptions import UnsupportedDialogueModelError
from kidx_core.featurizers import MaxHistoryTrackerFeaturizer
from kidx_core.policies import Policy
...
class PolicyEnsemble(object):
    versioned_packages = ["kidx_core", "tensorflow", "sklearn"]
    ...
    def ensure_model_compatibility(metadata, version_to_check=None):
        ...
        model_version = metadata.get("kidx_core", "0.0.0")
        if version.parse(model_version) < version.parse(version_to_check):
            raise UnsupportedDialogueModelError(
                "The model version is to old to be "
                "loaded by this Kidx Core instance. "
                "Either retrain the model, or run with"
                "an older version. "
                "Model version: {} Instance version: {} "
                "Minimal compatible version: {}"
                "".format(model_version, kidx_core.__version__,
                          version_to_check),
                model_version)
