...
from kidx_core.actions import Action, action
from kidx_core.constants import REQUESTED_SLOT
from kidx_core.slots import Slot, UnfeaturizedSlot
from kidx_core.trackers import SlotSet
from kidx_core.utils import read_file, read_yaml_string, EndpointConfig
...
class Domain(object):
    ...
    def persist_clean(self, filename: Text) -> None:
        ...
        for slot in data["slots"].values():
            ...
            if slot["auto_fill"]:
                del slot["auto_fill"]
            if slot["type"].startswith('kidx_core.slots'):
