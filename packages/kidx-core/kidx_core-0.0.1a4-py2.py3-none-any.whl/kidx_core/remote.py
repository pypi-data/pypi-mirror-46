...
class KidxCoreClient(object):
    """Connects to a running Kidx Core server.
    ...
    def respond(self,
        ...
        """Send a parse request to a kidx core server."""
        ...
        if response.status_code == 200:
            ...
            logger.warning("Got a bad response from kidx core :( Status: {} "
    ...
    def upload_model(self,
        ...
        """Upload a Kidx core model to the remote instance."""

        logger.debug("Uploading model to kidx core server.")
        ...
        if response:
            logger.warning("Got a bad response from kidx core while uploading "
                           "the model (Status: {} "
                           "Response: {})".format(response.status_code,
                                                  response.text))
        return None
