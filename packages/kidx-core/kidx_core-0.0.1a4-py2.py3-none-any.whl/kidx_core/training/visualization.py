...
from kidx_core.events import UserUttered, ActionExecuted, Event
from kidx_core.interpreter import RegexInterpreter, NaturalLanguageInterpreter
from kidx_core.training.generator import TrainingDataGenerator
from kidx_core.training.structures import StoryGraph, StoryStep
...
def _nodes_are_equivalent(graph, node_a, node_b, max_history):
    ...
    return (graph.node[node_a]["label"] == graph.node[node_b]["label"] and
            (_outgoing_edges_are_similar(graph, node_a, node_b) or
             _incoming_edges(graph, node_a) ==
             _incoming_edges(graph, node_b) or
...
def visualize_stories(
    ...
    The training data parameter can be used to pass in a Kidx NLU training
