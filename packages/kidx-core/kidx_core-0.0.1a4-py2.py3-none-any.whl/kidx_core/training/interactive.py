    ...
    try:
        ...
        if is_new_action:
            warning_questions = questionary.confirm(
                ...
                "".format(action_name))
            await _ask_or_abort(warning_questions, sender_id, endpoint)
...
async def _request_free_text_intent(
    ...
    return await _ask_or_abort(question, sender_id, endpoint)
...
async def _request_action_from_user(
    ...
    action_name = await _ask_or_abort(question, sender_id, endpoint)
...
async def _confirm_form_validation(action_name, tracker, endpoint, sender_id):
    ...
    validate_input = await _ask_or_abort(validation_questions, sender_id, endpoint)
    ...
    if not validate_input:
        ...
        await _ask_or_abort(warning_question, sender_id, endpoint)
...
async def _validate_action(action_name: Text,
    ...
    is_correct = await _ask_or_abort(question, sender_id, endpoint)
...
async def _correct_entities(latest_message: Dict[Text, Any],
    ...
    annotation = await _ask_or_abort(question, sender_id, endpoint)
    # noinspection PyProtectedMember
...
async def _enter_user_message(sender_id: Text,
    ...
    message = await _ask_or_abort(question, sender_id, endpoint,
                                  lambda a: not a)
