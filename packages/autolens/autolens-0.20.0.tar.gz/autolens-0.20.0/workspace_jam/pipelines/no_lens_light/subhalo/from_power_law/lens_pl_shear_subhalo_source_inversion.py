...
def make_pipeline(phase_folders=None, tag_phases=True, sub_grid_size=2, bin_up_factor=None, positions_threshold=None,
    ...
    class GridPhase(autofit_ph.as_grid_search(ph.LensSourcePlanePhase)):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens = results.from_phase('phase_1_lens_pl_shear_source_inversion').constant.lens_galaxies.lens
            ...
            self.source_galaxies.source.pixelization = \
                results.from_phase('phase_2_lens_pl_shear_refine_source_inversion').constant.source_galaxies.source.pixelization

            self.source_galaxies.source.regularization = \
                results.from_phase('phase_2_lens_pl_shear_refine_source_inversion').variable.source_galaxies.source.regularization
    ...
    class SubhaloPhase(ph.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens = results.from_phase('phase_1_lens_pl_shear_source_inversion').variable.lens_galaxies.lens

            ### Subhalo, TruncatedNFW -> TruncatedNFW ###

            self.lens_galaxies.subhalo.mass.centre = \
                results.from_phase('phase_2_subhalo_search').best_result.variable_absolute(a=0.3).lens_galaxies.subhalo.mass.centre

            self.lens_galaxies.subhalo.mass.kappa_s = \
                results.from_phase('phase_2_subhalo_search').best_result.variable_relative(r=0.5).lens_galaxies.subhalo.mass.kappa_s

            self.lens_galaxies.subhalo.mass.scale_radius = \
                results.from_phase('phase_2_subhalo_search').best_result.variable_relative(r=0.5).lens_galaxies.subhalo.mass.scale_radius
            ...
            self.source_galaxies.source.regularization = \
                results.from_phase('phase_2_subhalo_search').best_result.variable.source_galaxies.source.regularization
