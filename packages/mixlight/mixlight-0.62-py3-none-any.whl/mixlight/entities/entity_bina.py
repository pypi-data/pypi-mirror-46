...
PAYLOAD_CONFIG = '{"name": "%s", "unique_id": "%s.%s", "state_topic":"%s", "device_class": "motion", "device": %s }'
PAYLOAD_STATE = '%s'


class entity_Bina(mixlight_Entity):
  ...
  def create(self):
    if self.linker.client is None or not self.name:
      return
