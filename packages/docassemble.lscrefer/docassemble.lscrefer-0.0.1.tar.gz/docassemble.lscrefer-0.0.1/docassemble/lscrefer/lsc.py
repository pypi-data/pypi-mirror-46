...
lsc_programs = DADict(object_type=Organization)
lsc_programs_by_rin = DADict(object_type=Organization)
lsc_programs_by_serv_a = DADict(object_type=Organization)
lsc_programs.set_random_instance_name()
lsc_programs_by_rin.set_random_instance_name()
lsc_programs_by_serv_a.set_random_instance_name()
...
def service_areas():
    ...
    if result is None:
        sys.stderr.write('load_service_areas: calling arcgis.\n')
...
def load_program_data():
    ...
    for program_dict in program_list:
        program = lsc_programs.initializeObject(program_dict["Serv_Area_ID"].strip())
        program.name.text = program_dict["R_Legalname"].strip()
        program.phone_number = program_dict["Local_800"].strip()
        program.url = program_dict["Web_URL"].strip()
    lsc_programs.gathered = True
    ...
    lsc_programs_by_rin.gathered = True
    lsc_programs_by_serv_a.gathered = True
...
def service_area_for(person):
    ...
    return lsc_programs_by_serv_a[service_area]
