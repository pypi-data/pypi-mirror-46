...
urlpatterns = [
    ...
    url(r'^question/record/detail/(?P<pk>[0-9]+)$', views.QuestionRecordDetail.as_view(), name='question_record_detail'),
    ...
    url(r'^user/part/update/(?P<user_part_id>[0-9]+)$', views.UserPartUpdate.as_view(), name='user_part_update'),

    # ========== 用户前台 ==============
    url(r'^custom_user/section/list/redirect/(?P<page_type>(.)+)/(?P<user_id>[0-9]+)/(?P<section_type>[0-9]+)/$', views.CustomUserSectionRedirect.as_view(), name='custom_user_section_redirect'),
    url(r'^custom_user/section/list/(?P<section_type>[0-9]+)/(?P<user_id>[0-9]+)/(?P<user_course_id>[0-9]+)$', views.CustomUserSectionList.as_view(), name='custom_user_section_list'),
    url(r'^custom_user/section/detail/(?P<section_type>[0-9]+)/(?P<pk>[0-9]+)$', views.CustomUserSectionDetail.as_view(), name='custom_user_section_detail'),
    ...
    url(r'^custom_user/part/detail/(?P<pk>[0-9]+)$', views.CustomUserPartDetail.as_view(), name='custom_user_part_detail'),


    # ============ VUE JS ===============
    url(r'^vue_user_section/(?P<user_section_id>\d+)$', views.VueUserSection.as_view(), name='vue_user_section'),
]
