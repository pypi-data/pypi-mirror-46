...
# @trace_cls('http://localhost:12222')
class TCPRelayHandler(object):
    ...
    def __init__(self, server, fd_to_handlers, loop, local_sock, config,
        ...
        self._cryptor_tunnel = None
        self._config_tunnel = None
    ...
    def _handle_dns_resolved(self, result, error):
        ...
        if self._is_local:
            ...
            # tunel change start
            if self._tunnel_mode:
                remote_port = self._config_tunnel['server_port']
            else:
        ...
        if self._is_local and self._config['fast_open']:
            ...
            if self._tunnel_mode:
                ...
                #pdb.set_trace()
                data = b''.join(self._data_to_tunnel)
                
                # self._handle_stage_stream(data)
                data = self._cryptor_tunnel.encrypt(data)
                # logging.info("dns to %s %s " % (colored(md5(data).hexdigest(), 'green'), data[:30]))
                self._data_to_write_to_remote.append(data)
                self._data_to_tunnel = []
                self._update_stream(STREAM_UP, WAIT_STATUS_WRITING)
    ...
    def _on_remote_write(self):
        ...
        if self._data_to_write_to_remote:
            data = b''.join(self._data_to_write_to_remote)
            # logging.info("send %s %s" % (colored(md5(data).hexdigest(), 'yellow'), data[:30]))
            self._data_to_write_to_remote = []
            self._write_to_sock(data, self._remote_sock)
        else:
            self._update_stream(STREAM_UP, WAIT_STATUS_READING)
...
class TCPRelay(object):
    ...
    def __init__(self, config, dns_resolver, is_local, stat_callback=None, redirect=False):
        self._config = config
        self._is_local = is_local
        self._dns_resolver = dns_resolver
        self._closed = False
