# coding=utf-8
# --------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for
# license information.
#
# Code generated by Microsoft (R) AutoRest Code Generator 2.3.33.0
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from .image_request_base import ImageRequestBase


class DockerImageRequest(ImageRequestBase):
    """Request to create a Docker image.

    :param name:
    :type name: str
    :param description:
    :type description: str
    :param driver_program:
    :type driver_program: str
    :param assets:
    :type assets: list[~_restclient.models.ImageAsset]
    :param target_runtime:
    :type target_runtime: ~_restclient.models.TargetRuntime
    :param image_type:
    :type image_type: str
    :param model_ids:
    :type model_ids: list[str]
    :param tags:
    :type tags: list[str]
    :param image_request_type_discriminator: Constant filled by server.
    :type image_request_type_discriminator: str
    :param docker_file_uri: The Uri to the docker file.
    :type docker_file_uri: str
    """

    _validation = {
        'name': {'required': True},
        'driver_program': {'required': True},
        'target_runtime': {'required': True},
        'image_request_type_discriminator': {'required': True},
    }

    _attribute_map = {
        'name': {'key': 'name', 'type': 'str'},
        'description': {'key': 'description', 'type': 'str'},
        'driver_program': {'key': 'driverProgram', 'type': 'str'},
        'assets': {'key': 'assets', 'type': '[ImageAsset]'},
        'target_runtime': {'key': 'targetRuntime', 'type': 'TargetRuntime'},
        'image_type': {'key': 'imageType', 'type': 'str'},
        'model_ids': {'key': 'modelIds', 'type': '[str]'},
        'tags': {'key': 'tags', 'type': '[str]'},
        'image_request_type_discriminator': {'key': 'imageRequestTypeDiscriminator', 'type': 'str'},
        'docker_file_uri': {'key': 'dockerFileUri', 'type': 'str'},
    }

    def __init__(self, name, driver_program, target_runtime, description=None, assets=None, image_type=None, model_ids=None, tags=None, docker_file_uri=None):
        super(DockerImageRequest, self).__init__(name=name, description=description, driver_program=driver_program, assets=assets, target_runtime=target_runtime, image_type=image_type, model_ids=model_ids, tags=tags)
        self.docker_file_uri = docker_file_uri
        self.image_request_type_discriminator = 'DockerImageRequest'
