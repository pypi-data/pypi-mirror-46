...
def attach_project(project_id, project_path, scope):
    ...
    if not original_project_info:
        ...
        _create_default_run_configs(project_path, scope)
...
def _create_default_run_configs(project_directory, scope):
    """
    ...
    # Get a list of all computes for this workspace
    # subscription_id = re.search(r'/subscriptions/([^/]+)', scope).group(1)
    # resource_group = re.search(r'/resourceGroups/([^/]+)', scope).group(1)
    # workspace_name = re.search(r'/workspaces/([^/]+)', scope).group(1)
    from azureml.core.workspace import Workspace
    workspace_scope = Workspace._get_scope_details(scope)
    workspace_obj = Workspace.get(
        resource_group=workspace_scope.resource_group,
        name=workspace_scope.workspace_name, subscription_id=workspace_scope.subscription_id)
    compute_targets = workspace_obj.compute_targets
    for compute_target_name, compute_target in compute_targets.items():
