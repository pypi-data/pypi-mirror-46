...
class ServiceException(AzureMLException):
    """
    An exception related to failed service calls. Contains the status code and
    ...
    def __init__(self, error_response_exception, **kwargs):
        ...
        self.details = error_response_exception.error.error.details if error_response_exception.error.error else []
        self.status_code = error_response_exception.response.status_code
        super(ServiceException, self).__init__(error_response_exception.message, error_response_exception)
