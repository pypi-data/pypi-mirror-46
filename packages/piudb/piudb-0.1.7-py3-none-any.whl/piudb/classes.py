...
class Piu:
    def __init__(self,tpath,cls=None,auto_update_fields=False,delete_fields_ok=False,i_am_sure=False,overwrite_fields=False):
        self.tpath=tpath
        self.errors=DBError()
        self._open(cls)
    ...
    def _open(self,cls):
        ...
        self.tb=Table._create_whatever(self.tpath,primary_key=self.primary_key,searchable_keys=self.searchable_keys,
                                       fields=self.fields,all_keys=self.all_keys,test_mode=True
                                       )
...
##------------------------------------代码重构----------------------------------------##
    ...
    def _insert_(self,**kws):
        ...
        return record
    async def upsert(self,pk,**kws):
        return self._upsert_(pk,**kws)
