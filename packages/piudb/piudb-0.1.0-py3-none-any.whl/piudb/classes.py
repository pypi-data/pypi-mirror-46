...
class TableManager:
    ...
    def _create(self,tpath,cls,test_mode=True):
        base_names=[i.__name__ for i in cls.__bases__]
        if (not cls is Model) and (not 'Model' in base_names):
...
class ObjectTable:
    def __init__(self,tb,cls,auto_update_fields=False,delete_fields_ok=False,i_am_sure=False):
        self.tb=tb
        self.cls=cls
