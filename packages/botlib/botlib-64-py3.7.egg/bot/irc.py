...
from obj.shell import parse_cli
...
class IRC(bot.Bot):
    ...
    def handle_privmsg(self, event):
        users.userhosts._set(event.nick, event.origin)
    ...
    def start(self):
        ...
        last = self.cfg._last()
        if last:
            self.cfg._update(last)
        prev = self.cfg._cpy()
        prev._update(self.cfg)
        self.cfg._update(obj.cfg, skip=True)
        if prev != self.cfg:
            self.cfg.save()
