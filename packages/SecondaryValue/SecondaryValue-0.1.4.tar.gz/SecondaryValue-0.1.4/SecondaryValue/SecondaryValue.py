...
T = SecondaryValue('T0 + (T1 - T0) / (R1 - R0) * R',
                   defaults=dict(T0=T0, T1=T1, R0=R0, R1=R1))

alpha = (1, 1, 1) # H / K
beta = (1, 1, 1) # H
L0 = SecondaryValue('alpha * T + beta',
                    defaults=dict(alpha=alpha, beta=beta), dependencies=dict(T=T))

ra = (1, 1, 1) # m, outer radious
ri = (1, 1, 1) # m, inner radious

Chi = SecondaryValue('(ra/ri)^2*(L/L0 - 1) + 2*(ra/ri - 1)',
                     defaults=dict(ra=ra, ri=ri),
                     dependencies=dict(L0=L0))
