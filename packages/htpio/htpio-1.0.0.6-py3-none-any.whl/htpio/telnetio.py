...
class RaspberryPi(Telnet, Device):
    ...
    def __init__(
        ...
        self._ramdisk = RaspberryPi.RAMDISK
    ...
    def islocked(self, pin):
        ...
        *** Example: ***
        ...
        # Flush command echo
        self.read_until(RaspberryPi.EOF.encode('ascii'), self.timeout)

        # Read command result
        result = self.read_until(self.shell_prompt, self.timeout)
        result = bytes(result).decode()
        result = result.split()
        result = result[0].replace(RaspberryPi.EOF, "")
    ...
    def isconfigured(self, pin):
        ...
        ***Example***:
        ...
        result = self.read_until(self.shell_prompt, self.timeout)
        result = bytes(result).decode()
        result = result.split(RaspberryPi.EOF)
    ...
    def configure(self, pin, direction):
        ...
        ***Example***:
        ...
        if not self.isconfigured(pin):
            ...
            if direction == RaspberryPi.INPUT:
            ...
            if direction == RaspberryPi.OUTPUT:
    ...
    def get(self, pin):
        ...
        ***Example***:
        ...
        value = self.read_until(RaspberryPi.EOF.encode('ascii'), self.timeout)
        value = bytes(value).decode()

        # Flush buffer
        self.read_until(self.shell_prompt, self.timeout)

        return int(value.replace(RaspberryPi.EOF, ""))
    ...
    def __islockdir(self):
        ...
        # Read command result
        result = self.read_until(RaspberryPi.EOF.encode('ascii'), self.timeout)
        result = bytes(result).decode()
        result = result.replace(RaspberryPi.EOF, "")
