...
__copyright__ = 'Copyright (c) 2015-2019 VMware, Inc.  All rights reserved. '
...
class CliCommand(object):
    ...
    def get_parser_arguments(self, command=None):
        ...
        :param command: command string; used when discriminator field is set
    ...
    def handle_discriminator(self, all_options, command):
        ...
        for arg in self.cmd_info.options:
            if arg.type == 'complex-hasfieldsof':
                ...
                # we need to remove duplicate options
                for option in discriminator_options:
                    if option.long_option in [opt.long_option for opt in self.cmd_info.options]:
                        continue
    ...
    def handle_discriminator_options(self, discriminator_option, cli_discriminable_arg, discriminable_field, all_options, command):
        ...
        list_options_output = self.call_external_command(discriminable_field.dynamic_contract_service,
                                                         'list',
                                                         [])
        ...
        for option in self.cmd_info.options:
            if option.field_name.startswith(discriminator_option.field_name[:discriminator_option.field_name.rfind('.')]) \
                ...
                option.depends_on = (discriminator_long_option, None)
        ...
        if args_values \
            ...
            get_type_command_output = self.call_external_command(discriminable_field.dynamic_contract_service,
                                                                 'get',
            ...
            for option in discriminator_additional_options:
                option.depends_on = (discriminator_long_option, [args_values[discriminator_long_option]])
            all_options.extend(discriminator_additional_options)

    def call_external_command(self, service_id, operation_id, args):
        """
        Calls a command different from the one handled by current instance

        :type  service_id: :class:`str`
        :param service_id: command's service id
        :type  operation_id: :class:`str`
        :param operation_id: command's operation id
        :type  args: :class:`list` of :class:`str`
        :param args: Command input as list of strings

        :rtype:  :class:`vmware.vapi.data.value.DataValue`
        :return: DataValue corresponding to the input type
        """
        external_command = CliCommand(
            self.server_type,
            self.connector,
        ...
        if not external_command.is_cmd:
            handle_error("Could not find command '{}' in service '{}'".format(operation_id, service_id),
                         print_error=False)
        else:
            external_command_result = \
                external_command.execute_command(self.connector.new_context(), args)
            if external_command_result.success():
                return external_command_result.output
            else:
                handle_error("There was an error while executing command '{}' on service '{}'".format(operation_id, service_id),
                             print_error=False)
        return None
