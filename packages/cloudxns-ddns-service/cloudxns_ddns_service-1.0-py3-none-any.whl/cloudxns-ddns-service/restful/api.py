...
class Api:
    ...
    def __init__(self, api_key=None, secret_key=None):
        ...
        self.__headers = {
            ...
            'user-agent': 'CloudXNS-Python/v2 (CloudXNS) Python-httplib2/0.9.2'
        }
        self.__api_key = ''
        self.__secret_key = ''
        if api_key:
    ...
    def __header_update(self, uri, data=None):
        ...
        strs = self.__api_key + __API_URL__ + uri + data + date + self.__secret_key
        md5.update(strs.encode("utf8"))
    ...
    def __request(self, method, uri, data=None):
        ...
        if method in ['POST', 'PUT']:
            ...
            if method == 'POST':
                return requests.post(url=__API_URL__ + uri, json=data, headers=self.__headers)
            else:
                return requests.put(url=__API_URL__ + uri, data=json.dumps(data), headers=self.__headers)
        elif data:
        ...
        content = requests.get(url=__API_URL__ + uri, params=data, headers=self.__headers)

        return content

    @staticmethod
    def version():
        return requests.get(__API_URL__ + 'version', params=__httplib_params__)
