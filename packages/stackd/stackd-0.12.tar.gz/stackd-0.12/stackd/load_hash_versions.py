...
def version_iter_vars(immutable_vars, var_namespace, working_dir):
...
def version_iter_yaml(yaml_file, working_dir):
  if 'secrets' in yaml_file:
    yield from version_iter_vars(yaml_file['secrets'].items(), 'SECRET', working_dir)

  if 'configs' in yaml_file:
    yield from version_iter_vars(yaml_file['configs'].items(), 'CONFIG', working_dir)
...
def load_hash_versions(files_compose=[], env_vars={}):
  for file in files_compose:
    ...
    for key, value in version_iter_yaml(parsed, '.'):
      env_vars[key] = value
