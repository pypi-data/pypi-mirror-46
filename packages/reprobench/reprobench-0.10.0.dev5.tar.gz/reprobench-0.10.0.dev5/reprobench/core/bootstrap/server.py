...
from reprobench.core.db import (MODELS, Limit, Observer, Parameter,
                                ParameterGroup, Run, Step, Task, TaskGroup,
                                Tool, db)
from reprobench.utils import (chunk_sequence, get_db_path, import_class,
                              init_db, is_range_str, parse_pcs_parameters)
    ...
    for (name, tasks) in config["tasks"].items():
        ...
        with db.atomic():
            for batch in chunked(tasks, 100):
                Task.insert_many([{"path": task, "group": name} for task in batch]).execute()
