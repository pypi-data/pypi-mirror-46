...
#depreceated intentionally left to compare with get length as get length is more general
...
def downsample(cmnd):
    ...
    if (RATE == 8000):
        ...
        wf = wave.open('downsample.wav', 'wb')
        wf.setnchannels(CHANNELS)
        wf.setsampwidth(swidth)
        Change_RATE = 0.0
        Change_RATE = ( 8000.0 / RATE ) 
        print ("The change rate", Change_RATE)
        wf.setframerate(RATE*Change_RATE)
        wf.writeframes(signal)
        wf.close()
