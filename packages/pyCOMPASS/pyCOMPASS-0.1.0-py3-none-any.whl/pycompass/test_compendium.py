...
class TestCompendium(TestCase):
    ...
    def test_all(self):
        ...
        url = 'http://127.0.0.7:8000/graphql'
        conn = Connect(url, username='marco', password='mauriziomoroso')
        compendia = conn.get_compendia()

        #ds = compendia[0].get_data_sources(fields=['sourceName'])
        #plts = compendia[0].get_platforms()
        #pt = compendia[0].get_platform_types()
        #es = compendia[0].get_experiments(fields=['experimentAccessId', 'experimentName'])
        ...
        bf = BiologicalFeature.using(compendia[0]).get(filter={'name': 'BSU00010'})
