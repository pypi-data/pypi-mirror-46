...
def run_deer_script(path, deer_jar_file_path=None):
    if not deer_jar_file_path:
        resource_package = __name__
        resource_path = '/'.join(('deer-cli', 'deer-cli-2.0.1.jar'))
        deer_jar_file_path = pkg_resources.resource_stream(resource_package, resource_path).name
        #deer_jar_file_path = './reindeer/deer-cli/deer-cli-2.0.1.jar'
    p = Popen(['java', '-jar', deer_jar_file_path, path], stdout=PIPE, stderr=STDOUT)
...
class DeerFlow:
    ...
    @staticmethod
    def play(last_node, path="./temp/script.ttl", deer_jar_file_path=None):
        generate_script(last_node, path)
        run_deer_script(path, deer_jar_file_path)
