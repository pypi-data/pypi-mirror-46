...
def make_model(arch_params, patch_size):
    ...
    return RDSN(arch_params, patch_size)


class RDSN(ImageModel):
    ...
    def _UPN(self, input_layer):
        ...
        x = Conv2D(
            32, kernel_size=3, padding='same', kernel_initializer=self.initializer, name='UPN2'
        )(x)
    ...
    def _RDBs(self, input_layer):
        ...
        for d in range(1, self.D + 1):
            ...
            for c in range(1, self.C + 1):
                ...
                if c > 1:
                    # Reduce the depth by 1x1 conv
                    x = Conv2D(
                        self.G0,
                        kernel_size=1,
                        padding='same',
                        kernel_initializer=self.initializer,
                        name='Shrink_%d_%d' % (d, c),
                    )(x)
