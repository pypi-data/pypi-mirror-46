...
def fetch(globalDict, url):
    ...
    _ = ''
    soup = ''
    if len(url) < getConfig('BS4', 'bs4.min_url_length', int):
        return None
    try:
        ...
        _.set_text_origin(text)
        _.set_title(fn.preprocess(soup.title.string))
        ts = soup.findAll(attrs={"name": re.compile(r"publishdate", re.I)})
        if ts:
            _.set_timestamp(ts[0]['content'].encode('utf-8'))
...
def main():
    ...
    frontUrl = [x for x in frontUrl if fn.filter_links(x)]
    ...
    for i in range(getConfig('GA', 'ga.max_generation', int)):
        ...
        results = p.uimap(partial(fetch, globalDict), pop.crawlingQueue)
        # p.close()
