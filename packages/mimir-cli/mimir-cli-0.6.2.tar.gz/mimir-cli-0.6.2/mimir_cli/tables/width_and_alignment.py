...
def align_and_pad_cell(string, align, inner_dimensions, padding, space=" "):
    ...
    :param str space: Character to use as white space for resizing/padding
        (use single visible chars only).
...
def max_dimensions(
    ...
    :return: 4-item tuple of n-item lists.
        Inner column widths and row heights, outer column widths and row heights.
    :rtype: tuple
    """
    ...
    for j, row in enumerate(table_data):
        for i, cell in enumerate(row):
            ...
            inner_widths[i] = max(
                inner_widths[i], *[visible_width(l) for l in cell.splitlines()]
            )
