...
class OKEx():
    ...
    async def getPosition(self):
        self.instrument_id = self.get_latest_future_instrument_id(self.pair)
        r = await self.request(path = 'api/spot/v3/accounts')
        print_during_input(r)
        if r['holding']:
    ...
    async def parse(self, message):
        ...
        if 'event' in body:
            ...
            if 'spot/ticker' == body['table']:
                for item in body['data']:
                    self.parseTicker(item)
            if 'spot/position' == body['table']:
                self.parsePosition(body['data'][0])
                print_during_input("> " + message)
            if 'spot/order' == body['table']:
                for item in body['data']:
    ...
    async def placeMakerOrder(self, side, vol):
        if side in [2, 3]:
            ...
            price = self.bid
        r = await self.request('api/spot/v3/order', req={'instrument_id': self.instrument_id, 'type': side, 'price': price, 'size': vol, 'leverage': 20}, method='post')
        print_during_input(r)
        return r
    ...
    async def cancelOrder(self, oid):
        r = await self.request(
            'api/spot/v3/cancel_order/{0}/{1}'.format(self.instrument_id, oid),
            req={},
            method='post'
        )
        print_during_input(r)
