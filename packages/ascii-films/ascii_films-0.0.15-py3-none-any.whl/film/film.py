...
class Film:
    ...
    frame = property('', set_frame, '', "Retourne ou lit les frames à partir \
d'un seul fichier. Ces frames sont séparées par un séparateur, par défaut #")
    ...
    frames = property('', set_frames, '', "Retourne ou lit les frames à \
partir de fichiers dans un répertoire. Chaque frame est dans un fichier \
propre. Les fichiers sont lus dans le répertoire dans le sens alphnumérique \
croissant de leurs noms.")
    ...
    def projection(self, nb_frames=0, x=2, y=4, trace=0, delai=0.1):
        ...
        - nb_frames: nombre de frames à exécuter. 0 = infini
        ...
        while no_frame < nb_frames or nb_frames == 0:
            try:
                for f in self._frames:
                    ...
                    if self.affiche_no_frame:
                        self.__ccxy1(self.no_frame_xy)
                        print("frame " + str(no_frame))
