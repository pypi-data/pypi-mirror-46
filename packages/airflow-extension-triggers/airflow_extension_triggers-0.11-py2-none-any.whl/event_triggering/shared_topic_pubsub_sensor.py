...
class SharedTopicPubSubPullSensor(CustomPubSubPullSensor):
    ...
    def poke(self, context):
        ...
        if self._messages and self.ack_messages:
            logging.info('found acknowledged message')
            if self.ack_messages:
                logging.info('found acknowledged messages')
                for t in self._messages:
                    if t.get('ackId'):
                        logging.info('ackId: ' + str(t['ackId']))
                        for key in t.keys():
                            logging.info('printing key: ' + str(key))
                        if t.get('attributes'):
                            logging.info('found attributes')
                            if t['attributes']['trigger_msg']:
                                logging.info('found trigger msg: ' + str(t['attributes']['trigger_msg']))
        ...
        if len(ack_ids) > 0:
            logging.info('found trigger message sensor is waiting for')
...
class SharedTopicSubscriptionTaskSet:
    ...
    def delete_task(self):
        return PubSubSubscriptionDeleteOperator(
            ...
            project=self.config.project,
            suscription=self.config.subscription,
            dag=self.dag)
