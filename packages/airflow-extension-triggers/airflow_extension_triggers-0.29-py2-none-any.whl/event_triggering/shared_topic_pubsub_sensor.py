...
class SharedTopicPubSubPullSensor(PubSubPullSensor):
    """
        ...
        :param project: the GCP project where the DAG will run that waits for the trigger message
        :type project : string
    ...
    def poke(self, context):
        ...
        if not self._messages:
            ...
            self.log.info("checking if any of the messages are the one we're waiting for...")
...
class SharedTopicSubscriptionTaskSet:
    ...
    def __init__(self, dag, project, topic, subscription, subscription_project, trigger_msg, max_messages=1000, ack_messages=True):
        self.dag = dag
        self.project = project
        self.topic = topic
        self.subscription = subscription
