...
describe('buildrequest controller', function () {
  var $httpBackend, $rootScope, $scope, $stateParams, createController;
  var dataService = $scope = $httpBackend = $rootScope = null;
  var $timeout = createController = $stateParams = null;
  var goneto = null; // override "$state"
  ...
  var injected = function injected($injector) {
    $rootScope = $injector.get('$rootScope');
    $scope = $rootScope.$new();
    ...
    createController = () => $controller('buildrequestController', {
      $scope
    });
  };
  ...
  it('should query for builds again if first query returns 0', function () {
    ...
    $scope.$apply(() => $scope.buildrequest.claimed = true);
    $timeout.flush();
    expect($scope.builds.length).toBe(0); // simulate new builds from event stream
...
beforeEach(angular.mock.module('app'));
describe('buildrequest controller', function () {
  var $httpBackend, $rootScope, $scope, $stateParams, $timeout, modal;
  var dataService = $scope = $rootScope = null;
  var createController = $stateParams = modal = $httpBackend = $timeout = null;
  ...
  var injected = function injected($injector) {
    ...
    createController = () => $controller('forceDialogController', {
      $scope,
      builderid: 1,
...
describe('page with sidebar', function () {
  var queries, rootScope, scope, timeout;
  beforeEach(angular.mock.module("app"));
  var elmBody = scope = rootScope = queries = timeout = null;
  var elmContent = null;
  ...
  var elements = (start, end) => ({
    type: "elements",
    start,
    end
  });

  var assertDOM = function assertDOM(l) {
    var childs = [];
    $("div", elmContent).each((i, c) => childs.push(c));

    for (var _i = 0, _Array$from = Array.from(l); _i < _Array$from.length; _i++) {
      ...
      if (item.type === "elements") {
        for (var i = item.start, end = item.end, asc = item.start <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
          c = childs.shift();
  ...
  var printDOM = () => $("div", elmContent).each(function () {
  ...
  var scrollTo = function scrollTo(pos, verifyPos) {
    if (verifyPos == null) {
      verifyPos = pos;
    } // we scroll pos


    elmBody.scrollTop(pos); // make sure that worked

    expect(elmBody.scrollTop()).toBe(verifyPos); // as the scroll is automatic, we need to force the event

    elmBody.trigger("scroll");
    timeout.flush(); // make sure it did not changed
  ...
  beforeEach(inject(function ($rootScope, $compile, glMenuService, $timeout, $q, $document) {
    ...
    scope.items = {
      get(index, num) {
        ...
        $timeout(function () {
          ...
          for (var i = 0, end = num - 1, asc = 0 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
            ret.push({
              v: index + i
            });
          }
    ...
    scope.$digest(); // we need to append to body, so that the element is styled properly, and gets a height
...
describe('buildrequest summary controller', function () {
  var $q, $rootScope, $timeout, createController;
  var $scope = $rootScope = $q = $timeout = null;
  var goneto = createController = null;
  var dataService = null; // override "$state"
  ...
  var injected = function injected($injector) {
    ...
    var $controller = $injector.get('$controller');
    $q = $injector.get('$q');
    dataService = $injector.get('dataService'); // stub out the actual backend of mqservice

    createController = () => $controller('_buildrequestsummaryController', {
      $scope
    });
  };
...
describe('buildsticker controller', function () {
  var $compile, $rootScope, $timeout, results, scope;
  var dataService = scope = $compile = results = $timeout = $rootScope = null;
  ...
  it('directive should generate correct html', function () {
    ...
    var data = dataService.open();

    data.getBuilds(build.buildid).onNew = build => scope.build = build;

    var element = $compile("<buildsticker build='build'></buildsticker>")(scope);
    $timeout.flush();
    $rootScope.$digest();
    var sticker = element.children().eq(0);
    var row0 = sticker.children().eq(0);
    var row1 = sticker.children().eq(1);
    var resultSpan = row0.children().eq(0);
    var buildLink = row0.children().eq(1);
    var durationSpan = row1.children().eq(0);
    var startedSpan = row1.children().eq(1);
    var stateSpan = row1.children().eq(2); // the link of build should be correct

    expect(buildLink.attr('href')).toBe('#/builders/2/builds/1'); // pending state
    ...
    expect(stateSpan.text()).toBe('pending'); // success state
    ...
    expect(stateSpan.text()).toBe('finished'); // failed state
...
describe('buildsummary controller', function () {
  var $compile, $rootScope, $stateParams, baseurl, createController, results, scope;
  var dataService = scope = $rootScope = $compile = null;
  var $timeout = createController = $stateParams = results = baseurl = null;
  var goneto = null;
  ...
  it('should provide correct isStepDisplayed when condensed', function () {
    ...
    var element = $compile("<buildsummary buildid='buildid' condensed='condensed'></buildsummary>")(scope);
    scope.$apply();

    var _element$isolateScope = element.isolateScope(),
        buildsummary = _element$isolateScope.buildsummary;
  ...
  it('should provide correct isStepDisplayed when not condensed', function () {
    ...
    var _element$isolateScope2 = element.isolateScope(),
        buildsummary = _element$isolateScope2.buildsummary;
  ...
  it('should provide correct getBuildRequestIDFromURL', function () {
    var element = $compile("<buildsummary buildid='buildid'></buildsummary>")(scope);
    scope.$apply();

    var _element$isolateScope3 = element.isolateScope(),
        buildsummary = _element$isolateScope3.buildsummary;

    expect(buildsummary.getBuildRequestIDFromURL("".concat(baseurl, "#buildrequests/123"))).toBe(123);
  });
  it('should provide correct isBuildRequestURL', function () {
    var element = $compile("<buildsummary buildid='buildid'></buildsummary>")(scope);
    scope.$apply();

    var _element$isolateScope4 = element.isolateScope(),
        buildsummary = _element$isolateScope4.buildsummary;

    expect(buildsummary.isBuildRequestURL("".concat(baseurl, "#buildrequests/123"))).toBe(true);
    expect(buildsummary.isBuildRequestURL("http://otherdomain:5000/#buildrequests/123")).toBe(false);
    expect(buildsummary.isBuildRequestURL("".concat(baseurl, "#builds/123"))).toBe(false);
    expect(buildsummary.isBuildRequestURL("".concat(baseurl, "#buildrequests/bla"))).toBe(false);
  });
  it('should provide correct isBuildURL', function () {
    var element = $compile("<buildsummary buildid='buildid'></buildsummary>")(scope);
    scope.$apply();

    var _element$isolateScope5 = element.isolateScope(),
        buildsummary = _element$isolateScope5.buildsummary;

    expect(buildsummary.isBuildURL("".concat(baseurl, "#builders/123/builds/123"))).toBe(true);
    expect(buildsummary.isBuildURL("".concat(baseurl, "#builders/sdf/builds/123"))).toBe(false);
  });
});
beforeEach(angular.mock.module('app'));
describe('ansicode service', function () {
  var ansicodesService = null;

  var injected = $injector => ansicodesService = $injector.get('ansicodesService');

  beforeEach(inject(injected));

  var runTest = function runTest(string) {
    var ret = ansicodesService.parseAnsiSgr(string);
  ...
  it('should provide correct split_ansi_line', function () {
    var ret = ansicodesService.splitAnsiLine("\x1b[36mDEBUG [plugin]: \x1b[39mLoading plugin karma-jasmine.");
    expect(ret).toEqual([{
      class: 'ansi36',
      text: 'DEBUG [plugin]: '
    }, {
  ...
  it('should provide correct split_ansi_line for nested codes', function () {
    var ret = ansicodesService.splitAnsiLine("\x1b[1m\x1b[36mDEBUG [plugin]: \x1b[39mLoading plugin karma-jasmine.");
    expect(ret).toEqual([{
      class: 'ansi1 ansi36',
      text: 'DEBUG [plugin]: '
    }, {
  ...
  it('should provide correct split_ansi_line for reset codes', function () {
    ...
    var ret = ansicodesService.splitAnsiLine("\x1b[32m.\x1b[0m\x1b[31mF\x1b[0m\x1b[32m.\x1b[39m\x1b[32m.\x1b[0m");
    expect(ret).toEqual([{
      class: "ansi32",
      text: "."
    }, {
  ...
  it('should provide correct split_ansi_line for 256 colors', function () {
    var ret = ansicodesService.splitAnsiLine("\x1b[48;5;71mDEBUG \x1b[38;5;72m[plugin]: \x1b[39mLoading plugin karma-jasmine.");
    expect(ret).toEqual([{
      class: 'ansibg-71',
      text: 'DEBUG '
    }, {
  ...
  it('should provide correct split_ansi_line for joint codes', function () {
    var ret = ansicodesService.splitAnsiLine("\x1b[1;36mDEBUG [plugin]: \x1b[39mLoading plugin karma-jasmine.");
    expect(ret).toEqual([{
      class: 'ansi1 ansi36',
      text: 'DEBUG [plugin]: '
    }, {
  ...
  it('should provide correct split_ansi_line for unsupported modes', function () {
    var val = "\x1b[1A\x1b[2KPhantomJS 1.9.8 (Linux 0.0.0)";
    var ret = ansicodesService.splitAnsiLine(val);
    expect(ret).toEqual([{
      class: '',
  ...
  it('should provide correct ansi2html', function () {
    var ret = ansicodesService.ansi2html("\x1b[36mDEBUG [plugin]: \x1b[39mLoading plugin karma-jasmine.");
  ...
  it('should provide correct color cube generator', function () {
    var ret = ansicodesService.generateStyle();
  ...
  it('should inject generated style only once', function () {
    var before = document.getElementsByTagName("style").length;
    ansicodesService.injectStyle();
    var after1 = document.getElementsByTagName("style").length;
    ansicodesService.injectStyle();
    var after2 = document.getElementsByTagName("style").length;
...
describe('results service', function () {
  ...
  var injected = $injector => resultsService = $injector.get('resultsService');

  beforeEach(inject(injected));
  it('should provide correct results2class', function () {
    var _resultsService = resultsService,
        results = _resultsService.results;

    var results2class = r => resultsService.results2class({
      results: r
    });
  ...
  it('should provide correct results2Text', function () {
    var _resultsService2 = resultsService,
        results = _resultsService2.results;

    var results2text = r => resultsService.results2text({
      results: r
    });
...
describe('settingsService', function () {
  ...
  it('should merge groups when old group has values already set', inject(function (bbSettingsService) {
    ...
    var group_result = bbSettingsServiceProviderRef._mergeNewGroup(old_group, new_group);
  ...
  it('should merge groups when new group is undefined', inject(function (bbSettingsService) {
    ...
    var group_result = bbSettingsServiceProviderRef._mergeNewGroup(old_group, undefined);
  ...
  it('should merge groups when old group is undefined', inject(function (bbSettingsService) {
    ...
    var group_result = bbSettingsServiceProviderRef._mergeNewGroup(undefined, new_group);
  ...
  it('should not add a group without name', inject(function (bbSettingsService) {
    ...
    var exceptionRun = function exceptionRun() {
      var group_result;
  ...
  it('should generate correct settings', inject(function (bbSettingsService) {
    var groups = bbSettingsService.getSettingsGroups();
  ...
  it('should return correct setting', inject(function (bbSettingsService) {
    var userSetting1 = bbSettingsService.getSetting('User.checkbox1');
    var userSetting2 = bbSettingsService.getSetting('User.whatever');
    var userSetting3 = bbSettingsService.getSetting('UserAA.User_checkbox1');
  ...
  it('should save correct settings', inject(function (bbSettingsService) {
    var checkbox = bbSettingsService.getSetting('User.checkbox1');
    expect(checkbox.value).toBe(false);
    checkbox.value = true;
    bbSettingsService.save();
    var storageGroups = angular.fromJson(localStorage.getItem('settings'));
    var storageCheckbox = storageGroups['User'].items[0].value;
    expect(storageCheckbox).toBeTruthy();
  }));
  it('should be overriden by master.cfg', inject(function (bbSettingsService) {
    var to_override = bbSettingsService.getSetting('User.config_overriden');
