...
def run_device_action(topic, msg, db):
    ...
    if(topic_parts[2] == "device"):
        result = Database.select_all("SELECT * FROM MQTT_DEVICES WHERE TOPIC = :topic",
                    {'topic': topic}, db)
        ...
        for device_type in device_types:
            results = Database.select_all("SELECT * FROM " + device_type + " WHERE DEVICE_ID = :dev_id",
                        {'dev_id': result['ID']})
        ...
        # Run MQTT-Trigger
        '''Database.select_all("SELECT * FROM MQTT_TRIGGERS WHERE TOPIC = :topic", {'topic': message.topic})
        ...
        smart_speaker = Database.select_all("SELECT * FROM SMART_SPEAKER WHERE ID = :id",
                    {'id': topic_parts[3]}, db)
...
def publish(topic, msg):
    ...
    gateway = Item.load_from_db(Gateway, MQTT_BROKER, Database.get_database_con())
