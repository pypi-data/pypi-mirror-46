...
try:
    ...
    print ('DATABASE_WAIT_TIMEOUT not in settings.py, defaulting to 120.')
...
# must match or be less than wait_timeout in mysql 
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def get_new_connection(self, conn_params):
        ...
        conv = conn_params.pop('conv', None)
        ssl = conn_params.pop('ssl', None)
        if conv:
            conn_params['conv'] = HashableDict(conv)

        if ssl:
            conn_params['ssl'] = HashableDict(ssl)
