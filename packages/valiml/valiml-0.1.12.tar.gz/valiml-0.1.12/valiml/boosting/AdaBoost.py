...
class AdaBoost(BaseEstimator, ClassifierMixin):
    ...
    def _compute_loss(self, weighted_error, classifier_decision, X, y):
        if self.mode == 'exponential':
            if self.n_labels > 2:
                linear_combination = (self._y[:y.shape[0]] * classifier_decision[:y.shape[0]]).sum(axis=1)
                linear_combination /= -self.n_labels

                if np.exp(linear_combination).sum() / y.shape[0] > 1000000:
                    idx = linear_combination.argmax()
                    print(linear_combination[idx])
                    print(self._y[idx])
                    print(classifier_decision[idx])
                    decisions = []
                    for alpha, decision_stump in self.decision_stumps:
                        decision = decision_stump.decision_function(X[[idx]])
                        decisions.append(np.abs(decision).max())

                    idx = np.argmax(decisions)
                    decision_stump = self.decision_stumps[idx][1]
                    print(decision_stump.decision_function(X[[idx]]))
                    print(decision_stump.left_probabilities)
                    print(decision_stump.right_probabilities)
    ...
    def fit(self, X, Y, validation_split=None):
        ...
        while n_iter <= self.n_estimators:
            ...
            loss = self._compute_loss(weighted_error, classifier_decision, X[:used_samples], Y[:used_samples])
