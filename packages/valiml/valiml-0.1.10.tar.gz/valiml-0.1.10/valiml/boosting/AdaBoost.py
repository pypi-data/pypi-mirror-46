...
class AdaBoost(BaseEstimator, ClassifierMixin):
    ...
    def _compute_loss(self, weighted_error, classifier_decision, y):
        if self.mode == 'exponential':
            if self.n_labels > 2:
                linear_combination = (self._y * classifier_decision).sum(axis=1)
    ...
    def fit(self, X, Y, validation_split=None):
        ...
        while n_iter <= self.n_estimators:
            if self.valid_boost and n_iter <= self.valid_boost * self.n_estimators:
                ...
                missclassified = self._predict(classifier_decision[used_samples:t]) != Y[used_samples:t]
                n_misclassified = missclassified.sum()
                if n_misclassified:
                    p = (t - used_samples) / (2 * t * n_misclassified)
                    self.instance_importance[used_samples:t][missclassified] = p
                if t - used_samples - n_misclassified:
                    p = (t - used_samples) / (2 * t * (t - used_samples - n_misclassified))
                    self.instance_importance[used_samples:t][~missclassified] = p
