...
class AdaBoost(BaseEstimator, ClassifierMixin):
    def __init__(self, n_estimators=200, n_random_features=None, mode='exponential', type='discrete', verbose=False, learning_rate=1.0, valid_boost=True):
    ...
    def fit(self, X, Y, validation_split=None):
        ...
        while n_iter <= self.n_estimators:
            if self.valid_boost:
                valid_boost_factor = (np.log(n_iter + 1) / np.log(self.n_estimators + 1)) / 2

                train_idx, test_idx = train_test_split(np.arange(X.shape[0]), test_size=valid_boost_factor)

                decision_stump = self._get_weak_classifier(self.instance_importance[train_idx], X[train_idx], Y[train_idx])
                stump_prediction = decision_stump.predict(X)

                weighted_error = self.instance_importance[train_idx][stump_prediction[train_idx] != Y[train_idx]].sum()

                weighted_error_train_validation = self.instance_importance[test_idx][stump_prediction[test_idx] != Y[test_idx]].sum()

                weighted_error = valid_boost_factor * weighted_error_train_validation + (1 - valid_boost_factor) * weighted_error
            else:
