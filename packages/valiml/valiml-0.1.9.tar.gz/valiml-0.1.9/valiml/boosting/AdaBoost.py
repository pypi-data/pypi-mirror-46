...
class AdaBoost(BaseEstimator, ClassifierMixin):
    def __init__(self, n_estimators=200, n_random_features=None, mode='exponential', type='discrete',
                 verbose=False, learning_rate=1.0, entropy_reg=0.1, valid_boost=True):
    ...
    def _compute_loss(self, weighted_error, classifier_decision, y):
        if self.mode == 'exponential':
            if self.n_labels > 2:
                linear_combination = (self._y[:y.shape[0]] * classifier_decision).sum(axis=1)
                return np.exp((-1 / self.n_labels) * linear_combination).sum() / y.shape[0]
            if self.n_labels == 2:
    ...
    def fit(self, X, Y, validation_split=None):
        ...
        while n_iter <= self.n_estimators:
            if self.valid_boost:
                t = int(X.shape[0] / 2 + int((np.log(n_iter) / np.log(self.n_estimators)) * X.shape[0] / 2))
            ...
            loss = self._compute_loss(weighted_error, classifier_decision[:used_samples], Y[:used_samples])
