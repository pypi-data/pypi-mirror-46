...
class _pricegen ():
    ...
    def gen_custom(self,func,name,var=None,add_type="factor"):
        """
        ...
        if name in self.factor_list + self.noise_list:
            raise ValueError ("name already used in the factor/noise list")
        ...
        if add_type=="factor":
            if name not in self.factor_list:
                self.factor_list.append(name)
        elif add_type=="noise":
            if name not in self.noise_list:
    ...
    def gen_ln(self,mean=0,var=0.01,name=None,add_type="factor",seed=None):
        ...
        """
        if name is None:
            var_name = "ln_u=%s_v=%s" %(mean,var)
        if seed is not None:
            np.random.seed(seed)
        self.data[var_name] = np.exp(np.random.normal(mean,var,self.data.shape[0]))
        if add_type == "factor":
            if var_name not in self.factor_list:
                self.factor_list.append(var_name)
        elif add_type == "noise":
            if var_name not in self.noise_list:
        ...
        if len(self.factor_list)  == 0 and len(self.noise_list) == 0:
            ...
            for n in self.noise_list:
                ...
                if temp_m == "m":  #multiplicative
                    noise = self.data[n].fillna(1)
                    self.data["price"] = self.data["price_r"]*noise
                elif temp_m == "a": #additive
                    noise = self.data[n].fillna(0)
                    self.data["price"] = self.data["price_r"] + noise
                else:
                    raise ValueError ("unsupported method for factor %s" %(f))
