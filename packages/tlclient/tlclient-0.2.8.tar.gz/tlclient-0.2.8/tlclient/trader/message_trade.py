...
from tlclient.trader.constant import Direction, ExchangeID, OrderStatus, OrderType, PosiDirection
...
class PositionInfo:
    ...
    def __init__(self, d):
        self.exchange = ExchangeID.parse(d['ex'])
        self.ticker = d['ti']
        self.posi_direction = PosiDirection.parse(d['d'])
        self.position = d['pos']
        self.yd_position = d['yd_pos']
        self.position_cost = d['cost']
    ...
    def to_influx(self, dt):
        return {
            ...
            "tags": {
                "exchange": self.exchange,
                "ticker": self.ticker,
                "posi_direction": self.posi_direction,
            },
            "time": dt,
            "fields": {
...
class RspPosition:
    ...
    def __init__(self, d):
        self.tg_name = d['tg']
        self.pos_list = list() if d['pos'] is None else [PositionInfo(item_d) for item_d in d['pos']]
    ...
    def to_influx(self):
        dt = Timer.datetime()
        return [info.to_flux(dt) for info in self.pos_list]

    def to_dict(self):
        return {
            'tg_name': self.tg_name,
            'pos_list': [p.to_dict() for p in self.pos_list],
        }
...
class ActiveOrderInfo:
    ...
    def __init__(self, d):
        ...
        self.exchange = ExchangeID.parse(d['ex'])
        self.ticker = d['ti']
        self.order_status = OrderStatus.parse(d['os'])
        self.volume_traded = d['vt']
        self.order_id = d['oid']
        self.order_ref = d['ref']
...
class RspActiveOrders:
    ...
    def __init__(self, d):
        self.tg_name = d['tg']
        self.order_list = list() if d['ods'] is None else [ActiveOrderInfo(item_d) for item_d in d['ods']]
    ...
    def to_influx(self):
        dt = Timer.datetime()
        return [info.to_flux(dt) for info in self.order_list]
