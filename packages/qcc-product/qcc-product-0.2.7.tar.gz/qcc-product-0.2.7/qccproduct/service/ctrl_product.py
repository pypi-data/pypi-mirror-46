...
from qccwallet.utils.coinnum import CoinNum
import re
...
def simple_read(pid, min_stat=0):
    ...
    if isinstance(p['the_pledged'], ObjectId):
        pc = ctrl_contract.simple_read(p['the_pledged'])
        p['the_pledged'] = pc
        if 'min_pledge' in p:
            dec = pc['decimals']
            prec = pc['precision']
            p['min_pledge'] = CoinNum(p['min_pledge']).set(dec, prec).group()
...
def query_products(cid, page_no, page_size, *auth_args):
    ...
    for item in cursor:
        if isinstance(item['the_pledged'], ObjectId):
            pc = ctrl_contract.simple_read(item['the_pledged'])
            item['the_pledged'] = pc
            if 'min_pledge' in item:
                dec = pc['decimals']
                prec = pc['precision']
                item['min_pledge'] = CoinNum(item['min_pledge']).set(dec, prec).group()
