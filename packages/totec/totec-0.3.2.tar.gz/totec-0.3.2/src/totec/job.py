...
class SimpleJob:
    ...
    def to_dict(self):
        ...
        obj = {
            ...
            'type': self.job_type,
            'tag': self.tag,
            'transitions': [(status.name, t.isoformat()) for (status, t) in self.transitions]
        }
    ...
    def transition(self, status, data=None):
        ...
        if (self.status, status) in _ALLOWED_TRANSITIONS:
            log.info("Transitioning job %s from %s to %s", self.tag, self.status, status)
            self.status = status
            self.outputs = data
        else:
            raise ValueError('Cannot transition from {} to {}'.format(self.status, status))

    def __repr__(self):
        return "<{}(tag={}, status={})>".format(self.__class__.__name__, self.tag, self.status)
