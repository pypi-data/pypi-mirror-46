...
def _add_square_from_coordinates(x, y, w, h, label, page):
    ...
    annot.setInfo({'label': f'{label}', 'name': '', 'title': '', 'creationDate': '', 'modDate': '',
                   'subject': ''})
...
def annotate_pdf(adf, pdf_path, dest_pdf_path, debug=False):
    ...
    for index, row in adf.iterrows():
        ...
        if ('page' in adf.columns) and row['page']:
            if all(c in adf.columns for c in 'xywh'):
                _add_square_from_coordinates(row['x'], row['y'], row['w'], row['h'], row['label'], pdf[row['page'] - 1])
            ...
            for page in pdf:
                ...
                if all(c in adf.columns for c in 'xywh'):
                    _add_square_from_coordinates(row['x'], row['y'], row['w'], row['h'], row['label'], page)
