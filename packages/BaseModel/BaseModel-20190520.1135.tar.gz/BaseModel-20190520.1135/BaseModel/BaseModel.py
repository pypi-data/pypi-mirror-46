...
class BaseModel:
    ...
    def getDistinctK(cls, k, session, **kw):
        if kw.get("columns"):
            ...
            return session.query(*cls_colum).distinct(getattr(cls, k)).order_by(kw.get("order", "id")).all()
        else:
            return session.query(distinct(getattr(cls, k))).order_by(kw.get("order", "id")).all()
