...
class PyStorage(object):
    ...
    def load_from_disk(self, package_path=None):
        ...
        if package:
            for name in dir(package):
                if not name.startswith("_") and name not in processed:
                    value_from_a_package = getattr(package, name)
                    if not inspect.ismodule(value_from_a_package):
                        setattr(self, name, Reference(type(value_from_a_package)))
                        setattr(self, name, value_from_a_package)
    ...
    def _dump_package_init(self, labels, ref_watches):
    ...
    def __setattr__(self, attribute_name, value):
        # basic case - all protected attributes
        if attribute_name.startswith("_"):
            return object.__setattr__(self, attribute_name, value)

        # renew.Label or renew.Reference
        # "static" (from class definition)
        ...
        # adding new fields of type renew.Reference
    ...
    def _split_references(self):
        ...
        references = {}
        for name, proxy in self._collect_serializables():
            element = self._get_instance_obj(name)
            if isinstance(proxy, Reference):
                references[id(element)] = name
            else:
                labels[name] = element
