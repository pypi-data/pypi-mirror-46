...
class Signal_2D:
    ...
    def __init__(self, values=None, time=None, verbose=False):
        if not values is None:
            ...
            self.values = [self.x, self.y]
            if verbose:
                print('[ S2D ]  Signal created {}'.format(len(self.x), len(self.y)))
...
class Interpolation_Cubic:
    ...
    def __init__(self, signal=None, total_number=10, verbose=False):
        if signal is None:
            ...
            x = self.signal.x
            y = self.signal.y
            print('x inside: {}| {}'.format(len(x), x))
            print('y inside: {}| {}'.format(len(y), y))
...
class Intersection:
    ...
    def find_intersect_points(self, verbose=False):
        ...
        if len(points) > 1:
            for point in points:
                x, y = point.xy
                cordinates[0].append(x)
                cordinates[1].append(y)
        else:
            if verbose:
                print('collecting found: {}'.format(len(points)))
            for point in res:
                x, y = point.xy
                cordinates[0].append(x)
                cordinates[1].append(y)
