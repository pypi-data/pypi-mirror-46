...
def test_plot_2d_list():
    print('| 2D List  |', end='-->')
    signal_from_list = Signal_2D(values=samples.dataset_list, verbose=True)
...
def test_plot_2d_tuple():
    print('| 2D Tuple |', end='-->')
    signal_from_tuple = Signal_2D(values=samples.dataset_list, verbose=True)
...
def test_plot_2d_npArray(verbose=False):
    print('| 2D Array |', end=' ---->')
    signal_1_from_ndarray = Signal_2D(values=samples.dataset_npArray, verbose=True)
