...
from microlab.data.intersection import Intersection
from microlab.vision.processing import show_mask, create_representation, export_mask
from microlab.data import plots
...
class Sinusoidal:
    ...
    def __init__(self, user_id, record_id, joint, signal_1=Signal_2D(), signal_2=Signal_2D(), intersections=Intersection(), reports_directory='', images_directory=''):
    ...
    def tune_s1(self, accuracy, target_value, verbose=False):
        ...
        # Before first index
        for idx in range(first_index, 0, -1):
        ...
        if len(s1_valid) == 0:
            ...
            for idx in range(first_index, second_index, 1):
                ...
                if offset_value - s1_value < accuracy and offset_value - s1_value > 0 and s1_time < second[0][0] - 5:
                    # print(s1_time, s1_value)
                    s1_valid.append([idx, s1_time, s1_value])
    ...
    def tune_s2(self, accuracy, target_value, verbose=False):
        ...
        if len(s2_valid) == 0:
            ...
            for idx in range(first_index, second_index, 1):
                ...
                if offset_value - s2_value < accuracy and offset_value - s2_value > 0 and s2_time < second[0][0] - 5:
                    s2_valid.append([idx, s2_time, s2_value])
            if verbose:
    ...
    def collect_data_between_closest(self, verbose=False):
        """
            collect the io between final intersected points
    ...
    def create_s1_representation(self, verbose=False):
        ...
        for y in self.s1_s_2_3.y:
            ...
            a = len(s1_time)
            x = s1_max_idx - a
            if y < point1_y and x >= point1_x or collect:
    ...
    # export
    def export_plot(self, verbose=False):
        if verbose:
            print('[ results ]  saving to {}'.format(self.reportFile))
        plt = plots.plt
        create_folder(path=self.reports_directory)

        reportFile = os.path.join(self.reports_directory, '{}.jpg'.format(self.joint))
        plt.savefig(fname=reportFile, dpi=100, facecolor='w', edgecolor='w', orientation='portrait', papertype=None, format=None, transparent=False, bbox_inches=None, pad_inches=0.1, frameon=None, metadata=None)

        # close the plt to release memory
        plt.close('all')

    def export_masks(self, verbose=False):
        create_folder(path=self.images_directory)
