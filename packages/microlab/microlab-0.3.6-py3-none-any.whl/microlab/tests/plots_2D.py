...
def test_plot_2d_list():
    print('2D List  ->  2D Plot', end='.....')
    signal_1_from_list = Signal_2D(values=Data.dataset_list, verbose=True)

    if is_2d_signal(object=signal_1_from_list):
        print('[OK]')
        p1 = Plot_2D(title='Plot 2D List', signal=signal_1_from_list, marker='.')
    else:
        print('[!!]')

def test_plot_2d_tuple():
    print('2D Tuple ->  2D Plot', end='.....')
    signal_1_from_list = Signal_2D(values=Data.dataset_list, verbose=True)
    if is_2d_signal(object=signal_1_from_list):
        print('[OK]')
        p1 = Plot_2D(title='Plot 2D Tuple', signal=signal_1_from_list, marker='.')
    else:
        print('[!!]')


def test_plot_2d_npArray(verbose=False):
    print('2D Array ->  2D Plot', end='.....')
    signal_1_from_list = Signal_2D(values=Data.dataset_npArray, verbose=True)
    if is_2d_signal(object=signal_1_from_list):
        print('[OK]')
        p1 = Plot_2D(title='Plot 2D Numpy Array', signal=signal_1_from_list, marker='.')
    else:
        print('[!!]')
