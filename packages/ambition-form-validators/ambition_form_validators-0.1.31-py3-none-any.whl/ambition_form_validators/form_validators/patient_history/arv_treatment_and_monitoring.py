...
class ArvTreatmentAndMonitoringFormValidatorMixin(FormValidator):
    ...
    def validate_arv_treatment_and_monitoring(self):
        ...
        self.applicable_if(
            NO, field="current_arv_is_defaulted", field_applicable="current_arv_is_adherent"
        )

        self.applicable_if(YES, field="taking_arv",
                           field_applicable="current_arv_is_adherent")

        self.required_if(NO, field="current_arv_is_adherent",
                         field_required="current_arv_tablets_missed")

        self.applicable_if(YES, field="taking_arv",
                           field_applicable="current_arv_decision")
    ...
    def validate_initial_arv_date(self):
        ...
        if self.cleaned_data.get("taking_arv") == YES:
            try:
                days = (self.cleaned_data.get("initial_arv_date") -
                        self.cleaned_data.get("report_datetime").date()).days
            except TypeError:
                pass
            else:
                if days > 0:
    ...
    def validate_current_arv_date(self):
        if self.cleaned_data.get("has_switched_regimen") == YES:
            try:
                days = (self.cleaned_data.get("current_arv_date") -
                        self.cleaned_data.get("initial_arv_date")).days
            except TypeError:
                pass
            else:
                if days == 0:
    ...
    def validate_current_arv_defaulted_date(self):
        if self.cleaned_data.get("current_arv_is_defaulted") == YES:
            try:
                days = (self.cleaned_data.get("current_arv_defaulted_date") -
                        self.cleaned_data.get("initial_arv_date")).days
            except TypeError:
                pass
            else:
                if days == 0:
                    ...
                    raise forms.ValidationError({
                        ...
                        "Date cannot be before the initial ARV date."})
        self.not_applicable(
            None, field="current_arv_defaulted_date",
