...
class NotebookTab(ttk.Frame):
    def __init__(self, master, title, file):
        ...
        if file:
            ...
            self.text_area.insert("1.0", "print('hello world!')")
...
class Editor(tk.Frame):
    ...
    def load_file(self):
        file_path = filedialog.askopenfilename(initialdir="", title="Load file",
                                               filetypes=(("python files", "*.py"), ("all files", "*.*")))
...
class Repl(tk.Frame):
    ...
    def _key_event(self, event):
        if event.keysym == "Left" and self.repl_text_field.compare(self.repl_text_field.index(tk.INSERT), '==',
                                                                   self.repl_stop):
            return "break"
        if event.keysym == "BackSpace" and self.repl_text_field.compare(self.repl_text_field.index(tk.INSERT), '==',
                                                                        self.repl_stop):
            return "break"
        if event.keysym == "Up":
            return "break"
