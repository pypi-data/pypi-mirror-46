...
class GraphAccessor:
    ...
    def create_or_update_graph_node(self, node, node_id):
        if node is None:
        ...
        remote_node = self.find_node_by_node_id(node_id)
    ...
    def find_node_by_node_id(self, node_id):
        try:
            query = 'MATCH (n) where n.node_id=%d RETURN n LIMIT 1' % node_id
