...
class Neo4jImporter:
    ...
    def import_all_graph_data(self, graph_data):
        self.graph_accessor.create_index(label="entity", property_name="node_id")
        node_info_dict = graph_data.graph.nodes._nodes
        ...
        print("all entity import")
        relations = graph_data.get_relations()
        for r in relations:
            start_node_id, r_name, end_node_id = r
            start_node = self.graph_accessor.find_node_by_node_id(start_node_id)
            end_node = self.graph_accessor.find_node_by_node_id(end_node_id)
            if start_node is not None and end_node is not None:
                try:
                    relationship = Relationship(start_node, r_name, end_node)
                    self.graph_accessor.graph.merge(relationship)
    ...
    def import_one_entity(self, node_id, property_dict, labels):
        builder = NodeBuilder()
        builder.add_property(**property_dict). \
            add_one_property(property_name="node_id", property_value=node_id).add_labels(*labels)
        node = builder.build()
        try:
            node = self.graph_accessor.create_or_update_graph_node(node, node_id)
