...
class TFIDFModel(DocumentSimModel):
    ...
    def doc_id_2_pos(self, doc_id):
        if doc_id not in self.doc_id_2_doc_index_map:
            return None
    ...
    def similarity_by_doc_id(self, doc_id):
        if doc_id not in self.doc_id_2_doc_index_map:
            return None
        pos = self.doc_id_2_pos(doc_id)
        return self.similarity_tfidf_index.similarity_by_id(pos)
