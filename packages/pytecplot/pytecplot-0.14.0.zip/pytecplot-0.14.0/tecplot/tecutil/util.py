...
XYPosition.__new__.__defaults__ = (None, None, None)

XYZPosition = namedtuple('XYZPosition', ['x', 'y', 'z'])
XYZPosition.__new__.__defaults__ = (None, None, None)

RectPosition = namedtuple('RectPosition', ['x1', 'y1', 'x2', 'y2'])

RectSize = namedtuple('RectSize', ['width', 'height'])
RectSize.__new__.__defaults__ = (None, None)

IJKIndex = namedtuple('IJKIndex', ['i', 'j', 'k'])
IJKIndex.__new__.__defaults__ = (None, None, None)

Limits = namedtuple('Limits', ['min', 'max'])
Limits.__new__.__defaults__ = (None, None)
...
_VarInfo = namedtuple('_VarInfo', ('types', 'values', 'names'))
...
def warn_api_moved(old, new):
    if __debug__:
        msg = textwrap.dedent('''\
        PyTecplot API Change:
            {} has been moved to {}
            Please update your code as the legacy interface will
