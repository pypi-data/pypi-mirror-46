...
from .. import legend, session, text, version
...
from . import blanking, effects, labels, rgb_coloring, scatter, slice, view
...
class FieldPlot(Plot):
    ...
    def slice(self, index):
        msg = 'Access to 3D slice groups from 2D plots has been removed.' \
            + ' Use frame.plot(PlotType.Cartesian3D).slice() to access' \
            + ' slice groups.'
        raise TecplotInterfaceChangeError(msg)

    def isosurface(self, index):
        msg = 'Access to 3D isosurface groups from 2D plots has been removed.' \
    ...
    def rgb_coloring(self):
        ...
        Example usage::
            ...
            >>> plot.rgb_coloring.red_variable = 3
            >>> plot.rgb_coloring.green_variable = 4
            >>> plot.rgb_coloring.blue_variable = 5
            >>> plot.show_contour = True
    ...
    def fieldmap(self, key):
        ...
        Parameters:
            key (`Zone <data_access>` or `int`): The
                ...
                assocated frame of this plot. A negative index is interpreted
...
class Cartesian3DFieldPlot(FieldPlot):
    ...
    def line_lift_fraction(self):
        ...
        """
        style = session.Style(sv.GLOBALTHREED, **self._style_attrs)
...
class LinePlot(Plot):
    ...
    def linemaps(self, pattern=None):
        ...
        Returns:
            Generator of `XYLinemaps <XYLinemap>` or `PolarLinemaps
            <PolarLinemap>` depending on the plot type. All linemaps if
    ...
    def _linemap_unique_id(self, index):
        index = self.num_linemaps + index if index < 0 else index
        with self.frame.activated():
    ...
    def base_font(self):
        ...
        """
        return text.BaseFont(sv.GLOBALLINEPLOT, **self._style_attrs)
