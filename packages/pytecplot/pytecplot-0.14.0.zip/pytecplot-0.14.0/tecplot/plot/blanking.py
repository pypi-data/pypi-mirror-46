...
class IJKBlanking(session.Style):
    ...
    def min_percent(self):
        ...
        """
        return tecutil.IJKIndex(self._get_style(float, sv.IMINFRACT),
                                self._get_style(float, sv.JMINFRACT),
                                self._get_style(float, sv.KMINFRACT))

    @min_percent.setter
    def min_percent(self, values):
        svargs = (sv.IMINFRACT, sv.JMINFRACT, sv.KMINFRACT)
        for svarg, val in zip(svargs, [float(v) for v in values]):
    ...
    def max_percent(self):
        ...
        """
        return tecutil.IJKIndex(self._get_style(float, sv.IMAXFRACT),
                                self._get_style(float, sv.JMAXFRACT),
                                self._get_style(float, sv.KMAXFRACT))

    @max_percent.setter
    def max_percent(self, values):
        svargs = (sv.IMAXFRACT, sv.JMAXFRACT, sv.KMAXFRACT)
        for svarg, val in zip(svargs, [float(v) for v in values]):
            self._set_style(val, svarg)
'''
