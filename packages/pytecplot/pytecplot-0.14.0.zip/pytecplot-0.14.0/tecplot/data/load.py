...
def load_telemac(filenames=None,
    ...
    Parameters:
        filenames (`str` or `list` of `strings <str>`, optional): Telemac data
            file(s) to be read. (See note below conerning absolute and relative
            paths.) Not required if a boundary file is being appended to an
            existing data set.
        boundary_filename (`str`, optional): Boundary file. If loaded with multiple
            Telemac files, will be applied to the first Telemac file. If loaded with
            no Telemac files, must be appended to an existing data set
            (read_data_option must be `ReadDataOption.Append`, the default).
        ...
        initial_plot_type (`PlotType`, optional): Forces a specific type of
            ...
            `PlotType.Automatic`, `Cartesian3D`, `Cartesian2D`,
            `XYLine`, `PlotType.Sketch`, `PolarLine`. (default: `PlotType.Automatic`)
...
def load_fluent(case_filenames=None,
    ...
    Parameters:
        case_filenames (`str` or `list` of `strings <str>`, optional):
            ...
            extension *.gz* are supported. (See note below conerning absolute
            and relative paths.)
