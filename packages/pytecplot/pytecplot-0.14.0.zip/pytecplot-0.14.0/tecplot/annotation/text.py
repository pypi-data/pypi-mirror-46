...
class TextBox(object):
    ...
    @property
    def position(self):
        """(**deprecated**) Position of the four corners of the `TextBox`.

        .. deprecated:: 0.13
            Use `TextBox.corner_locations` instead.
        """
        tecutil.warn_api_moved('TextBox.position', 'TextBox.corner_locations')
...
class Text(object):
    ...
    @property
    def text_box(self):
        """(**deprecated**) The frame and area around this `Text` object.

        .. deprecated:: 0.13
            Use `Text.box` instead.
        """
        tecutil.warn_api_moved('Text.text_box', 'Text.box')
        return self.box
    ...
    def position(self):
        ...
        with self.frame.activated():
            ...
            if self.position_coordinate_system == constant.CoordSys.Grid3D:
                return tecutil.XYZPosition(*pos)
            else:
                return tecutil.XYPosition(*pos[:2])

    @position.setter
    @tecutil.lock()
    def position(self, values):
        pos = tecutil.XYZPosition(*[float(x) for x in values])
        with self.frame.activated():
    ...
    def anchor_position(self):
        """(**deprecated**) Anchor position of the `Text`.

        .. deprecated:: 0.13
            Use `Text.position` instead.
        """
        tecutil.warn_api_moved('Text.anchor_position', 'Text.position')
    ...
    def zone_or_map(self):
        """(**deprecated**) Index of the attached fieldmap or linemap.

        .. deprecated:: 0.13
            Use `Text.attached_map_index` instead.
        """
        tecutil.warn_api_moved('Text.zone_or_map',
                               'Text.attached_map_index')
        return self.attached_map_index

    @zone_or_map.setter
    def zone_or_map(self, value):
        tecutil.warn_api_moved('Text.zone_or_map',
                               'Text.attached_map_index')
        self.attached_map_index = value
