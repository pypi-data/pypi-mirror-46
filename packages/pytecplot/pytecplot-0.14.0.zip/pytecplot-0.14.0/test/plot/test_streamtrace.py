...
class TestStreamtraces(PropertyTest):
    ...
    def test_deprecated_termination_line_is_active(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            # is_active -> active
            self.streamtraces.termination_line.active = True
            self.assertTrue(self.streamtraces.termination_line.is_active)
            if __debug__:
                self.assertEqual(w[-1].category, TecplotDeprecationWarning)
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            # is_active -> active
            self.streamtraces.termination_line.active = True
            self.streamtraces.termination_line.is_active = False
            self.assertFalse(self.streamtraces.termination_line.active)
            if __debug__:
                self.assertEqual(w[-1].category, TecplotDeprecationWarning)
