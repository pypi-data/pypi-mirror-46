...
class TestContourLegend(TestTabularLegend, unittest.TestCase):
    ...
    def test_show_cutoff_levels(self):
        ...
        if tp.version_info >= (1, 0):
            raise Exception('''
                There are deprecation warnings that should be converted to
                errors now.''')

        with warnings.catch_warnings(record=True) as w:
            # include_cutoff_levels -> show_cutoff_levels
            self.assertIsInstance(self.legend.include_cutoff_levels,
                                  type(self.legend.show_cutoff_levels))
            if __debug__:
                self.assertEqual(w[-1].category, TecplotDeprecationWarning)

        with warnings.catch_warnings(record=True) as w:
            # include_cutoff_levels -> show_cutoff_levels
            self.legend.show_cutoff_levels = True
            self.legend.include_cutoff_levels = False
            self.assertFalse(self.legend.show_cutoff_levels)
            if __debug__:
                self.assertEqual(w[-1].category, TecplotDeprecationWarning)
