...
def create(args):
    ...
    if args.type == "ELB":
        ...
        if True:  # lambda target
            target_group = ensure_target_group(args.target_group.format(elb_name=args.elb_name), TargetType="lambda")
        else:
        ...
        if args.path_pattern:
            ...
            clients.elbv2.create_rule(ListenerArn=listener["ListenerArn"],
                                      ...
                                      Priority=len(rules))
    lambda_arn = ARN(service="lambda", resource="function:" + args.lambda_name)
    clients.elbv2.register_targets(TargetGroupArn=target_group["TargetGroupArn"], Targets=[dict(Id=str(lambda_arn))])
    #replace(args)
