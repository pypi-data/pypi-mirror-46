...
async def run_command(command, timeout=None):
    ...
    python3 script/retry.py -t 10 "echo abcd ; false" # Immediate fail
    python3 script/retry.py -r 1 "sleep 70" # Fail after default timeout = 60
    python3 script/retry.py -s -r 1 -t 10 "sleep 15" # Should "skip timeout and therefore succeed after 15s"
    python3 script/retry.py -t 10 "echo abcd ; true" # Immediate success
    python3 script/retry.py -p -o abcd -t 10 " sleep 5; echo abc ;sleep 20; false" # Should Complete with error on second try
    python3 script/retry.py -o abcd -t 10 " sleep 5; echo abc ;sleep 20; false" # Should fail after retrymax (no progressive)
    python3 script/retry.py -o abc -t 10 " sleep 5; echo abc ; false"           # Should retry max but never with ok
    python3 script/retry.py -o abc -t 10 ">&2 echo abc ; false"                 # stderr, should retry max
    python3 script/retry.py -o nomatch -t 10 ">&2 echo abc ; false"             # stderr, should fail immediately
    python3 script/retry.py -o abc -t 10 "echo abc ; true"                      # stderr, should succeed immediately
    python3 script/retry.py -o nomatch -t 10 ">&2 echo abc ; sleep 2; echo def; true"  # Output should be: abc \ndef
    python3 script/retry.py -t 10 ">&2 echo abc ; sleep 10; echo def; sleep 10; true" # Staggered output
    """
