...
def parse(handle, pdbid, verbose=False):
    ...
    if len(dataBuffer) != header.symmetryBytes + header.mapSize:
        ...
        assert header.symmetryBytes == 0 | len(
            dataBuffer) != header.mapSize, "Error: File contains suspicious symmetry records" + pdbid
        assert header.mapSize == 0 | len(dataBuffer) != header.symmetryBytes, "Error: File contains no map data" + pdbid
        assert len(dataBuffer) > header.symmetryBytes + header.mapSize, "Error: contains incomplete data" + pdbid
        assert len(dataBuffer) < header.symmetryBytes + header.mapSize, "Error: File contains larger than expected data" + pdbid
...
class DensityHeader(object):
    ...
    def fromFileHeader(cls, fileHeader):
        ...
        if len(fileHeader) < 224:
            return None
        # Header
        headerFormat = endian + 10 * 'i' + 6 * 'f' + 3 * 'i' + 3 * 'f' + 3 * 'i' + 27 * 'f' + 4 * 'c' + 'ifi'
        headerTuple = struct.unpack(headerFormat, fileHeader[:224])
        # Sanity check for mode and axis correspondance
        if headerTuple[3] < 0 or headerTuple[3] >6 or headerTuple[16] not in [1,2,3] or headerTuple[17] not in [1,2,3] or headerTuple[18] not in [1,2,3]:
            return None
