from schemas import validator, TransformerSchema
    ...
    def get_transform_query(self, source):
        ...
        if self.options.get('time', None) and 'step' in self.options['time']:
            ...
            cte = f"""
                ...
                (
                    SELECT * FROM UNNEST(
                        (SELECT GENERATE_TIMESTAMP_ARRAY('{start}', '{finish}', INTERVAL {step}))
                    ) AS __serie
                )
                """
            ...
            join = f"""
                INNER JOIN __gs
                    ON q0.updated_at >= __gs.__serie
                        AND q0.updated_at < TIMESTAMP_ADD(__gs.__serie, INTERVAL {step})
                """
