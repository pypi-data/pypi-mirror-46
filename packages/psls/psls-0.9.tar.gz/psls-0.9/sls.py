...
def gen_up(starID, numax,dt,T, mag , delta_nu =  -1. , mass = -1. , seed = None, pn_ref = 7.7, wn_ref= 0., mag_ref = 6.,verbose = False, teff = 4800., DPI = -1.,  q = 0. , GST = 0 , incl = -1 , rot_f = 0., path= '', granulation = True):
    '''
...
def ExtracSystematicDataMagRange(filename,Mag):
    ...
    for d in Data:
        if( (d['Mag']+magcorr-0.25<=Mag) and (Mag<d['Mag']+magcorr+0.25) ):
            ...
            i += 1
    if(i==0):
        print ('No systematic error parameters for magnitude = %f' % Mag)
    return DataOut
