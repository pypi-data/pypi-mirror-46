...
ua2Km = 149.59e6  # km
NSR_mag = np.array([8.1,8.5,9,9.5,10,10.5,11,11.5,12,12.5,12.9])# V magnitude
NSR_values_24 = np.array([10.6,12.9,16.4,20.8,26.7,34.5,44.8,59.2,79.1,106.8,138.5])
...
if(len(sys.argv)<2):
    ...
    sys.exit(2)
try:
    opts,args = getopt.getopt(sys.argv[1:],"hvPVo:f",["pdf"])
...
if (UP):
    ...
    time,ts,f,ps,mps_nf,opar,_ =  sls.gen_up(StarID, numax,Sampling,Duration, StarMag,delta_nu =  delta_nu, mass = -1. , seed =seeds[0] , pn_ref = 0.,  wn_ref= 0., mag_ref = 6.,verbose = Verbose, teff = StarTeff, DPI = DPI,  q = q , GST = 1 , incl = Star['Inclination'] , rot_f = Star['CoreRotationFreq'] , path = OutDir  , granulation = (Granulation['Enable'] == 1) )
...
if RandomNoise['Enable']:
    if (RandomNoise['Type'].lower() == 'user'):
        ...
        if( (StarMag<NSR_mag[0]-0.25) or (StarMag>NSR_mag[-1]+0.25) ):
...
if (Systematics['Enable']):
    DataSystematic = sls.ExtracSystematicDataMagRange(Systematics['Table'],StarMag) 
...
full_ts_SC = np.zeros((nt,NGroup)) # systematic error only
...
for i in range(NGroup):
    ...
    for j in range(NCamera): 
        ...
        if(W>0.):
            # adding random noise
            cumul += np.random.normal(0.,W*1e-6/math.sqrt(Sampling),size=nt)/float(NCamera) 
        if(DataSystematic is not None):
            # adding systematic errors
            tmp = (sls.SimSystematicError(Sampling,nt,DataSystematic,seed=seeds[i*NCamera+1+j])-1.)/float(NCamera)
            cumul += tmp 
            tsSC *= (1. + tmp)
    ts *= (1. + cumul)        
    if(Transit['Enable']):
    ...
    full_ts[:,i,1] = ts
    full_ts[:,i,2] = i+1
    full_ts_SC[:,i] = (tsSC/np.mean(tsSC)-1.)*1e6
...
single_ts_SC = np.sum(full_ts_SC,axis=1)/float(NGroup)
