...
def universal_pattern_modes(df, delta_nu, numax, Ampli, norders = None, verbose = False , \
                             ...
                             gamma = -1. ,  rot_f = 0. , beta = 0. , alpha = -1., h_threshold  = 1e-2 , \
                             width=-1. , epsg = 0., V1 = -1. ):
    '''
            ...
            if ( (l ==1) & (DPI >0.)  ):
                ...
                for nu in num: # Loop over the mixed-modes
                    ...
                    if( h>h_threshold*Ampli ):                    
                        ...
                        d_split = 0.5*zeta[i]*math.fabs(rot_f) # splitting in muHz
                        if( (rot_f > 0.)):
                ...
                '''    
                d_split = math.fabs(rot_f) # splitting in muHz
                if( (rot_f > 0.) & (l>0) ): # &  (rot_f > gamma)):
                    pr = power_ratio(l,math.fabs(beta)) # power ratios of the mutliplets
                    for m in range(-l, l+1) : 
                        F=np.append(F,nu + m*d_split)
                        L=np.append(L,l)
                        ...
                        G=np.append(G,gamma)
                        N=np.append(N,n)
                        H=np.append(H,V[l]*h*pr[m+l]*gaussenvelop(nu,Ampli,numax,width))
                else:
                '''
                F=np.append(F,nu)
                L=np.append(L,l)
...
def universal_pattern_new(delta_nu, numax, Ampli, f, nyquist, norders = None, verbose = False, teff=4800., \
                      DPI = -1., q = 0.,  eps = -10., d01 = 10., d02=-10.  ,  gamma = -1. , rot_f = 0. , beta = 0. , alpha = -1., h_threshold = 1e-3 , width = -1. , epsg = 0.):
    ...
    (F,L,M,N,H,G)  = universal_pattern_modes(df, delta_nu, numax, Ampli, norders = norders ,  verbose = verbose , teff=teff \
                                               ...
                                               alpha = alpha ,  rot_f = rot_f , beta = beta , h_threshold =  h_threshold , width = width , \
                                               epsg = epsg )
...
def universal_pattern(delta_nu, numax, Ampli, f, nyquist, norders = None, verbose = False, teff=4800., \
                      DPI = -1., q = 0.,  eps = -10., d01 = 10., d02=-10.  ,  gamma = -1. , rot_f = 0. , \
