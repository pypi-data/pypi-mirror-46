...
def cp(source, target, copy_attrs=True, recursive=False,
    ...
    for f in file_list:
        try:
            ...
            raise FSError(
                origin='cp',
                message='error occurred while copying %s' % f).with_traceback(sys.exc_info()[2])
    ...
    if os.path.isdir(path):
        ...
        try:
            ...
            raise FSError(origin='mkdir',
                          message='can\'t create %s' % path).with_traceback(sys.exc_info()[2])
...
def rm(path, recursive=False, glob=True):
    ...
    for f in file_list:
        try:
            ...
            raise FSError(
                origin='rm',
                message='error occurred while removing %s' % f).with_traceback(sys.exc_info()[2])
