...
def makevid(image,sound,tmp_path):
    ...
    cmd="ffmpeg -y -r 12 -loop 1 -i {} -i {} -shortest -flags global_header -pix_fmt yuv420p  -c copy {}".format(image,sound,output)
    os.popen(cmd).read()
    os.remove(image)
    os.remove(sound)
    return output

def mergvid(arr_vid,tmp_path):
    output = os.path.join(tmp_path, uuid.uuid4().hex + ".avi")
...
def make_sound_null(time,tmp_path):
    output = os.path.join(tmp_path, uuid.uuid4().hex + ".wav")
    cmd="ffmpeg -y  -f lavfi -i anullsrc -t {} {}".format(time,output)
    os.popen(cmd).read()
    return output
