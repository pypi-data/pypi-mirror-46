...
class MSNSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        arr_text = response.xpath('//div[@id="maincontent"]//p[not (descendant::img)]//text()').extract()
        ...
        while(i<len(arr_image)):
            ...
            if arr_text[i]==None or arr_text[i] =="":
                ...
                img_source = draw.content(arr_image[i], arr_text[i],font_file,tmp_path)
                textTran=";;".join(textwrap.wrap(arr_text[i].replace("\"","\\\""),190))
...
def run(parent_path,bg_sound_path,start_urls,output):
    ...
    process.crawl(MSNSpider,bg_sound_path,parent_path,start_urls,output)
    process.start()