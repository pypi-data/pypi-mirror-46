...
#         try:
#             if stack.img_prep["crop"]:
#                 raise ValueError("Stack is cropped: sensitivity mask can "
#                                  "only be determined for uncropped images")
#         except:
#             pass
...
#         value), an array containing the corresponding camera optical
#         densities (tau, 2. return value) and the corresponding cell CDs
#         (3. return val)
#
...
#         the :func:`fit_calib_polynomial` is called ultimately using the
#         default position settings (i.e. center of image).
#
#         Parameters
#         ----------
#         kwargs
...
#
# class CellCalibEngineOLD(Dataset):
#     """Class for performing automatic cell calibration
#
#     This class is designed to define datasets related to time windows, where
#     cell calibration was performed, i.e. the camera pointing into a gas (and
...
#         bg_info = CellSearchInfo(filter_id, "bg", y_max)
#         rest = CellSearchInfo(filter_id, "rest", y_max)
#         cell_info = od() # will be filled with CellSearchInfo objects in
#                          # the loop
#         cell_count = 0 #counter for number of cells detected
#         on_cell = 0 #flag which is set when cell entry_cond is fulfilled
#         for k in range(len(y) - 2):
#             # Look for dip in intensity => candidate for calib cell time
#             # stamp
#
#             # Define entry and leave acceptance condition for detection of
#             # time window
...
#                 result = CellSearchInfo(filter_id, cell_id, y_max)
#
#             # Look for increase in intensity => candidate for removal of
#             # calib cell
#             elif leave_cond and on_cell:
#             #and onFilter:
#                 print "Reached end of cell DIP at %s, %s" %(k,x[k])
...
#
#         if (
#           not len(self._cell_info_auto_search.keys())==len(cell_info.keys())
#            ):
#             raise CellSearchError("Number of detected cells (%s) is "
#                                   "different from number of cells "
#                                   "specified in cellSpecInfo (%s) "
...
#             else:
#                 if len(val) != 2:
#                     raise ValueError("Invalid cell column specification, "
#                     "need list containing [value, uncertainty] of gas "
#                     "column with id %s, got %s" %(key, val))
#                 for k in range(len(val)):
...
#             raise ValueError("%s calibration data is not available"
#                              % calib_id)
...
#             warn("Invalid input for cell_cmap, using Oranges")
#             cmap = get_cmap("Oranges")
#         # get stored time series (was automatically saved
#         # in :func:`find_cells`)
#         ts_all = self.pix_mean_tseries[("%s_auto_search" %filter_id)]
#         # get cell search results
#         res = self.search_results
