...
class DecoratorRunner(object):

    def __init__(self, params={}, **kwargs):

        # Ensure subclass was created correctly
        for req in ['required_params', 'type']:
            if not hasattr(self, req):
                bot.exit('A DecoratorRunner must have %s' % req)

        # Every runner will collect some list of results (timepoints)
        self.timepoints = []
        self.params = {}
...
# Identification
    ...
    def __repr__(self):
        return "[decorator|%s]" % self.name

    def __str__(self):
        return "[decorator|%s]" % self.name


from watchme.tasks.decorators import DecoratorRunner

class ProcessRunner(DecoratorRunner):
    ...
    def __init__(self, name, params={}, **kwargs): 
        ...
        super(ProcessRunner, self).__init__(name, params, **kwargs)

        # Set defaults if not found in params
        self._set_defaults()
    ...
    def _set_defaults(self):
        '''iterate through params, and set defaults for those that aren't
           required.
        '''
        self.seconds = self.params.get('seconds', 3)
        self.only = self.params.get('only', [])
        self.skip = self.params.get('skip', [])
        self.include = self.params.get('include', [])
