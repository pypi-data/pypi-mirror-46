...
class OclCsvToJsonConverter:
    ...
    def process_csv_row_with_definition(self, csv_row, csv_resource_def, attr=None):
        ...
        if self.DEF_KEY_TRIGGER_COLUMN in csv_resource_def:
            if self.DEF_KEY_TRIGGER_COLUMN not in csv_row:
                raise("Trigger column '%s' not found in CSV row" % self.DEF_KEY_TRIGGER_COLUMN)
            if csv_row[self.DEF_KEY_TRIGGER_COLUMN] != self.DEF_KEY_TRIGGER_VALUE:
                if self.verbose:
                    print "SKIPPING: Trigger column '%s' does not match trigger value of '%s': %s" % (
                        self.DEF_KEY_TRIGGER_COLUMN, self.DEF_KEY_TRIGGER_VALUE, csv_row[self.DEF_KEY_TRIGGER_COLUMN])
                return
