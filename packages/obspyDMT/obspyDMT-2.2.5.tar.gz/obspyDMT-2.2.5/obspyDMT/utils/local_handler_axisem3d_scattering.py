...
def plot_sta_ev_ray(input_dics, events):
    ...
    m.scatter(x_ev, y_ev, 20, color='red', marker="o",
              edgecolor="black", zorder=10, label='0-20km')
    m.scatter(x_ev, y_ev, 20, color='orange', marker="o",
              edgecolor="black", zorder=10, label='20-50km')
    m.scatter(x_ev, y_ev, 20, color='green', marker="o",
              edgecolor="black", zorder=10, label='50-100km')
    m.scatter(x_ev, y_ev, 20, color='cyan', marker="o",
              edgecolor="black", zorder=10, label='100-200km')
    m.scatter(x_ev, y_ev, 20, color='violet', marker="o",
              edgecolor="black", zorder=10, label='200-400km')
    m.scatter(x_ev, y_ev, 20, color='blue', marker="o",
              edgecolor="black", zorder=10, label='400-700km')
    ...
    for ei in range(len(events)):
        if plt_events:
            if input_dics['plot_focal']:
                ...
                try:
                    ax = plt.gca()
                    if 0 <= events[ei]['depth'] <= 20: facecc = 'red'
                    if 20 < events[ei]['depth'] <= 50: facecc = 'orange'
                    if 50 < events[ei]['depth'] <= 100: facecc = 'green'
                    if 100 < events[ei]['depth'] <= 200: facecc = 'cyan'
                    if 200 < events[ei]['depth'] <= 400: facecc = 'violet'
                    if 400 < events[ei]['depth'] <= 700: facecc = 'blue'

                    if 5.5 <= events[ei]['magnitude'] <= 6.0: width_beach = 5e5
                    if 6.0 < events[ei]['magnitude'] <= 6.5: width_beach = 6e5
                    if 6.5 < events[ei]['magnitude'] <= 7.0: width_beach = 7e5
                    if 7.0 < events[ei]['magnitude'] <= 7.5: width_beach = 8e5

                    b = Beach(focmecs, xy=(x, y), facecolor=facecc,
