...
def main():
    ...
    parser.add_argument('-s', '--save-fasta', help='Save prophage sequences to a fasta file.', action='store_true')
    parser.add_argument('--verbose', action='store_true', help=argparse.SUPPRESS)
    parser.add_argument('--no-cleanup', action='store_true', help=argparse.SUPPRESS)
    parser.add_argument('--substitute-output', action='append', help=argparse.SUPPRESS)
    ...
    if (not args.output) and (args.extension != ['stdout']):
        ...
        exit(1)
    if (not args.output) and (args.save_fasta):
        print('Error! The basename of a fasta file is required. Please, specify it at -o/--output argument.')
        exit(1)
