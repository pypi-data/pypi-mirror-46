...
class Polygons(object):
  ...
  of an object mask. The object can be represented as a set of polygons.
  """
...
class MaskList(Extend):
  ...
  def transpose(self, method):
    ...
    return MaskList(flipped, size=self.size, mode=self.mode)
  ...
  def crop(self, box):
    ...
    return MaskList(cropped, size=(w, h), mode=self.mode)

  def resize(self, size, *args, **kwargs):
    scaled = []
    for polygon in self.polygons:
      scaled.append(polygon.resize(size, *args, **kwargs))
    return MaskList(scaled, size=size, mode=self.mode)
  ...
  def __getitem__(self, item):
    ...
    return MaskList(selected_polygons, size=self.size, mode=self.mode)
