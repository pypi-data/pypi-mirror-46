...
class EvaluatedCatalog:
	...
	@property
	def net_sales(self) -> float:
		"""Return total sales (sum of NetSales)"""
		return sum(p.net_sales for p in self.products)

	@property
	def cost_of_sales(self) -> float:
		"""Return total costs (sum of CostOfSales)"""
		return sum(p.cost_of_sales for p in self.products)

	@property
	def cons_cost_of_sales(self) -> float:
		"""Return total consolidated costs (sum of ConsCostOfSales)"""
		return sum(p.cons_cost_of_sales for p in self.products)

	@property
	def profit(self) -> float:
		"""Return total profit (sales - costs)"""
		return self.net_sales - self.cost_of_sales

	@property
	def cons_profit(self) -> float:
		"""Return total consolidated profit (sales - consolidated costs)"""
		return self.net_sales - self.cons_cost_of_sales

	@property
	def gross_margin(self) -> float:
		"""Return total gross margin"""
		return 1 - (self.cost_of_sales / self.net_sales)

	@property
	def cons_gross_margin(self) -> float:
		"""Return total consolidated gross margin"""
		return 1 - (self.cons_cost_of_sales / self.net_sales)

	@property
	def upa(self) -> float:
		"""Return catalog-wide units-per-active"""
		return sum(p.forecast for p in self.products) / self.activeCount

	@property
	def npu(self) -> float:
		"""Return catalog-wide netprice-per-unit"""
		return self.net_sales / sum(p.forecast for p in self.products)

	@property
	def spa(self) -> float:
		"""Return catalog-wide sales-per-active"""
		return self.net_sales / self.activeCount
