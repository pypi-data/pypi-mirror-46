...
from azureml.core import model as mlsModel
...
class ForecastTrainer(object):
	...
	def __init__(self, name, batch_size, epochs, nodes, test_data, train_data, accuracy_sample, features, dropout, layers):
		self.name = name
	...
	def build_regressor(self):
		self.regressor = KerasRegressor(build_fn=self.setup_base_line_model, batch_size=self.batch_size, epochs=self.epochs, verbose=2)
	...
	def get_best_model(self):
		...
		m.model.load_weights(ForecastTrainer.get_weight_file_path(self.run.id))

		return m


	def save_best_model(self, model_name: str, ws: Workspace, path_prefix = '/dbfs/mnt/ForecastStore/staging/'):
		...
		ModelSerializer.save_keras_model(model.model, path_prefix + model_name, "model")

		exportedModel = mlsModel.Model.register(workspace=ws,
												model_path=path_prefix + model_name + "/",
												model_name=model_name,
												description="Accuracy: {:.2f} (ep: {:d})".format(
													self.callback.floating_score, self.callback.saved_epoch), )
...
class TimeHistory(Callback):
	def __init__(self, m, epochs, accuracy_sample):
		self.m = m
		self.accuracy_sample = accuracy_sample
		self.accuracies = []
	...
	def checkpoint(self, epoch):
		...
		self.floating_model_weights = self.m.regressor.model.save_weights(ForecastTrainer.get_weight_file_path(self.m.run.id))
