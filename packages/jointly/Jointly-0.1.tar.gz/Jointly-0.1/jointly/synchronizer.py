    ...
    def _calculate_sync_params(self):
        ...
        for column in df_equi.columns:
            if column == self.ref_source_name:
                ...
                #         old_delta = new_segments[column][segment][timestamp] - start_time
                #         new_delta = old_delta * self.sources[column]['stretch_factor']
                #         new_segments[column][segment][timestamp] = start_time + new_delta
                # new_timeshifts = timeshifts
                # for segment in ['first', 'second']:
                #         shifted_start = segments[column][segment]['start'] + timeshifts[segment]
                #         old_delta = shifted_start - start_time
                #         new_delta = old_delta * self.sources[column]['stretch_factor']
                #         new_shifted_start = start_time + new_delta
                #         new_timeshifts[segment] = new_shifted_start - segments[column][segment]['start']
                # logger.debug(new_timeshifts)
                # self.sources[column]['timeshift'] = new_timeshifts['first']
