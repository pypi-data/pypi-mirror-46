...
.. code:: python
  ...
  def test_futures (app, dbpath):
    @app.route ("/")
    def index (was):
        def response (was, tasks, stock):
            stock.announcements = tasks [0].fetch ()
...
.. code:: python
    ...
    @app.route ("/")
    def index (was):
        def repond (was, tasks, b, status_code):
            return was.API (status_code_db = [task.status_code for task in tasks.dispatch ()], b = b, status_code = status_code) 
        
        def checkdb (was, rss, a):
            reqs = [was.db ("@sqlite").execute ('SELECT * FROM stocks WHERE symbol=?', ('RHAT',))]
            status_code = [rs.status_code for rs in rss.dispatch ()]
