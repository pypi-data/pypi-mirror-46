...
def run_subprocess_split(command):
    ...
    if sys.version_info >= (3, 0):
        ...
        output = unicode(output, 'utf-8').strip()  # noqa
...
def check_python_packages(edit_mode=False, packages=None):
    ...
    if edit_mode:
        all_packages = run_subprocess_split(['pip', 'list', '-e'])
    else:
        # list all packages, including in editable mode
        all_packages = run_subprocess_split(['pip', 'list'])
...
def check_conda():
    ...
    try:
        all_info = run_subprocess_split(['conda', 'info'])
...
def check_conda_packages(edit_mode=False, packages=None):
    ...
    try:
        all_packages = run_subprocess_split(['conda', 'list', '--no-pip', '--export'])
    except subprocess.CalledProcessError:
...
def check_hardware():
    ...
    if sys.platform.startswith('darwin'):
        all_info = run_subprocess_split(['sysctl', '-a'])
        for line in all_info.split("\n"):
    ...
    # linux
        ...
        all_info = run_subprocess_split(['lscpu'])
        for line in all_info.split("\n"):
    ...
    # windows
        ...
        all_info = run_subprocess_split(['wmic', 'cpu', 'get', 'name'])
        if "Name" in all_info:
