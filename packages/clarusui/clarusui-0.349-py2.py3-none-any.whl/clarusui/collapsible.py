...
class Collapsible(Element):
    ...
    def _get_scroll_period(self):
        if self._get_header() is None or len(self._get_header()) == 0:
            ...
            calced = len(self._get_header())/self._scrollSpeed
            min = 10
...
class EventTicker(Collapsible):
    def __init__(self, response, scrollSpeed=5, **options):
        super(EventTicker, self).__init__(header=self._extract_latest(response), scrollSpeed=scrollSpeed, body=Table(response), scroll=True, **options)
    ...
    def _extract_latest(self, response):
        ...
        if df.empty:
            ...
            for index, row in latestRows.iterrows():
                extracted.append('<i class="fa fa-clock-o"></i>  ' + row['Time'] + '   <i class="fa fa-angle-double-right"></i>   ' + row['Description'])
