...
class Element(object):
    ...
    def _set_inputs(self, response):
        ...
        self.otherResponses = None
        if type(response) in [tuple, list]:
            realTime = self.extract_response_by_context(response, 'realtime')
            ...
            lastMove = self.extract_response_by_context(response, 'lastmove')
            if lastMove is not None:
                self.lastMoveResponse = lastMove
                
            if len(response) > 0:
                self.otherResponses = response
            #else:
            #    if len(response) >= 2:
    ...
    def extract_response_by_context(self, fullResponse, context):
