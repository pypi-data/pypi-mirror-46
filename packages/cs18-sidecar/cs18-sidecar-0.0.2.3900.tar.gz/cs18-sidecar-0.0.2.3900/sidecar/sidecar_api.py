...
from sidecar.model.objects import ISidecarConfiguration, KubernetesSidecarConfiguration, \
    AwsSidecarConfiguration, AzureSidecarConfiguration, SidecarApplication, EnvironmentType
...
def get_status_updater(config: ISidecarConfiguration,
    ...
    if isinstance(config, AwsSidecarConfiguration):
        ...
        _deployment_outputs_store = AwsDeploymentOutputStore(_logger, aws_session, aws_status_maintainer)
...
def save_application_deployment_output(app_name: str, instance_id: str):
    req = request  # type: Request
    _deployment_outputs_store.save_app_output(app_name, instance_id, req.data)
    return '', 200
...
def save_service_deployment_output(name: str):
    ...
    try:
        ...
        _logger.exception(f"service '{name}' deployment output is not valid:\n{output}")
        raise exc
