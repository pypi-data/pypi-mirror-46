    ...
    def get_deployment_outputs(self, outputs: List[str]) -> str:
        ...
        for output in outputs:
            try:
                ...
                outputs = entity_name_outputs_map.get(entity_name, None)
                if outputs is None:
                    outputs = self._status_maintainer.get_deployment_output(entity_name)
                    entity_name_outputs_map[entity_name] = outputs
                ...
                res[output] = repr(ex)
        return " ".join("{}={}".format(k, shlex.quote(v)) for k, v in res.items())
