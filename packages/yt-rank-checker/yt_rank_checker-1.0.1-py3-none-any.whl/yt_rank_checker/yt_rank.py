...
def yt_rank(channel, keywords=[], iterator_value=4):
    channel_name = channel
    keyword_list = keywords
    if type(keyword_list) is not list:
        raise Exception('did not provide keywords in list format')
        
    if len(keyword_list) == 0:
        raise Exception('please add keywords before running the rank checker')
    try:
