...
def entry_point(command):
    """Generate entry point for a command."""
    package = about['__project__'].replace('-', '_')
    module = command.replace('-', '_')
    return '{}={}.{}:{}'.format(command, package, module, 'main')
...
setup(
    ...
    entry_points={
        'console_scripts': [
            entry_point(about['__player__']),
            entry_point(about['__recorder__']),
        ]
    })
