...
class LifeGame:
    ...
    def __init__(self, screen_width=800, screen_height=600, cell_size=10, alive_color=(0, 255, 255),
        ...
        self.grid_count = (self.screen_width*self.screen_height)/100
        ...
        self.frame_num = 0
        self.old_frame_num = 0
        self.max_fps = 45
        self.desired_milliseconds_between_updates = (1.0 / self.max_fps) * 10000
    ...
    def check_cell_neighbors(self, row_index, col_index):
        ...
        if self.grids[self.active_grid][row_index][col_index] == 1:  # alive
            if num_alive_neighbors > self.over_pop:  # Overpopulation
                return 0
            if num_alive_neighbors < self.under_pop:  # Underpopulation
                return 0
            if num_alive_neighbors == self.from_pop or num_alive_neighbors == self.to_pop:
                return 1
        elif self.grids[self.active_grid][row_index][col_index] == 0:  # dead
            if num_alive_neighbors == self.cometo_pop:
    ...
    def update_generation(self):
        ...
        for r in range(self.num_rows - 1):
            for c in range(self.num_cols - 1):
                if self.frame_num > self.old_frame_num + self.grid_count:  # old_frame_num + grid_count = 1 generation
                    if self.step:  # (step == True) = Frame by Frame is ON
                        input("Press Enter for next generation...")
                    self.old_frame_num = self.frame_num
                    print('Generation: ' + str(int(self.frame_num/self.grid_count)))
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.unicode == 's':
                    ...
                    self.to_pop += 1
                    self.cometo_pop += 1
                    print('Death if more than: ' + str(self.over_pop)
                          + '   Death if less than: ' + str(self.under_pop)
                          + '   Alive from: ' + str(self.from_pop)
                          + '   Alive to: ' + str(self.to_pop)
                          + '   Come alive at: ' + str(self.cometo_pop))
