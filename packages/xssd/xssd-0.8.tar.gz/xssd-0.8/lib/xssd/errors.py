...
class ValidateError(object):
    ...
    def __repr__(self):
        result = [self._msg]
        if self._cont:
            result.append('{' + str(self._cont) + '}')
        if self._desc:
            result.append('"' + self._desc + '"')
        return ' '.join(result)

    def __unicode__(self):
        return unicode(str(self._msg))
...
INVALID_FRACTION = ValidateError(0x12, 'Invalid Fraction', 'Fraction is not the right format.')
