...
from sdk.providers import AdminProvider, RetailerProvider, ProviderHTTPClientException
from sdk.providers.facenet import FacenetProvider
from sdk.utils import Timer
...
@click.argument('image', help='URL to image, use product preview/product display')
@pass_rebotics_context
def warmup_facenet(ctx, retailer_id, retailer_secret, facenet_url, image):
    ...
    with Timer() as t:
        result = facenet_classifier(image)
    if ctx.verbose:
        click.echo(result)
