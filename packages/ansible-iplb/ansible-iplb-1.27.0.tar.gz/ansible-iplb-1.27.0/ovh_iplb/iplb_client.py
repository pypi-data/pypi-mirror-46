...
class Frontend(Resource):
    ...
    def body(self):
        body = dict({
            ...
            for k in ('allowed_source', 'dedicated_ipfo', 'default_farm_id', 'default_farm_name',
                      'default_ssl_id', 'disabled', 'display_name', 'hsts', 'http_header', 'port',
                      'redirect_location', 'ssl', 'zone')
            if self.frontend_def.get(k) is not None
        }, display_name=self.name())

        if 'default_farm_id' in self.frontend_def and 'default_farm_name' in self.frontend_def:
            raise ValueError('It is not allowed to declare default_farm_id and default_farm_name as a same time')
...
class IPLB(Resource):
    ...
    def apply(self):
        ...
        change = any(resource.apply() for resource in itertools.join(self.farms(), self.frontends()))
