...
class PTBaseModel(ABC):
    ...
    def fit(self):
        ...
        while epoch <= self.num_training_epochs:
            ...
            if validation_gap_loss > 0 or validation_gap_metric > 0:
                ...
                print(f" * Save model at Epoch {epoch}\t | Improved loss: {validation_gap_loss:.3f}\t | Improved accuracy: {validation_gap_metric:.3f}")
            else:
                # early stop schedule
                epochs_since_improvement += 1
                print(f" * Have not improved for {epochs_since_improvement} rounds")
    ...
    def _save_latest_checkpoint(self, checkpoint_dir, max_to_keep=4, verbose=0):
        ...
        for file in files_tobe_deleted:
            os.remove(file)
            if verbose:
                print("Only keep {} model files, remove {}".format(max_to_keep, checkpoint_dir + file))
