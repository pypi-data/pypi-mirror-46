    ...
    def backoff(cls, tries=10, delay=3, backoff=1.1, added_exceptions=list()):
        ...
        def deco(f):
            ...
            def retry_func(*args, **kwargs):
                ...
                while max_tries > 1:
                    logger.info(f'[AWSRetry] *re-running function {f.__name__}... (tries left: {max_tries}')
                    try:
                        logger.info(f'[AWSRetry] args {args}')
                        logger.info(f'[AWSRetry] args {kwargs}')
                        return f(*args, **kwargs)
                    except Exception as e:
                        logger.error(f'[AWSRetry] we got an exception: {e}')
...
class AWSRetry(CloudRetry):
    ...
    @staticmethod
    def status_code_from_exception(error):
        logger.error('[AWSRetry] errorformatted #1: {}'.format(error))
        logger.error('[AWSRetry] errorformatted #2: {}'.format(error.response['Error']['Code']))
        logger.error('[AWSRetry] errorformatted #3: {}'.format(error.response['Error']))
        if isinstance(error, botocore.exceptions.ClientError):
