            ...
            # Event: Mouse-Button Down
                ...
                if "F7" in keys_pressed:
                    if not self.active_actor:
                        self.active_actor_toolbar = active_actor_toolbar.ActiveActorToolbar(self.board)
                        self.add_container(self.active_actor_toolbar, dock="bottom", size=40)
                        self.log.info("Added active actor toolbar")
                    elif self.active_actor:
                        self.remove_container(self.active_actor_toolbar)
                        self.active_actor_toolbar = None
                else:
