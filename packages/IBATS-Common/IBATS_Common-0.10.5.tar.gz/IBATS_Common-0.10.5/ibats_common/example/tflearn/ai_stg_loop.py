...
class AIStg(StgBase):
    ...
    def __init__(self, unit=1, train=True):
        ...
        folder_path = os.path.join(module_root_path, 'tf_saves')
        model_folder_path = os.path.join(folder_path, 'model_tfls')
        if not os.path.exists(model_folder_path):
            os.makedirs(model_folder_path, exist_ok=True)
        file_path = os.path.join(model_folder_path, f"model_{int(self.classify_wave_rate * 10000)}.tfl")
        self.model_file_path = file_path
        self.checkpoint_path = os.path.join(model_folder_path, f'model_{int(self.classify_wave_rate * 10000)}.tfl.ckpt')
        tensorboard_dir = os.path.join(model_folder_path, 'tensorboard_logs')
        ...
        with sess.as_default():
            ...
            for num in range(1, 4):
                ...
                if result[0] > 0.55:
                    ...
                    logger.warning('第 %d 次训练，样本外训练精度不足，继续训练', num)
