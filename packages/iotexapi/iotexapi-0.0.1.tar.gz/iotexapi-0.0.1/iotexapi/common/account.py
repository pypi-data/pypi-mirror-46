...
from iotexapi.utils  import is_hex

from iotexapi.common.datastructures import AttributeDict
...
class Address(object):
    @staticmethod
    def from_eth(address):
        """Helper function that will convert a generic value from hex"""
        if is_hex(address):
            addressBytes= unhexlify(bytes(address[2:], encoding='utf8'))
            return bech32.bech32_encode('io', bech32.convertbits(addressBytes, 8, 5))    
    ...
    def to_eth(address):
        ...
        if not is_hex(address) and address[0:2]=='io':
            hrpgot, data = bech32.bech32_decode(address)
...
class PrivateKey(object):
    ...
    def address(self):
        ...
        addressBytes= unhexlify(bytes(address[2:], encoding='utf8'))
        addressBech32 =  bech32.bech32_encode('io', bech32.convertbits(addressBytes, 8, 5))    
