...
def _parse_substitutions_url(url) -> List[SubstitutionTable]:
    ...
    assert len(has_content_list) == len(dates), "amount of has_content is not equal to amount of dates"
    assert len(add_infos) == len(subs_tables), "amount of add infos is not equal to amount of subs tables"
    ...
    for (i, (date, has_content)) in enumerate(zip(dates, has_content_list)):
        if not has_content:
            ...
            tables.append(_parse_substitution_table(date, add_infos[i - amount_no_content], subs_tables[i - amount_no_content]))
        
    
    assert len(subs_tables) == len(has_content_list) - amount_no_content, "amount_no_content logic is wrong"
