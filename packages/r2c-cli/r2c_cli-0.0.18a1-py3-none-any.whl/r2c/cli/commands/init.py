...
def init(ctx, analyzer_name, org, author_name, author_email, run_on, output_type):
    ...
    desc = click.prompt(
        "Short description (2-3 sentence) of your analyzer!",
        default=f"{analyzer_name} by {author_name} for {org}",
    )
    create_template_analyzer(
        org, analyzer_name, author_name, author_email, run_on, output_type, desc
    )
