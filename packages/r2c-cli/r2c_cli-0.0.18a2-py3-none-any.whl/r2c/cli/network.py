...
from r2c.lib.constants import PLATFORM_BASE_URL, PLATFORM_PIPELINE_PREFIX
...
def do_login(
    ...
    if org is None:
        ...
        if org is None:
            org = click.prompt(
                "Please enter your group name, or to use the common r2c platform, press enter",
                default=PLATFORM_PIPELINE_PREFIX,
            )
    if not login_token:
        if click.confirm(
...
def get_base_url(org: Optional[str] = get_default_org()) -> str:
    ...
    if org == PLATFORM_PIPELINE_PREFIX:
