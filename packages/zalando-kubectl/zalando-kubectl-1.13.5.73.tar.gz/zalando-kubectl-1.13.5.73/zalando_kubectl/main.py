...
@click.pass_obj
def completion(env: Environment, kubectl_arg):
...
@click.pass_obj
def traffic(env: Environment, force, ingress, backend, weight):
    kube_config.update_token()

    try:
        if backend is None:
            # print existing weights
            ingress = zalando_kubectl.traffic.get_ingress(env.kubectl, ingress)
