...
class OnshapeXBlock(StudioEditableXBlockMixin, XBlock):
    ...
    client_id = String(scope=Scope.content, default="", display_name="Client ID", help='Please create an OAuth app on the Onshape dev portal for this block here: https://dev-portal.onshape.com . Make sure to give the app read scope. This value need only be entered once per course')
    client_secret = String(scope=Scope.content, display_name="Client Secret", default="")
    redirect_uri = String(scope=Scope.content, display_name="Redirect URI", default="")
