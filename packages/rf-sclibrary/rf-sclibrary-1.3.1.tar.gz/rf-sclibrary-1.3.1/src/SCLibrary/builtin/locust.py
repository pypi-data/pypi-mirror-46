...
class LocustWorker(object):
    ...
    def run(self, task, host, num_requests=10, num_clients=1, hatch_rate=10, timeout='3m'):
        ...
        `hatch_rate`: 请求频率
        '''
        logger.info('num_requests: %s num_clients: %s hatch_rate: %s' % (num_requests, num_clients, hatch_rate))
        ...
        for name, value in runners.locust_runner.stats.entries.items():
            ...
            logger.info("[压测报告]==> 接口:%s 请求方式:%s 最快响应时间:%s ms 平均响应时间:%s ms 最慢响应时间:%s ms 每秒请求数:%s" % (name[0], name[1], min_response_time, median_response_time, max_response_time, total_rps))
    ...
    def run_with_web(self, task, host):
        ...
        logger.info("Web本地监视器已运行: %s:%s" % (options.web_host or "*", options.port))
    ...
    def vaildate(self, url, result):
        ...
        try:
            if self.rd_min_threshold[url]:
                assert int(self.rd_min_threshold[url]) >= int(result.min_response_time), '接口: %s%s ===> 最快响应不大于%s毫秒' % (self.host, url, self.rd_min_threshold[url])
        except KeyError:
            pass
        try:
            if self.rd_ave_threshold[url]:
                assert int(self.rd_ave_threshold[url]) >= int(result.median_response_time), '接口: %s%s ===> 平均响应时间不能大于%s毫秒' % (self.host, url, self.rd_ave_threshold[url])
        except KeyError:
            pass
        try:
            if self.rd_max_threshold[url]:
                assert int(self.rd_max_threshold[url]) >= int(result.max_response_time), '接口: %s%s ===> 最大响应时间不能大于%s毫秒' % (self.host, url, self.rd_max_threshold[url])
        except KeyError:
            pass
