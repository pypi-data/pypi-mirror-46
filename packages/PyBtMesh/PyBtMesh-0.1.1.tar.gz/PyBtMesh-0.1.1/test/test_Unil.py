...
class TestUtil(unittest.TestCase):
    ...
    def test_s1(self):
        self.assertEqual(Util.s1(b'test'), bytes.fromhex(
    ...
    def test_k1(self):
        ...
        self.assertEqual(Util.k1(N, SALT, P), bytes.fromhex(
    ...
    def test_appkey(self):
        appkey = Util.ApplicationKey(bytes.fromhex(
            '63964771734fbd76e3b40519d1d94a48'))
        self.assertEqual(appkey.aid, 0x26)

    def test_netkey(self):
        netkey = Util.NetworkKey.fromString(
            '7dd7364cd842ad18c17c2b820c84c3d6', iv_index=0)
        self.assertEqual(netkey.nid, 0x68)
        self.assertEqual(netkey.encrypt_key, bytes.fromhex(
            '0953fa93e7caac9638f58820220a398e'))
        self.assertEqual(netkey.privacy_key, bytes.fromhex(
            '8b84eedec100067d670971dd2aa700cf'))
        self.assertEqual(netkey.network_id, bytes.fromhex('3ecaff672f673370'))
        self.assertEqual(netkey.identity_key, bytes.fromhex(
            '84396c435ac48560b5965385253e210c'))
        self.assertEqual(netkey.beacon_key, bytes.fromhex(
