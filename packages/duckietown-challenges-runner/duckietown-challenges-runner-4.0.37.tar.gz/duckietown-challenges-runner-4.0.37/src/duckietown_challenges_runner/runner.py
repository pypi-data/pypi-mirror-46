...
"""
    ...
    parser.add_argument("--ipfs", dest='no_cache', action="store_true", default=False)
    parser.add_argument("--one", dest='no_cache', action="store_true", default=False)
    ...
    args = dict(do_upload=do_upload, do_pull=do_pull, more_features=more_features,
                ...
                allow_host_network=True, use_ipfs=parsed.ipfs)
    ndots = 0
...
def go_(submission_id, do_pull, more_features, do_upload, delete, reset, evaluator_name,
    ...
    cr, uploaded = get_cr(res, tmpdir, do_upload, evaluator_name, job_id, delete, debug_volumes, do_pull,
                          copy_to_machine_cache=copy_to_machine_cache, allow_host_network=allow_host_network)

    msg = ('Reporting status %s for job %s submission %s.\n\n%s\n\n%s\n\n%s' %
           (cr.get_status(), job_id, submission_id,
            indent(cr.msg, ' msg |'), indent(json.dumps(cr.get_stats(), indent=4), 'stats |'), cr.ipfs_hashes))
...
def get_cr(res, tmpdir, do_upload, evaluator_name, job_id, delete, debug_volumes, do_pull,
           copy_to_machine_cache: bool, allow_host_network: bool) -> Tuple[
...
def upload_files(wd, aws_config, ignore_patterns=('.DS_Store',), copy_to_machine_cache: bool = True):
    ...
    # elogger.info(f'Files to upload: \n {toupload}')
    if not aws_config:
