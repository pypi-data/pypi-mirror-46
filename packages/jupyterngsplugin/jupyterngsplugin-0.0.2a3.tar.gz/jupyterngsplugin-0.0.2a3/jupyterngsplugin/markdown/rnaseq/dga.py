...
def print_tools(c, tools, files, data_path, width, height):
    ...
    for t in tools:
        ...
        if len(f) == 1:
            str_msg += get_link_image(os.path.relpath(os.path.join(data_path, f[0])),
                                      width, height, ' --- |')
        else:
            str_msg += ' --- |'
    str_msg += '\n| '
...
def dga_gene_list_union(conditions, data_path, organism):
    ...
    for c in conditions:
        ...
        if os.path.exists(f) and os.path.getsize(f) != 0:
            str_msg += '### Condition: ' + c.replace('_vs_', ' vs ') + '\n\n'
            ...
            if len(over_df) > 0:
                over_df['Gene_Id'], over_df['Chr'], over_df['Start_pos'] = \
                    over_df['Gene_Id'].str.split('_', 2).str
            ...
            if len(under_df) > 0:
                under_df['Gene_Id'], under_df['Chr'], under_df['Start_pos'] = \
                    under_df['Gene_Id'].str.split('_', 2).str
            ...
            if under_count > 0 or over_count > 0:
                ...
                if len(under_df) > 0:
                    ...
                    for i, r in under_df.head(30).iterrows():
                        ...
                        str_msg += '<a href="https://www.ncbi.nlm.nih.gov/gene/?term=' \
                                   + r['Gene_Id'] \
                                   + '%5BGene+Name%5D+AND+Homo+sapiens%5BOrganism%5D" ' \
                                   + 'target="_blank">' \
                                   + r['Gene_Id'] + '</a>'
                        str_msg += '</td>'
                        str_msg += '<td>' + "{:.3f}".format(r['logFC']) + '</td>'
