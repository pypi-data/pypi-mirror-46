...
def add_quyu_fast(quyu,tag='all'):
    ...
    try:
        c.run("""/opt/python35/bin/python3 -c "from lmfhawq.zl import add_quyu_fast_local;add_quyu_fast_local('%s','%s') " """%(quyu,tag),pty=True,encoding='utf8')
    except:
        traceback.print_exc()
    finally:
        c.close()
...
def add_quyu_fast1(quyu,tag='all'):
    ...
    c=Connection(conp_remote[0],connect_kwargs={"password":conp_remote[1]})
    try:
        c.run("""/opt/python35/bin/python3 -c "from lmfhawq.zl import add_quyu_fast1_local;add_quyu_fast1_local('%s','%s') " """%(quyu,tag),pty=True,encoding='utf8')
    except:
        traceback.print_exc()
    finally:
        c.close()
...
def add_quyu_fast2(quyu,tag='all'):
    ...
    c=Connection(conp_remote[0],connect_kwargs={"password":conp_remote[1]})
    try:
        c.run("""/opt/python35/bin/python3 -c "from lmfhawq.zl import add_quyu_fast2_local;add_quyu_fast2_local('%s','%s') " """%(quyu,tag),pty=True,encoding='utf8')
    except:
        traceback.print_exc()
    finally:
        c.close()
...
###主要两个接口，HAWQ 删除src上相应分区
def add_quyu_fast3_local(quyu,tag='all'):
    arr=quyu.split("_")
    db,schema=arr[0],'_'.join(arr[1:])
    conp=['gpadmin','since2015','192.168.4.179','base_db','src']
    conp_src=['postgres','since2015','192.168.4.188',db,schema]
    add_quyu(quyu,conp=conp,conp_src=conp_src,addr='192.168.4.187:8111',dir='/data/lmf',tag=tag)


def add_quyu_fast3(quyu,tag='all'):
    conp_remote=["root@192.168.4.187","rootHDPHAWQDatanode5@zhulong"]
    c=Connection(conp_remote[0],connect_kwargs={"password":conp_remote[1]})
    try:
        c.run("""/opt/python35/bin/python3 -c "from lmfhawq.zl import add_quyu_fast3_local;add_quyu_fast3_local('%s','%s') " """%(quyu,tag),pty=True,encoding='utf8')
    except:
        traceback.print_exc()
    finally:
        c.close()
