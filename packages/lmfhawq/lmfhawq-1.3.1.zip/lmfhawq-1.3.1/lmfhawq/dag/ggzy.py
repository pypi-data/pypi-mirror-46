import sys
import os 
from lmfhawq.dag.data import ggzy_para 
...
def write_dag(quyu,dirname,**krg):
    ...
    content=content.replace("##zhulong.anqing##",'zhulong.%s'%schema)
...
def write_dags(dirname):
    for w in ggzy_para[:2]:
        quyu='_'.join(w[:2])
        timeout=w[2]
        write_dag(quyu,dirname,timeout=timeout)
