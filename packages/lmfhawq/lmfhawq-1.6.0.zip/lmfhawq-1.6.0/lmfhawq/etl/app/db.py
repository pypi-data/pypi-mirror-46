...
def update_tb1(conp):
    ...
    bgdate=datetime.strftime(datetime.now()-timedelta(days=30),'%Y-%m-%d')
    eddate=datetime.strftime(datetime.now()-timedelta(days=29),'%Y-%m-%d')
    sql_a="delete from public.gg where ggstart_time<'%s' or ggstart_time>'2025-01-01'"%(eddate)
    sql_b="delete from public.gg_html where  ggstart_time<'%s'  or ggstart_time>'2025-01-01' "%(eddate)
    print(sql_a)
    print(sql_b)
