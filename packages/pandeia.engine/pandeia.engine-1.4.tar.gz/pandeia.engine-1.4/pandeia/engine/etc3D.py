    ...
    if hasattr(strategy, "calc_type"):
        ...
        for dither in dither_list:
            ...
            if instrument.projection_type in ['slitless', 'multiorder']:
                # backwards compatibility with the old way to specify orders as a number:
                if 'norders' in instrument.disperser_config[instrument.instrument['disperser']]:
                    orders = np.arange(1,instrument.disperser_config[instrument.instrument['disperser']]['norders']+1)
                    warn('The order-configuration API has changed. Support for older pandeia_refdata packages '
                                  'will be removed in v1.5.', category=FutureWarning)
                elif 'orders' in instrument.disperser_config[instrument.instrument['disperser']]:
