...
class ConvolvedSceneCube(object):
    ...
    def get_fov_size(self, pixbuffer=SCENE_FOV_PIXBUFFER):
        ...
        readmode = self.instrument.detector["readmode"]
        ...
        if self.instrument.projection_type == 'multiorder':
            ...
            if readmode in self.instrument.subarray_config:
                subarray_config = self.instrument.subarray_config[readmode][subarray]
...
class AdvancedPSF(object):
    ...
    def __init__(self, wave, psf_library, instrument, mode, model_scene=None, psf_source_offset=(0, 0),
        ...
        if (model_scene is None) or (empty_scene):
            ...
            # replace infinities in misconfigured scenes so they don't crash or disappear.
            # np.all will only be true if all of the elements are true (not nan or inf).
            if not np.all(np.isfinite(model_scene.int)):
                # warn the user about what they've done
                key = "scene_fp_overflow"
                self.warnings[key] = warning_messages[key]
                model_scene.int = np.nan_to_num(model_scene.int)
                model_scene.int[np.where(model_scene.int > SCENE_MAX_PIXELVAL)] = SCENE_MAX_PIXELVAL
