Currently Supported Calculation Modes (v1.3)
===============================================
    ...
    shape: dict
      Source shape parameters described by the following keys:
        ...
        geometry: string (default "point")
            Supported geometries are "point", "gaussian2d", "flat", "sersic", and "sersic_scale". Required
...
background: string (default 'minzodi') or list-like or numpy.ndarray
  ...
  For backwards compatability with older files, if background is set to string values of "high",
  "medium", or "low", it is taken as the background level, and the background location is set to
  "minzodi". A deprecation warning is issued.
