...
def _print_helpdoc():
    ...
    print("     \033[0;34m[-vn]\033[0m or \033[0;34m[--version]\033[0m  version number to package to,the default is the same as the project")
    print("     \033[0;34m[-bn]\033[0m or \033[0;34m[--build]\033[0m  build number to package to,the default is the same as the project")
...
def main(prog=None,args=None):
    ...
    args = sys.argv[2:]
    shortargs = 'a:p:b:e:vn:bn:' #短选项模式
    longargs = ['address=', 'platform=', 'branch=', 'environment=', 'version=', 'build='], #长选项模式
    try:
        try:
            ...
            for opt, arg in opts:
                if opt in ("-a", "--address"):
                    ...
                    NET_ENV = arg
                elif opt in ("-vn", "--version"):
                    PROJECT_VERSION = arg
                elif opt in ("-bn", "--build"):
                    PROJECT_BUILD = arg
