...
class Package:
    ...
    def merge_code(self,code_branch,package_branch):
        ...
        try:
            ...
            repo_remote.git.reset('--hard')#清空壳工程本地工作空间
            repo_remote.pull()#拉取开发分支最新代码
            ...
            for moduleConfig in moduleConfigList:
                ...
                sub_repo.git.reset('--hard')
                #拉取子模块开发分支代码
                dev = sub_repo.git.checkout(moduleConfig["branch"])
                sub_remote.pull()
                #拉取子模块打包分支代码
                package = sub_repo.git.checkout(package_branch)
                sub_remote.pull()
                #合并本地开发分支到本地打包分支
                base = sub_repo.merge_base(dev,package)#两个分支的合并基数
                sub_repo.index.merge_tree(package,base=base)#合并
                merge_str = 'Merge ' + code_branch + ' into ' + package_branch
                sub_repo.index.commit(merge_str,parent_commits=(dev.commit,package.commit))
                print("\033[1;32m" + moduleConfig["module"] + "\033[0m" + " merge success")
                package.push()
            
            shell_dev = repo.branches[code_branch]
            shell_package = repo.git.checkout(package_branch)
            repo_remote.pull()
            #合并本地开发分支到本地打包分支
            base = repo.merge_base(shell_dev,shell_package)#两个分支的合并基数
            repo.index.merge_tree(shell_package,base=base)#合并
            merge_str = 'Merge ' + code_branch + ' into ' + package_branch
            repo.index.commit(merge_str,parent_commits=(shell_dev.commit,shell_package.commit))
            print("\033[1;32m" + "shell" + "\033[0m" + " merge success")
            shell_package.push()
            successlog("push shell success")
