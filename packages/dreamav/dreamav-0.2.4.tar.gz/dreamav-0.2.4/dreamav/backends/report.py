# -*- coding: utf-8 -*-
import requests
import hashlib
import json
import os
import time
import datetime
from virus_total_apis import PublicApi as VirusTotalPublicApi
import multiprocessing as mp

key_file = 'key.txt'

file_path = '/mnt/hgfs/capstone/pdf/data/google_pdf'

report_path = '/mnt/hgfs/capstone/pdf/data/report/google_vt_report'

key_list = []


def key_list_update():
   with open(key_file, 'r') as f:
       lines = f.readlines()
       for line in lines:
           key_list.append(line.split('\n')[0])


def create_file_list(root):
   ret_list = []
   for path, dirs, files in os.walk(root):
       for file in files:
           ret_list.append(file)
   return ret_list


def report_save(items):
   _apikey, resource = items[0], items[1]
   _sha = items[1][:-4]

   try:
       vt = VirusTotalPublicApi(_apikey)
       response = vt.get_file_report(_sha)

       if response['response_code'] == 200:
           print("report save", _sha)
           outfile_name = _sha + '.json'
           with open(os.path.join(report_path, outfile_name), 'w') as outfile:
               json.dump(response, outfile)
       else:
           return items

   except Exception as ep:
       print("Error", ep)

def run():
   print("Start")
   key_list_update()

   start_time = time.time()

   file_list = create_file_list(file_path)

   total = list(zip([key_list[i%len(key_list)] for i in range(len(file_list))], file_list))

   p = mp.Pool(4)

   while len(total):
       temp = p.map(report_save, total)
       total = [ each for each in temp if each != None ]

   print("End \n")
   print("running time : ", time.time() - start_time)

if __name__ == '__main__':
   run()
