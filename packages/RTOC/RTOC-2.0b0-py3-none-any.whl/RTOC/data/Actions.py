...
#            print(mytext)
    ...
    def showAboutMessage(self):
        pyqtlib.info_message(self.tr("Über"), "RealTime OpenControl 1.9.7.8", self.tr(
    ...
    def updateNetworkMenu(self):
        ...
        for s in self.config['knownHosts']:
            if s not in activeConnections:
                ...
                action.triggered.connect(partial(self.logger.remote.connect,s))
    ...
    def EditRemoteHost(self, host):
        self.remoteHostWidget = QtWidgets.QDockWidget(host,self)
        widget = RemoteWidget(self, host, self.remoteHostWidget)
        self.remoteHostWidget.setWidget(widget)
        self.remoteHostWidget.setFloating(True)
        self.remoteHostWidget.show()

    def connectNewHost(self):
        ans, ok = pyqtlib.text_message(self, 'Verbinde mit neuem Host', 'Verbinde RTOC mit einem neuen Host\n(Inklusive Port ":")', '127.0.0.1')
        if ok:
            if len(ans.split(':')) == 2:
    ...
    def setRemoteSamplerate(self):
        ans, ok = pyqtlib.text_message(self, 'Remote Updaterate ändern', 'Ändere die Updaterate, mit der Remote-Geräte abgefragt werden', '1')
        if ok:
            try:
                samplerate = int(ans)
                self.logger.remote.changeSamplerate(samplerate)
