...
def _update_python_env(ectx, process_chain=(), mode=''):
    ...
    packet_data_by_name = {name: inst for name, inst in ectx.db.get_active_installs().items() if inst}
    ...
    for name, data in packet_data_by_name.items():
        if name not in ectx.pm.env_generators.keys():        # Skip if we don't know environment generator for a packet
            continue
        ...
        generators = ectx.pm.env_generators[name]
