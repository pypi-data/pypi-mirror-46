...
from . import error, util, templates, merge, context
from .util import b, cmdbuilder, BytesIO, strtobytes
...
class hgclient(object):
    ...
    def encoding(self):
        ...
        if not b('getencoding') in self.capabilities:
            raise error.CapabilityError('getencoding')
    ...
    def heads(self, rev=[], startrev=[], topological=False, closed=False, branch=None, template=templates.changeset):
        ...
        if branch:
            args = cmdbuilder(b('heads'), branch, r=startrev, t=topological, c=closed,
                              template=template, hidden=self.hidden,
                              *rev)
        else:
        ...
        if template == templates.changeset:
            out = self._parserevs(out.split(b('\0'))[:-1])
    ...
    def log(self, revrange=None, files=[], follow=False,
            ...
            include=None, exclude=None, template=templates.changeset):
