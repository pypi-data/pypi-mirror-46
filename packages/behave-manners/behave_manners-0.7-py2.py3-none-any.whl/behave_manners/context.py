...
class CountCalls(object):
    """Context manager for counting some method calls
    """

    def __init__(self, rclass, rmethod, msg=None):
        assert issubclass(rclass, object)
        assert isinstance(rmethod, str)
        self.rclass = rclass
        self.rmethod = rmethod
        self._orig_method = None
        self._count = 0
        self._msg = msg or 'Count of {}.{} calls: %d'.format(rclass.__name__, rmethod)
    ...
    def __enter__(self):
        if self._orig_method is not None:
            raise RuntimeError('reentrancy')
        self._orig_method = getattr(self.rclass, self.rmethod)

        def __nmethod(*args, **kwargs):
            self._count += 1
    ...
    def __exit__(self, *args):
        if self._orig_method is not None:
            setattr(self.rclass, self.rmethod, self._orig_method)
            print(self._msg * self._count)
