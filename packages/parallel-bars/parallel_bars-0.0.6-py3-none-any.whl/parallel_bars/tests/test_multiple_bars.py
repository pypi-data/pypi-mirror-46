...
from parallel_bars.main import compute_with_multiple_bars
...
class TestSingleBar(TestCase):
    def test_computes_with_default_params(self):
        results = compute_with_multiple_bars(
            fun=fun,
            items=items,
        )
    ...
    def test_returns_results_in_order(self):
        self.assertEqual(
            squared_items,
            compute_with_multiple_bars(fun, items, in_order=True)
        )
