...
from .layers import HSwish, Squeeze, GlobalPooling, InplaceReLU, SELayer
...
class InvertedResidual(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel: int, stride: int, exp_channels: int,
        ...
        se_layer = functools.partial(SELayer, reduction=4, no_linear=HSwish) if se else None  # h-sigmoid in figure 4
...
class MobileNetV3(nn.Module):
    def __init__(self, feature_dim: int = 1000, width_multiple: float = 1.0):
        super().__init__()
        ...
        last_block = nn.Sequential(
            ...
            HSwish(),
            nn.Linear(last_channel, feature_dim),
        )
