    ...
    def calc_jobs(self, job_list, solver=None, show_progress=True):
        ...
        if self._local:
            for i, job in enumerate(job_list):
                ...
                if show_progress:
                    _print_progress(i + 1, len(job_list))
            return results
...
def _progress_bar(counter, num_total):
    ...
    while True:
        ...
        _print_progress(num_done, num_total)
        if num_done == num_total:
            return
        sleep(0.25)


def _print_progress(num_done, num_total):
    """
    Print a progress bar (with a preceding carriage return).
