...
class MsgRecverLightweight:
    ...
    def _resolve_conf(conf):
        ...
        assert isinstance(conf['bootstrap.servers'], str)
        assert isinstance(conf['group_id'], str)
        confluent_kafka_conf['bootstrap.servers'] = conf['bootstrap.servers']
        ...
        if 'security.protocal' in conf:
            confluent_kafka_conf['security.protocol'] = conf['SECURITY_PROTOCAL']
        if 'sasl.mechanisms' in conf:
            confluent_kafka_conf['sasl.mechanisms'] = conf['SASL_MECHANISM']
        if 'sasl.username' in conf:
            confluent_kafka_conf['sasl.username'] = conf['SASL_USERNAME']
        if 'sasl.password' in conf:
            confluent_kafka_conf['sasl.password'] = conf['SASL_PASSWORD']
