...
class MsgRecverLightweight:
    ...
    def _resolve_conf(conf):
        ...
        confluent_kafka_conf['bootstrap.servers'] = conf.pop('bootstrap.servers')
        confluent_kafka_conf['group.id'] = conf.pop('group_id')
        confluent_kafka_conf['auto.offset.reset'] = 'latest'
        confluent_kafka_conf['queue.buffering.max.ms'] = '50'
        confluent_kafka_conf['enable.auto.offset.store'] = 'false'
        for key in conf:
            confluent_kafka_conf[key] = conf.pop(key)
