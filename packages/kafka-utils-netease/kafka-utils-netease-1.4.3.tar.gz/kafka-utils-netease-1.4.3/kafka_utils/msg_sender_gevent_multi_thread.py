__all__ = ['MsgSenderGeventMultiThread']
...
class MsgSenderGeventMultiThread:
    def __init__(self, conf):
        ...
        self._buffer_mutex = Lck()
        self._buffer_not_empty_cond = Cond(
            self._buffer_mutex)
        self._flush_finished_cond = Cond(self._buffer_mutex)

        self._controller_thread = gevent.threadpool.ThreadPool(1)
        self._running = False
        self._thread_started = Evnt()
    ...
    def _start_controller_thread(self):
        self._running = True
        self._controller_thread.spawn(self._controller_task)
