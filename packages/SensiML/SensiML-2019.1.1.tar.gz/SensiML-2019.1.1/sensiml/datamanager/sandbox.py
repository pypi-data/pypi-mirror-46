...
class Sandbox(object):
    ...
    def auto(self, auto_params, seed, run_parallel=False):
        ...
        if seed.name == 'Custom':
            if self.pipeline.to_list()[-1]['type'] != 'tvo':
                print("Only custom pipelines that end with a tvo call can be automated.")
                return 0
            print("Running Auto Pipeline with Custom Seed:\n")
            ...
            print("Running Auto Pipeline with {} Seed:\n".format(seed.name))
            ...
            print("Running Auto Pipeline with {} Seed and {} Segmenter:\n".format(
                seed.name, segmenter_name))
            full_pipeline = deepcopy(self.pipeline.to_list()) + seed.pipeline
        else:
            print("This pipeline contains steps that could be overwritten by the {}\n".format(seed.name) +
    ...
    def retrieve(self, silent=False, page_index=0, **kwargs):
        ...
        if err is False:
            ...
            debug('sandbox_results_status:' + str(response_data))
