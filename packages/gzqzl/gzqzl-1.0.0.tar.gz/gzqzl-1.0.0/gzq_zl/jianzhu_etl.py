import json
from gzqpypi.gzq_zl import db_command, db_query

from gzqpypi.gzq_zl import web,page
...
def est_html_work(conp, f, **args):
    ...
    # print(arr[0])
    # print(type(arr[0]))
    setting = {"num": 20, "arr": arr, "f": f, "conp": conp, "tb": "jianzhu_gg_html", "headless": True}
...
def est_html_cdc(conp, f, **args):
    ...
    setting = {"num": 5, "arr": arr, "f": f, "conp": conp, "tb": "jianzhu_gg_html", "headless": True}
    if "num" in args.keys():
        setting["num"] = args["num"]
    setting = {**setting, **args}
...
# 获取企业信息入口
def jianzhu_est_html(conp, f, **args):
    sql = """select table_name from information_schema.tables where table_schema='%s'""" % (conp[4])
    df = db_query(sql, dbtype="postgresql", conp=conp)
    arr = df["table_name"].values
...
# 获取人员信息入口
def jianzhu_ryxx_est_html(conp, f, **args):
    sql = """select table_name from information_schema.tables where table_schema='%s'""" % (conp[4])
    df = db_query(sql, dbtype="postgresql", conp=conp)
    arr = df["table_name"].values

    if "jianzhu_ryxx_html" in arr:
        ryxx_html_cdc(conp, f, **args)
        removal_ryxx_html_data(conp)
    else:
        ryxx_html_work(conp, f, **args)
        removal_ryxx_html_data(conp)




def ryxx_html_work(conp, f, **args):
    if "size" in args.keys():
        size = args["size"]
    else:
        size = None
    ...
    # print(arr[0])
    # print(type(arr[0]))
    setting = {"num": 20, "arr": arr, "f": f, "conp": conp, "tb": "jianzhu_ryxx_html", "headless": True}
...
def ryxx_html_cdc(conp, f, **args):
    ...
    if (ft == None) or (ft == 'f7'):
        ...
        sql = """select href,zcry from %s.jianzhu_gg_html as a where not exists(select 1 from %s.jianzhu_ryxx_html as b where
         a.href=b.href and (ryxx_href,ryxx_nanme,sex,id_type,id_number,zyzcxx,grgcyj,blxw,lhxw,hmdjl,bgjl) is not null)""" % (conp[4], conp[4])
    ...
    setting = {"num": 5, "arr": arr, "f": f, "conp": conp, "tb": "jianzhu_ryxx_html", "headless": True}
    if "num" in args.keys():
        setting["num"] = args["num"]
    setting = {**setting, **args}
    # if len(arr) > 2000 and setting['num'] < 20: setting["num"] = 20
...
def removal_ryxx_html_data(conp):
    ...
    sql3 = """
        ...
        select ctid from
        (select row_number() over(partition by href,ryxx_href,ryxx_nanme order by ctid) as rn,ctid from %s.jianzhu_ryxx_html)as t where t.rn<>1)
    """% (schema, schema)
