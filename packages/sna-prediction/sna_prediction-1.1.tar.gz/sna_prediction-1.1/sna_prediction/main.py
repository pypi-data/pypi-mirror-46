...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        for names in MainWindow.files:
            temp =  readfromcsv(names)
            plotPieChart(temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6], names, temp[7],temp[8])
            # cloud(temp[8])
            ...
            MainWindow.H =  plottest(MainWindow.H, temp[9], names)
        
        #for i in range(len(MainWindow.npostweets)):
            #MainWindow.npostweets[i]= percentage(MainWindow.npostweets[i],MainWindow.ntotaltweets[i])
 
        barGraphP(MainWindow.pos,MainWindow.neg,MainWindow.nums)
