...
def install_pack_unzip(packfile, installdir):
    ...
    logging.debug("  Vendor: {0}, Name: {1}, Version: {2}".format(vendor, name, version))
    ...
    if "DFP" in install_path:
        flashalgos = [x.attrib['name'] for x in tree.findall('devices/family/device/algorithm')]
        # validate flash algos
        if flashalgos:
            for algo in flashalgos:
                algo_path = os.path.join(install_path, algo)
                if not os.path.exists(algo_path):
                    logging.critical('Validate FLM failed: Flash Algo "%s" is not exists!', algo_path)
                    return None
        else:
            logging.critical('Validate FLM failed: .pdsc not contains the Flash Algo information!')
            return None
