...
class PYOCD(DebuggerBase):
    ...
    def __init__(self, **kwargs):
        ...
        if self.version and StrictVersion(self.version) >= StrictVersion("0.13.0"):
    ...
    def erase(self):
        ...
        if StrictVersion(self.version) >= StrictVersion("0.13.0"):
            command = "pyocd erase --mass-erase -u {0} {1}".format(self._board.usbid, self._overried_target(self._board.devicename))
        else:
            command = "{0} --mass-erase -d error -b {1} {2}".format(self._flashtool, self._board.usbid, self._overried_target(self._board.devicename))
    ...
    def reset(self):
        ...
        command = "{0} reset -d error -b {1} {2}".format(self._pyocdtool, self._board.usbid, self._overried_target(self._board.devicename))
        return run_command(command)[0]
    ...
    def unlock(self):
        ...
        command = "{0} unlock -b {1} {2}".format(self._pyocdtool, self._board.usbid, self._overried_target(self._board.devicename))
        return run_command(command)[0]



    def get_gdbserver(self, **kwargs):
        """Return gdb server command.

        If configured board.devicename is a target name in pyocd.target.TARGET.
        This will overried the target.
        """
        ...
        if devicename:
            command += " " + self._overried_target(devicename)
    ...
    def flash(self, filepath, erase=False, addr=0):
        ...
        command += self._overried_target(self._board.devicename)
