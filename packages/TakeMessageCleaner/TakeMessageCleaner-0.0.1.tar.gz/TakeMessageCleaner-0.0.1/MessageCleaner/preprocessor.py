...
class PreProcessor:
        ...
        if use_placeholder == True:
            for key, value in self.enabled_processing_dict.items():
                 with multiprocessing.Pool() as pool:
                     self.data = pool.map(self.enabled_processing_dict[key], self.data)
        else:
            for key, value in self.enabled_processing_dict.items():
                with multiprocessing.Pool() as pool:
                     self.data = pool.map(self.enabled_processing_dict[key], self.data, args = ('',))
    ...
    def remove_accentuation(self, message: str):
    ...
    def correct_spelling(self, message: str):    
    ...
    def __set_pt_dictionary(self):
        self.__pt_dict = self.__read_dictionary('portuguese_errors.json')
        self.__pt_dict.update(self.__read_dictionary('abbreviations.json'))
