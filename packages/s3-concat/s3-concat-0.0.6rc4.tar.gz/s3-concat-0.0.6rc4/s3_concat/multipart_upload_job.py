...
class MultipartUploadJob:
    ...
    def _assemble_parts(self, s3):
        ...
        def get_small_parts(data):
            ...
            small_parts = []
            for p in part[1]:
                small_parts.append(s3.get_object(Bucket=self.bucket,
                                                 Key=p[0])['Body'].read().decode('utf-8'))
        ...
        data_to_thread = []
        for idx, data in enumerate(_chunk_by_size(local_parts, MIN_S3_SIZE * 2), start=1):
