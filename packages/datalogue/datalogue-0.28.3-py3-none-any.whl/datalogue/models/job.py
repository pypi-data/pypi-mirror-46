...
class Job:
    def __init__(self, stream_id: UUID, run_at: datetime, status: JobStatus, stream_collection_id: UUID, job_id: UUID, remaining_time_millis: int, percentage_progress: int, created_by: UUID, details: str):
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(job_id: {self.job_id!r}, stream_collection_id: {self.stream_collection_id!r}, status: {self.status!r}, '
        f'run_at: {self.run_at!r}, created_by: {self.created_by!r}, remaining_time_millis: {self.remaining_time_millis!r}, '
        f'percent_progress: {self.percentage_progress!r}, errors: {self.details!r})'
...
def _job_from_payload(json: dict) -> Union[DtlError, Job]:
    ...
    if remaining_time_millis is None:
        ...
        try:
            ...
            return DtlError("'remainingtimemillis' was not a proper int")
    ...
    if details is None:
        return DtlError("Job object should have a 'details' property")
    elif not isinstance(details, str):
        return DtlError("'details' was not a proper string")

    return Job(stream_id, run_at, status, stream_collection_id, job_id, remaining_time_millis, percentage_progress, created_by, details)
