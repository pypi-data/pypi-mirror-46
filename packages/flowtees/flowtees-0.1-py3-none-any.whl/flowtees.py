...
def refactor(contents):
    in_multiline_import = False
    for idx, line in enumerate(contents):
        # Check for flow decorators
        if '// @flow' in line:
...
def process(file_path, dry_run=False):
    try:
        ...
        contents = ''.join(refactor(contents))

        if not dry_run:
            f = open(file_path, 'w')
            f.write(contents)

            if tsx_keywords in contents:
...
def make_tsconfig_files(root_dir):
...
@click.command()
@click.argument('component_dir')
def main(component_dir):
