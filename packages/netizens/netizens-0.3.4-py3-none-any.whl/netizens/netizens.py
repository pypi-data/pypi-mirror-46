...
class Netizens(AbstractGrapheneChain):
    ...
    def invoice_pay(self, invoice, merchant, core_amount=0, bonus_amount=0, account=None, **kwargs):
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        amount_ntz = Amount(core_amount, "1.3.0", blockchain_instance=self)
        amount_ntl = Amount(bonus_amount, "1.3.1", blockchain_instance=self)

        customer = Account(account, blockchain_instance=self)
        merchant = Account(merchant, blockchain_instance=self)

        op = operations.Invoice_pay(
            **{
                "fee": {"amount": 0, "asset_id": "1.3.0"},
                "invoice": invoice,
                "customer": customer["id"],
                "merchant": merchant["id"],
                "core_amount": {"amount": int(amount_ntz), "asset_id": "1.3.0"},
                "bonus_amount": {"amount": int(amount_ntl), "asset_id": "1.3.1"},
            }
        )
        return self.finalizeOp(op, customer, "active", **kwargs)