                    ...
                    )
{
    ...
    for ( size_t z = distThreshUS; z <= volSizeZ1-1-distThreshUS; z++ )
    {
//         printf( "\r\tSet Voronoi progress: \t\t%02.1f%%\t", 100 * (float)(z-distThreshUS+1) / (float)(volSizeZ1-2*distThreshUS) );

        # pragma omp parallel
        # pragma omp for
        for ( size_t y = distThreshUS; y <= volSizeY1-1-distThreshUS; y++ )
        {
            for ( size_t x = distThreshUS; x <= volSizeX1-1-distThreshUS; x++ )
            {
