                    ...
                    )
{
    ...
    for ( size_t z = 1; z <= volSizeZ1-2; z++ )
    {
//         printf( "\r\tContacts labelling progress: \t\t%02.1f%%\t\n", 100 * (float)(z+1) / (float)volSizeZ1-1 );

        for ( size_t y = 1; y <= volSizeY1-2; y++ )
        {
            for ( size_t x = 1; x <= volSizeX1-2; x++ )
            {
                size_t index_i = z  * volSizeX1 * volSizeY1   +   y * volSizeX1   +   x;
                ...
                if ( pixelValue != 0 )
                {
                    ...
                    for ( short dz = -1; dz <= 1; dz=dz+2 )
                    {
                        for ( short dy = -1; dy <= 1; dy=dy+2 )
                        {
                            ...
                            {
//                                 printf( "%i %i %i\n", dz, dy, dx );
                                size_t index_j = (size_t)(z+dz)  * volSizeX1 * volSizeY1   +   (size_t)(y+dy) * volSizeX1   +   (size_t)(x+dx);
