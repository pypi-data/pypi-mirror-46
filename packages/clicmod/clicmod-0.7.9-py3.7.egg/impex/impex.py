...
def do_import(args):
    ...
    if not os.path.exists(directory):
        logger.error("Given directory does not exist! {}".format(directory))
        raise SystemExit(1)

    # if given a specific category add that to the root
    if args.category is not None:
        if not os.path.exists(os.path.join(directory, args.category)):
            logger.error("Given directory and category does not exist! {}".format(directory))
            raise SystemExit(1)

    logger.info('Importing automata from directory %s, category: %s', directory, args.category)

    directory_tree = get_directory_tree(directory, args.category)
