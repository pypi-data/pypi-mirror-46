    ...
    try:
        ...
        with logging_utilities.log_activity(logger,
            ...
            with automl_run_context.get_run() as run:
                ...
                # move the metrics uploading to the end, to avoid run fails but metrics appears in the ux.
...
def _explain_model_in_fit(child_run, pipeline, transformed_data_context, class_labels, logger):
    ...
    if training_utilities._is_sparse_metrix_int_type(transformed_data_context.X):
        logger.info("Integer type detected for X, need to upgrade to float type")
    if training_utilities._is_sparse_metrix_int_type(transformed_data_context.X_valid):
