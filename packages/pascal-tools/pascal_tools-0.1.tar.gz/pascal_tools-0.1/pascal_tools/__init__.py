...
def onehot_easy(pandas_df,dummy_before=False,dummy_in_place="."):
    ...
    for col in data.columns:
        if isinstance(data[[col]].values[0][0],str):
            ...
            dic_col=dict()        
            for i in prev:
                i_unicode=unicodedata.normalize('NFKD', unicode(i, "utf-8")).encode('ascii', 'replace')
            ...
            for i in prev:
                ...
                if dummy_before==False:
                    new_col.append(col.replace(dummy_in_place,".dummy:")+"."+i_unicode)
                else:
