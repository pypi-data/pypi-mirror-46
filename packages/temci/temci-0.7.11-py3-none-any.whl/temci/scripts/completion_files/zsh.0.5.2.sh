...
# Auto generated tab completion for the temci (0.5.2) benchmarking tool.
...
_temci(){
    ...
    subcommand)
        local -a subcommands
        subcommands=(
            "version:Print the current version (0.5.2)" "setup:Compile all needed binaries in the temci scripts folder" "short:Utility commands to ease working directly on the command line" "assembler:Wrapper around the gnu assembler to allow assembler randomization" "clean:Clean up the temporary files" "build:Build program blocks" "completion:Creates completion files for several shells." "init:Helper commands to initialize files (like settings)" "exec:Implements a simple run driver that just executes one of the passed run_cmds" "report:Generate a report from benchmarking result"
        )
	...
	{--other_nice,--no-other_nice}"[Enable:   Allows the setting of the nice value of most other processes (as far as possible).]"
	...
	--runner"[If not '' overrides the runner setting for each program block]: :('' 'perf_stat' 'rusage' 'spec' 'spec.py')"
	...
	{--stop_start,--no-stop_start}"[Enable:   Stop almost all other processes (as far as possible).]"
