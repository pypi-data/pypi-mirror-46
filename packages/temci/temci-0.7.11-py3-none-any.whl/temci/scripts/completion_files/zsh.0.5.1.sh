...
# Auto generated tab completion for the temci (0.5.1) benchmarking tool.
...
_temci(){
    ...
    subcommand)
        local -a subcommands
        subcommands=(
            "assembler:Wrapper around the gnu assembler to allow assembler randomization" "completion:Creates completion files for several shells." "version:Print the current version (0.5.1)" "clean:Clean up the temporary files" "init:Helper commands to initialize files (like settings)" "setup:Compile all needed binaries in the temci scripts folder" "exec:Implements a simple run driver that just executes one of the passed run_cmds" "report:Generate a report from benchmarking result" "build:Build program blocks" "short:Utility commands to ease working directly on the command line"
        )
