...
class Window_Main(QMainWindow):
    ...
    def show_output(self, isNewImg=True):
        '''input panel'''
        self.anchors = self.pn_main.get_anchors() if isNewImg else self.anchors
        # test
        # import json
        ...
        # test
        self.pn_main = Panel_Output(img_raw=self.img_crop, img_bin=self.img_bin, map=self.map, anchors=self.anchors)
        '''navigation bar'''
