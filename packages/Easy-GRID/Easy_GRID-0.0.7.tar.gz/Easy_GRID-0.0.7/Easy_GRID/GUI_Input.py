...
class Panel_Input(QWidget):
    ...
    def initUI(self):
        ...
        self.lb_img.setText("Image (.jpg, .tif):")
        self.lb_map.setText("Map (.csv, .txt):")
    ...
    def get_img(self):
        ...
        if self.gr_user.isChecked():
            ...
            try:
                ...
                img = cv2.imread(self.fd_img.text(), cv2.IMREAD_LOAD_GDAL)
                img[:,:,:3] = img[:,:,:3][:,:,[2,1,0]] # swap to rgb instead of bgr
            map = pd.read_csv(self.fd_map.text(), header=None)
        else:
            # demo
            # img = read_jpg("/Users/jameschen/Dropbox/James_Git/Alfalfa_gis/data/img&map/alfalfa_0426/demo.jpg")
            # map = pd.read_csv("/Users/jameschen/Dropbox/James_Git/Alfalfa_gis/data/img&map/alfalfa_0426/map1.csv", header=None)
