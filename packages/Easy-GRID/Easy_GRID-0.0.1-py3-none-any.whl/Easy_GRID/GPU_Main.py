...
class Window_Main(QMainWindow):
    ...
    def show_cropper(self, isNewImg=True):
        ...
        self.bt_prev.clicked.connect(self.show_input)
    ...
    def show_anchor(self, isNewImg=True):
        '''input panel'''
        self.img_crop, self.img_bin = self.pn_main.getImages() if isNewImg else (self.img_crop, self.img_bin)
        ...
        self.bt_next.clicked.connect(self.show_output)
        '''finalize'''
        self.assemble_and_show()
    def show_output(self):
        '''input panel'''
        anchors = self.pn_main.get_anchors()
        # test
        # import json
        # with open('anchors', 'w') as fout:
        #     json.dump(anchors, fout)
        # np.save("img_crop", self.img_crop)
        # np.save("img_bin", self.img_bin)
        # np.save("map", self.map)
        # test
        self.pn_main = Panel_Output(img_raw=self.img_crop, img_bin=self.img_bin, map=self.map, anchors=anchors)
        '''navigation bar'''
