...
class Panel_Kmeaner(QWidget):
    def __init__(self, np_img):
        ...
        self.bt_ccw = QPushButton("ccw")
        self.bt_cw = QPushButton("cw")
        # Pre K (right)
        ...
        self.bt_kmean = QPushButton("Run K-Means\nClustering")
        # Pro K (right)
        self.gr_pro = QGroupBox("Clusters Refine")
        self.lo_pro = QVBoxLayout()
        self.gr_cut = QGroupBox("Cutoff = 1")
        ...
        self.gr_veri = QGroupBox("Verify")
        self.lo_veri = QHBoxLayout()
        self.bt_vis = QPushButton("RGB")
        self.bt_k = QPushButton("K-Means")
        '''initialize UI'''
        self.initUI()
    def initUI(self):
        '''img preview (left)'''
        ...
        '''pre keans (right)'''
        # components
        self.bt_kmean.clicked.connect(self.do_kmeans)
        ...
        self.lo_pre.addWidget(self.bt_kmean)
        ...
        self.sl_shad.setTickInterval(5)
        ...
        self.bt_vis.pressed.connect(self.wg_img.switch_imgVis)
        self.bt_vis.released.connect(self.wg_img.switch_imgB)
        self.bt_k.pressed.connect(self.wg_img.switch_imgK)
        self.bt_k.released.connect(self.wg_img.switch_imgB)
        # layout
        ...
        self.lo_veri.addWidget(self.bt_vis)
        self.lo_veri.addWidget(self.bt_k)
        self.gr_veri.setLayout(self.lo_veri)
        self.lo_pro.addWidget(self.gr_cut)
        self.lo_pro.addWidget(self.gr_shad)
        self.lo_pro.addWidget(self.gr_gb)
        self.lo_pro.addWidget(self.gr_veri)
        self.gr_pro.setLayout(self.lo_pro)
        '''assemble'''
        self.gr_pro.setEnabled(False)
        ...
        self.layout.addWidget(self.gr_left, 0, 0, 2, 1)
        self.layout.addWidget(self.gr_pre, 0, 1)
        self.layout.addWidget(self.gr_pro, 1, 1)
    ...
    def change_cut(self):
        ...
        self.gr_cut.setTitle("Cutoff = %d" % value)
        # self.sl_gb.setValue(0)
        self.wg_img.set_binarize(value=value)
    ...
    def do_kmeans(self):
        ...
        self.enable_proK()
    def get_img(self):
        img_raw = self.wg_img.imgRaw
        img_k, center = get_kmean(img_raw,
            k=self.sl_k.value(),
            ch_NIR=int(self.cb_nir.currentText()),
            ch_Red=int(self.cb_red.currentText()))
        return img_raw, img_k, center
    def disable_proK(self):
        self.gr_pro.setEnabled(False)
        self.wg_img.token = False
    def enable_proK(self):
        self.sl_cut.setValue(1)
        self.sl_gb.setValue(0)
        self.gr_pro.setEnabled(True)
        self.wg_img.token = True
    def getImages(self):
...
class Widget_Kmeans(Widget_Img):
    def __init__(self, img):
        ...
        self.val_bin, self.val_bin_tp = 0, 0
    ...
    def do_kmeans(self, k=3, ch_NIR=1, ch_Red=0):
        ...
        self.set_binarize(value=1)
        self.switch_imgB()
    def set_binarize(self, value=1):
        self.val_bin_tp = value
        self.do_process()
    ...
    def set_smooth(self, value=0):
        ...
        self.do_process()
    def do_process(self):
        if self.val_bin!=self.val_bin_tp:
            self.val_bin = self.val_bin_tp
            self.do_binarize()
            self.do_smooth()
        # smooth
        if self.val_sm!=self.val_sm_tp:
            self.do_smooth()
        # deshadow
    ...
    def do_binarize(self):
        ...
        idx_select = rank_g[:self.val_bin]
    ...
    def mouseMoveEvent(self, event):
        pos = event.pos()
        if self.zoom!=0:
            magnifying_glass(self, pos, area=400, zoom=self.zoom*4, isCursor=True)
        else:
