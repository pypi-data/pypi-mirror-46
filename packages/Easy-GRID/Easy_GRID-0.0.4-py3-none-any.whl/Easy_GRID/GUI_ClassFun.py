...
def magnifying_glass(widget, pos, area=400, zoom=2, isCursor=True):
    size = int(area/zoom)
    pixmap_temp = widget.grab(QRect(pos.x()-int(size/2), pos.y()-int(size/2), size, size))
    pixmap = pixmap_temp.scaled(area, area)
    area /= 2
    ...
    painter.setPen(pen)
    painter.drawRect(0, 0, int(area), int(area))
    if isCursor:
        pen.setWidth(3)
        pen.setColor(Qt.red)
        painter.setPen(pen)
        size_m = 10
        pt_paint = int(area/2)
        painter.drawLine(pt_paint-size_m, pt_paint, pt_paint+size_m, pt_paint)
        painter.drawLine(pt_paint, pt_paint-size_m, pt_paint, pt_paint+size_m)
    painter.end()
    cursor = QCursor(pixmap)
    widget.setCursor(cursor)

def draw_cross(x, y, painter, size_mark=3):
