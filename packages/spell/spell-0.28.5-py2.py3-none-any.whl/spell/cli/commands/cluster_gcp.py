...
def create_gcp(ctx, name):
    ...
    with api_client_exception_handler():
        ...
        if owner_details.requestor_role not in ("admin", "manager"):
            raise ExitException(
                "You must be a Manager or Admin of org {} to proceed".format(ctx.obj["owner"]))
    ...
    network_name, subnet_name, region, zone = get_vpc(compute_service, name, project_id)
    ...
    with api_client_exception_handler():
        spell_client.create_gcp_cluster(name, service_account['email'], bucket_name,
                                        network_name, subnet_name, region, project_id, zone, gs_access_key_id,
...
def get_vpc(compute_service, cluster_name, project):
    ...
    # TODO(libeks): Use 'quotas' in response to warn user if insufficient
    region_url = request.execute()['selfLink']
    ...
    # TODO(libeks): Skip this step once the API cycles through all zones
    zones = compute_service.zones().list(project=project, filter='region = "{}"'.format(region_url)).execute()
    zone = click.prompt("Please select a zone to create your instances in",
                        type=click.Choice([zone['name'] for zone in zones['items']])
                        )
    return network_name, subnet_name, region, zone
