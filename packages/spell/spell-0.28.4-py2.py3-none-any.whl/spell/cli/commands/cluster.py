...
def gke_init(ctx, cluster_id, auth_api_url, gcp_project_name, gke_cluster_name,
    ...
    if gcp_project_name is None:
        try:
            ...
            return
        gcp_project_name = get_gcp_project(project, credentials)
...
def get_cluster(spell_client, owner, cluster_id):
    ...
    with api_client_exception_handler():
        ...
        if cluster_id is None:
            ...
            if len(clusters) > 1:
                raise ExitException("More than one cluster found for owner, please specify a Cluster ID")
            return clusters[0]
...
# TODO(Benno): duplicate of method from @libeks PR:
# https://github.com/spellrun/spell/pull/3614/files#diff-4994a3a227eb82f6afe2e0fd116f91c3R272
def get_gcp_project(project_id, credentials):
    ...
    if not click.confirm(
        ...
        for p in projects['projects']:
            click.echo("name: '{}', id: '{}'".format(p['name'], p['projectId']))
        project_id = click.prompt("Please choose a project id from the list above")
    for p in projects['projects']:
        if p['projectId'] == project_id:
            return p['projectId']
    raise ExitException("Unable to find project '{}'.".format(project_id))
