__name__ = 'mtrx'
__doc__ = 'https://mtrx.ir'
__author__ = 'Mostafa Vahedi Nejad'
__version__ = '0.0.9'
__license__ = 'MIT'


import pyqrcode, os, sys, time, smtplib, requests, dns.resolver, autopy


def qrcode(text, Format):
    """mtrx.qrcode(text='YourText', Format='ImageFormat')
    Create Your QRcode just with 1 line and 2 variable
    formats = 'svg', 'jpg', 'png', 'eps', 'terminal'
    """
    if Format == 'svg':
    	pyqrcode.create('%s'%text).svg('%s.%s'%(text, Format), scale=8)
    if Format == 'png':
    	pyqrcode.create('%s'%text).png('%s.%s'%(text, Format), scale=8)
    if Format == 'jpg':
    	pyqrcode.create('%s'%text).jpg('%s.%s'%(text, Format), scale=8)
    if Format == 'eps':
	    pyqrcode.create('%s'%text).eps('%s.%s'%(text, Format), scale=2)
    if Format == 'terminal':
        pyqrcode.create('%s'%text).terminal(quiet_zone=1)
...
def screensh(name):
    """mtrx.screensh(name='ImageName')
    get screenshot to easy"""
    autopy.bitmap.capture_screen().save('%s.png'%name)

def download(url, name):
    """mtrx.download(url='downloadLink', name='fileName')
    download any file on one line and 2 args"""
