...
class Interface(Processor):
    ...
    def afni_TTest(self, input_a, regex_a, data_idx_a=1, output_filename=None, mask_path=None,
        ...
        if input_b is not None:
            itf.set_input(label='groupB', input_path=input_b, method=1,
                          join_modifier=dict(suffix="'[{}]'".format(data_idx_b)),
        ...
        itf.set_cmd('3dttest++ -mask *[mask] -prefix *[output] {} '
                    '-resid *[resid] -ACF {}'.format(input_sets, option))
        itf.check_output()
        itf.run()
    ...
    def afni_MultiVarModeling(self, input_path, regex, regex_label, subbrick_idx=1, mask_path=None,
        ...
        cped_regex_label = copy.copy(regex_label) #prevent modigying original input
        df_dict = OrderedDict()
        subj_idx = cped_regex_label.pop('Subj')
        df_dict['Subj'] = [p.search(f).group(subj_idx) for f in inputs]

        for label, idx in cped_regex_label.items():
