...
class Subscription:
    ...
    def update(self, deployment_uids=None, problem_type=None, input_data_type=None,
        ...
        :param deployment_uids:  update existing deployment with new one
        ...
        if deployment_uids is not None:
            if type(deployment_uids) is str:
                deployment_uids = [deployment_uids]
            if len(deployment_uids) == 1:
                artifact = self._binding_client.get_artifact(self.source_uid, deployment_uids=deployment_uids)
            else:
                raise IncorrectValue('deployment_uids',
                                     'Subscription can be created for one deployment only. ' + str(len(deployment_uids))
