...
class CustomMetricsViewer(TableFromRestApiViewer):
    ...
    def show_confusion_matrix(self, measurement_id, monitor_uid=None):
        ...
        if len(confusion_matrix_list) > 0:
            ...
            rows = [[confusion_matrix['labels'][i]] + row + [confusion_matrix['recall_per_label'][i], confusion_matrix['precision_per_label'][i]] for i, row in enumerate(confusion_matrix['values'])]
