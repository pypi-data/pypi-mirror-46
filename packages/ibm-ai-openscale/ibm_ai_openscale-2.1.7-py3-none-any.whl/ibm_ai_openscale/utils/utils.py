...
def get_paged_resource(method_func, url, headers, limit, initial_result, validate, merge, get_number_of_elements, join_character='?', default_limit=1000):
    ...
    while True:
        response = method_func(
            url + "{}limit={}&offset={}".format(join_character, min(default_limit, limit - elements_no), counter*default_limit),
            headers=headers
        )
        ...
        if get_number_of_elements(response) < default_limit or elements_no == limit:
            return result
