...
from django.utils.deprecation import \
    MiddlewareMixin  # https://stackoverflow.com/questions/42232606/django-exception-middleware-typeerror-object-takes-no-parameters
...
from .utils import (get_sso_id, get_profile_model, get_profile_from_sso,
...
class SsoMiddleware(MiddlewareMixin):
    ...
    header = settings.SSO_HEADER
    # Username for anonymous user
    anonymous_username = getattr(settings, "SSO_ANONYMOUS_USERNAME",
                                 "anonymous")
    ...
    def process_request(self, request):
        ...
        if request.user.is_authenticated and request.user.is_staff:  # django >= 1.10 has is_authenticated as parameter
        ...
        if request.user.is_authenticated:
            ...
            if settings.SSO_USER_PROFILE_MODEL is not None:
                user_profile_model = get_profile_model()
                user_profile = user_profile_model.objects.get(user=request.user)
        ...
        if user and is_to_subscribe:
            _qs = urlencode({"next": settings.SSO_SERVICE_URL})
            _url = "{sso}?{qs}".format(sso=settings.SSO_LOGIN_URL, qs=_qs)
