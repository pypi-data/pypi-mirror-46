...
from Pyautomators.formatter.base import Formatter
import base64
import six
import copy
try:
...
class CucumberJSONFormatter(Formatter):
    name = 'json_cucumber'
    description = 'JSON dump of test run equals the cucumber reports'
    dumps_kwargs = {}
    ...
    def result(self, result):
        if result.embeddings:
            if type(result.embeddings)== list: 
                for embedding in result.embeddings:
                    self.current_step["embeddings"].append({'mime_type':embedding['mime_type'],"data":embedding['data']})
            else: 
                Error="embeddings is not list"
                raise Exception(Error)
        else: 
            self.current_step["embeddings"]=[]

        self.current_step['result'] = {
            'status': self.status_replace(result.status),
    ...
    def status_replace(self,status):
        return str(status).replace("<Status","").replace(":","").replace(' ',"")\
                .replace("1>","").replace("2>","").replace("3>","").replace("4>","").replace("Status.","")

    def embedding(self, mime_type, data):
        step = self.current_feature_element['steps'][-1]
        step['embedding'].append({
            'mime_type': mime_type,
    ...
    def update_status_data(self):
        ...
        self.current_feature_data['status'] = self.status_replace(self.current_feature.status)
    ...
    def write_json_feature(self, feature_data):
        feature_new=str(feature_data)
        self.stream.write(json.dumps(ast.literal_eval(feature_new), **self.dumps_kwargs))
        self.stream.flush()
...
class PrettyCucumberJSONFormatter(CucumberJSONFormatter):
    ...
    """
    name = 'json_cucumber.pretty'
    description = 'JSON dump of test run equals the cucumber reports (human readable)'
