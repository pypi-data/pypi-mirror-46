# -*- coding: utf-8 -*-
"""
This module provides `JSON`_ formatters for :mod:`Pyautomators`:
...
class JSONFormatter(Formatter):
    ...
    def result(self, step):
        ...
        if step.error_message and step.status == Status.failed:
            ...
            if(str(error_message).find("Assert")==-1):
               steps[self._step_index]["result"]['status']=str(steps[self._step_index]["result"]['status']).replace('failed',"error")
