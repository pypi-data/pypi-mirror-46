...
def test_dummy_nodes_with_only_one_per_layer():
    G = LayeredGraph()
    G.add_node('a1', node=ProcessGroup())
    G.add_node('a2', node=ProcessGroup())
    G.add_node('b', node=ProcessGroup())
    G.ordering = Ordering([[['a1', 'a2']], [[]], [['b']]])

    G = add_dummy_nodes(G, 'a1', 'b', bundle_key=1, dummy_key='')
    G = add_dummy_nodes(G, 'a2', 'b', bundle_key=2, dummy_key='')

    assert set(G.nodes()) == {'a1', 'a2', 'b', '___1'}
    assert set(G.edges()) == {('a1', '___1'), ('a2', '___1'), ('___1', 'b')}
    assert G.ordering == Ordering([[['a1', 'a2']], [['___1']], [['b']]])
