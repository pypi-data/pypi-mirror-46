...
def check_id_existence(form, field):
    ...
    """
    try:
        hawat.db.db_get().session.query(GuiUserModel).\
            filter(GuiUserModel.login == field.data).\
            one()
...
def check_id_uniqueness(form, field):
    ...
    """
    user = hawat.db.db_get().session.query(GuiUserModel).\
        filter(GuiUserModel.login == field.data).\
        filter(GuiUserModel.id != form.db_item_id).\
        all()
    if not user:
        return
    raise wtforms.validators.ValidationError(gettext('User account with this login already exists.'))
