...
from mentat.reports.utils import SimpleMemoryThresholdingCache
...
def truncate_stats_with_mask(stats, mask, top_threshold = 20):
    """
    Make statistics more brief. For each of the statistical aggregation subkeys
    ...
    :param dict mash: Global truncated statistics to serve as a mask.
    :param int top_threshold: Toplist threshold size.
    :return: Updated structure containing statistics.
    :rtype: dict
    """
    ...
    if stats.get(ST_SKEY_CNT_ALERTS, 0) > 0 or stats.get(ST_SKEY_CNT_EVENTS, 0) > 0:
        for key in LIST_CALCSTAT_KEYS:
            if key in stats:
                stats = _mask_toplist(stats, mask, key, top_threshold)
...
def aggregate_timeline_groups(stats_list, dt_from, dt_to, max_count, min_step = None, result = None):
    ...
    for grp_key in LIST_STAT_GROUPS:
        ...
        for stat in stats_list:
            ...
            tl_key_idx = bisect.bisect_left(tl_keys, stat_dt) - 1
            if tl_key_idx < 0:
                raise ValueError('Statistical record does not fit into timeline with start time {}'.format(str(stat_dt)))
            _merge_stats(
                getattr(stat, grp_key),
...
def _include_event_to_stats(stats, event, recurring = False):
    """
...
def _mask_toplist(stats, mask, dict_key, top_threshold):
    """
