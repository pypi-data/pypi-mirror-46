...
// Prepare dataset for visualisations from given data series list.
function get_dataset_list(raw_data, data_series, key_attr = 'key', val_attr = 'value') {
    return data_series.map(function(serie) {
        return {
            ident: serie.ident,
            key: 'key' in serie ? serie.key : serie.ident,
            value: serie.ident in raw_data ? raw_data[serie.ident] : 0
        };
    }).sort(function(a, b) {
        return b.value < a.value ? -1 : b.value > a.value ? 1 : b.value >= a.value ? 0 : NaN;
    });
