...
import patgroom.broker
from patgroom.broker import backtesting
from patgroom import observer
from patgroom import dispatcher
import patgroom.strategy.position
from patgroom import logger
from patgroom.barfeed import resampled
...
class BaseStrategy(object):
    ...
    def marketOrder(self, instrument, quantity, onClose=False, goodTillCanceled=False, allOrNone=False):
        ...
        if quantity > 0:
            ...
            ret = self.getBroker().createMarketOrder(patgroom.broker.Order.Action.BUY, instrument, quantity, onClose)
        elif quantity < 0:
            # ret = self.getBroker().createMarketOrder(pyalgotrade.broker.Order.Action.SELL, instrument, quantity*-1, onClose)
            ret = self.getBroker().createMarketOrder(patgroom.broker.Order.Action.SELL, instrument, quantity * -1,
                                                     onClose)
        if ret:
    ...
    def limitOrder(self, instrument, limitPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        if quantity > 0:
            ...
            ret = self.getBroker().createLimitOrder(patgroom.broker.Order.Action.BUY, instrument, limitPrice,
                                                    quantity)
        elif quantity < 0:
            # ret = self.getBroker().createLimitOrder(pyalgotrade.broker.Order.Action.SELL, instrument, limitPrice, quantity*-1)
            ret = self.getBroker().createLimitOrder(patgroom.broker.Order.Action.SELL, instrument, limitPrice,
                                                    quantity * -1)
        if ret:
            ret.setGoodTillCanceled(goodTillCanceled)
            ret.setAllOrNone(allOrNone)
    ...
    def stopOrder(self, instrument, stopPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        if quantity > 0:
            ...
            ret = self.getBroker().createStopOrder(patgroom.broker.Order.Action.BUY, instrument, stopPrice, quantity)
        elif quantity < 0:
            # ret = self.getBroker().createStopOrder(pyalgotrade.broker.Order.Action.SELL, instrument, stopPrice, quantity*-1)
            ret = self.getBroker().createStopOrder(patgroom.broker.Order.Action.SELL, instrument, stopPrice,
                                                   quantity * -1)
        if ret:
            ret.setGoodTillCanceled(goodTillCanceled)
            ret.setAllOrNone(allOrNone)
    ...
    def stopLimitOrder(self, instrument, stopPrice, limitPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        if quantity > 0:
            ...
            ret = self.getBroker().createStopLimitOrder(patgroom.broker.Order.Action.BUY, instrument, stopPrice,
                                                        limitPrice, quantity)
        elif quantity < 0:
            # ret = self.getBroker().createStopLimitOrder(pyalgotrade.broker.Order.Action.SELL, instrument, stopPrice, limitPrice, quantity*-1)
            ret = self.getBroker().createStopLimitOrder(patgroom.broker.Order.Action.SELL, instrument, stopPrice,
    ...
    def enterLong(self, instrument, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.LongPosition(self, instrument, None, None, quantity, goodTillCanceled,
                                                      allOrNone)
    ...
    def enterShort(self, instrument, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.ShortPosition(self, instrument, None, None, quantity, goodTillCanceled, allOrNone)
    ...
    def enterLongLimit(self, instrument, limitPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.LongPosition(self, instrument, None, limitPrice, quantity, goodTillCanceled, allOrNone)
    ...
    def enterShortLimit(self, instrument, limitPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.ShortPosition(self, instrument, None, limitPrice, quantity, goodTillCanceled, allOrNone)
    ...
    def enterLongStop(self, instrument, stopPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.LongPosition(self, instrument, stopPrice, None, quantity, goodTillCanceled, allOrNone)
    ...
    def enterShortStop(self, instrument, stopPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.ShortPosition(self, instrument, stopPrice, None, quantity, goodTillCanceled, allOrNone)
    ...
    def enterLongStopLimit(self, instrument, stopPrice, limitPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.LongPosition(self, instrument, stopPrice, limitPrice, quantity, goodTillCanceled, allOrNone)
    ...
    def enterShortStopLimit(self, instrument, stopPrice, limitPrice, quantity, goodTillCanceled=False, allOrNone=False):
        ...
        return patgroom.strategy.position.ShortPosition(self, instrument, stopPrice, limitPrice, quantity, goodTillCanceled, allOrNone)
...
class BacktestingStrategy(BaseStrategy):
    ...
    def __init__(self, barFeed, cash_or_brk=1000000):
        ...
        if isinstance(cash_or_brk, patgroom.broker.Broker):
            broker = cash_or_brk
        else:
