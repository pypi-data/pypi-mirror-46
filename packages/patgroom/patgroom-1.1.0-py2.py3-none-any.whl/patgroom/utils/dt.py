...
# groom
def localtime_now():
    return as_utc(datetime.datetime.now(localTz))

def time_now():
    return localize(datetime.datetime.now(localTz),localTz)
# groom
def get_current_datatime(frequency):
    # 程序运行初始化获取数据时间-通过build_range函数根据当前时间获取本周期数据时间
    ret = localize(resamplebase.build_range(localtime_now(), frequency).getEnding(),
                                        localTz)-datetime.timedelta(seconds=frequency)
    return ret

def get_next_datatime(frequency):
    # 程序运行初始化获取数据时间-通过build_range函数根据当前时间获取本周期数据时间
    ret = localize(resamplebase.build_range(localtime_now(), frequency).getEnding(),
                                        localTz)
    return ret
