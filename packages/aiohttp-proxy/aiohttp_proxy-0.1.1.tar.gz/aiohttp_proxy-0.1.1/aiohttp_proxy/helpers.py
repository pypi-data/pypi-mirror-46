...
from .proto import Socks4SocketWrapper, Socks5SocketWrapper, ProxyType


def create_socket_wrapper(loop, proxy_type, host=None, port=None,
...
async def open_connection(socks_url=None, host=None, port=None, *,
    ...
    if socks_url is not None:
        proxy_type, socks_host, socks_port, username, password \
            = parse_proxy_url(socks_url)
    ...
    return await asyncio.open_connection(
        loop=loop, sock=sock.socket, **kwargs)
