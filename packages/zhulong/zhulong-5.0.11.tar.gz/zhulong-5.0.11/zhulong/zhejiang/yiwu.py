...
def f1(driver, num):
    ...
    if num != int(cnum):
        val = driver.find_element_by_xpath("//ul[@class='ewb-nbd-items']/li[1]/a").get_attribute('href')[-30:]
        ...
        driver.get(url)
        try:
            locator = (By.XPATH, "//ul[@class='ewb-nbd-items']/li[1]/a[not(contains(@href, '%s'))]" % val)
            WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
        except:
            driver.refresh()
            locator = (By.XPATH, "//ul[@class='ewb-nbd-items']/li[1]/a[not(contains(@href, '%s'))]" % val)
