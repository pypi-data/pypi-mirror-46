...
def f2(driver):
    ...
    try:
        if "--proxy" in driver_info['goog:chromeOptions']['args'][0]:
            ...
            total_page = re.findall('<.*?Page_TotalPage.*?(\d+).*?>', requests.post(URL,proxies=proxies, headers=headers, data=data1).text)[0]
        else:
            total_page = re.findall('<.*?Page_TotalPage.*?(\d+).*?>',requests.post(URL,headers=headers,data=data1).text)[0]
    except:
        total_page = re.findall('<.*?Page_TotalPage.*?(\d+).*?>', requests.post(URL, headers=headers, data=data1).text)[0]
...
if __name__ == '__main__':
    ...
    conp = ["postgres", "since2015", "192.168.4.175", "hunan", "changsha"]
    work(conp)
