...
from .helps import Help, help as help_
...
__all__ = ["ANBOT","ExitCodes"]

def _is_submodule(parent, child):
    return parent == child or child.startswith(parent + ".")


class ANBOTBase(commands.GroupMixin, commands.bot.BotBase, RPCMixin):
    """Mixin for the main bot class.

    This exists because `ANBOT` inherits from `discord.AutoShardedClient`, which
    ...
    def enable_sentry(self):
        """Enable Sentry logging for ANBOT."""
    ...
    async def c_run(self, ctx, *, call_once=False):
    ...
    async def invoke_s(self, ctx):
        ...
        if ctx.command is not None:
            ...
            try:
                if await self.c_run(ctx, call_once=True):
    ...
    async def g_context(self, message, *, cls=commands.context.Context):
        r"""|coro|
        ...
        ctx.command = self.all_commands.get(invoker.lower())
        return ctx

    async def p_commands(self, message):
        """|coro|
        ...
        ctx = await self.g_context(message)
        await self.invoke_s(ctx)

    def disable_sentry(self):
        """Disable Sentry logging for ANBOT."""
        if self._sentry_mgr is None:
            return
        ...
        for attr in dir(cog):
            ...
            if isinstance(_attr, discord.ext.commands.Command) and not isinstance(
                ...
                raise RuntimeError(
                    ...
                    " is not using ANBOT's command module, and cannot be added. "
...
class ANBOT(ANBOTBase, discord.AutoShardedClient):
    """
