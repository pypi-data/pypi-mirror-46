...
from .main.manage import create_temp_config, load_basic_configuration, config_file
from .json_io import JsonIO
from .global_checks import init_global_checks
from .events import init_events
from .cli import interactive_config, confirm, parse_cli_flags, ask_sentry
from .feature.developer import Developer
import asyncio
...
def init_loggers(cli_flags):
    ...
    from .main.manage import core_data_path
...
def main():
    ...
    red = ANBOT(cli_flags=cli_flags, description=description, pm_help=None)
    init_global_checks(red)
    init_events(red, cli_flags)
    loop = asyncio.get_event_loop()
    tmp_data = {}
    loop.run_until_complete(_get_prefix_and_token(red, tmp_data))
    token = os.environ.get("ANBOT_TOKEN", tmp_data["token"])
    if cli_flags.token:
