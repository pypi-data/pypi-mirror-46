...
def interactive_config(red, token_set, prefix_set):
    ...
    print("ANBOT - Discord Bot | Configuration process\n")
...
def ask_sentry(red: ANBOT):
    loop = asyncio.get_event_loop()
    print(
        "\nThank you for installing ANBOT V3! ANBOT is constantly undergoing\n"
...
def parse_cli_flags(args):
    parser = argparse.ArgumentParser(
        description="ANBOT - Discord Bot", usage="ANBOT <instance_name> [arguments]"
    )
    parser.add_argument("--version", "-V", action="store_true", help="Show ANBOT's current version")
    parser.add_argument(
        "--list-instances",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "ANBOT should be owner, this has "
    ...
    parser.add_argument(
        ...
        "to the system that is hosting ANBOT should be  "
    ...
    parser.add_argument(
        "--no-cogs", action="store_true", help="Starts ANBOT with no cogs loaded, only core"
    )
    parser.add_argument(
        "--load-cogs",
        type=str,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Makes ANBOT quit with code 0 just before the "
    ...
    parser.add_argument("--token", type=str, help="Run ANBOT with the given token.")
    parser.add_argument(
        "--no-instance",
        action="store_true",
        help=(
            "Run ANBOT without any existing instance. "
