...
from ..main.manage import cog_data_path
from ..utils.chat_formatting import box, pagify, humanize_list, inline
from ..utils.menus import start_adding_reactions
...
class Downloader:
    ...
    async def pipinstall(self, ctx, *deps: str):
        ...
        if success:
            await ctx.send("Libraries installed.")
        else:
            await ctx.send(
    ...
    async def _repo_add(self, ctx, name: str, repo_url: str, branch: str = None):
        ...
        try:
            ...
            await ctx.send("Repo `{name}` successfully added.".format(name=name))
    ...
    async def _repo_del(self, ctx, repo: Repo):
        ...
        await ctx.send(
            "The repo `{repo.name}` has been deleted successfully.".format(repo=repo)
        )
    ...
    async def _repo_info(self, ctx, repo: Repo):
        ...
        if repo is None:
            await ctx.send("Repo `{repo.name}` not found.".format(repo=repo))
            return

        msg = "Information on {repo.name}:\n{description}".format(
    ...
    async def _cog_install(self, ctx, repo: Repo, cog_name: str):
        ...
        if cog is None:
            await ctx.send(
                "Error: there is no cog by the name of `{cog_name}` in the `{repo.name}` repo.".format(cog_name=cog_name, repo=repo)
            )
            return
        ...
        if not await repo.install_requirements(cog, self.LIB_PATH):
            await ctx.send(
                "Failed to install the required libraries for `{cog_name}`: `{libraries}`".format(cog_name=cog.name, libraries=cog.requirements)
            )
            return
        ...
        await ctx.send("Cog `{cog_name}` successfully installed.".format(cog_name=cog_name))
    ...
    async def _cog_uninstall(self, ctx, cog: InstalledCog):
        ...
        if poss_installed_path.exists():
            ...
            await ctx.send(
                "Cog `{cog_name}` was successfully uninstalled.".format(cog_name=real_name)
            )
        else:
            await ctx.send(
                    ...
                    " with `{prefix}unload {cog_name}`.".format(prefix=ctx.prefix, cog_name=real_name)
            )
    ...
    async def _cog_update(self, ctx, cog_name: InstalledCog = None):
        ...
        async with ctx.typing():
            ...
            if installed_and_updated:
                ...
                await ctx.send("All installed cogs are already up to date.")
                return
        ...
        message = "Would you like to reload the updated cogs?"
    ...
    async def _cog_info(self, ctx, repo: Repo, cog_name: str):
        ...
        if cog is None:
            await ctx.send(
                "There is no cog `{cog_name}` in the repo `{repo.name}`".format(
                    cog_name=cog_name, repo=repo
                )
            )
            return

        msg = "Information on {cog_name}:\n{description}\n\nRequirements: {requirements}".format(
            cog_name=cog.name,
    ...
    def format_findcog_info(
        ...
        if isinstance(cog_installable, Installable):
            ...
            repo_url = "Missing from installed repos" if repo is None else repo.url
            cog_name = cog_installable.name
        else:
            made_by = "ANBOT"
            repo_url = "https://github.com/anbot-devl"
