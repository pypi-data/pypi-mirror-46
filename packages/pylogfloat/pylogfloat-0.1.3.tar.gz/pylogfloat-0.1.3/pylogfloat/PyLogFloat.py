...
class PyLogFloat(object):
    ...
    def __repr__(self):
        ...
        with warnings.catch_warnings():
            ...
            return "PyLogFloat(log_space = {:e}, log_sign = {}; linear_space = {:e})".format(self.p if self.sign != 0 else 0, self.sign, self.__to_float_-())
    ...
    def __addaslogs__(self, q):
        ...
        if self.p > q.p:
            self.p += np.__log1mexp__(np.exp(q.p-self.p))
        else:
            self.p = q.p + np.__log1mexp__(np.exp(self.p - q.p))
        assert np.isfinite(self.p), "PyLogFloat: __addaslogs__ result is not finite (i.e., it's nan or infinite) for\n\t{}\n\t{}".format(self, q)
        return
        
    def __subaslogs_-(self, q):
        assert isinstance(q, PyLogFloat), "PyLogFloat: Argument to add must be a PyLogFloat"
        if self.p > q.p:
...
#            self.p = q.p + np.log1p(- np.exp(self.p - q.p))
    ...
    ## Conditional use of log1p or exp1m for log value additions and
    ## subtractions, following Martin, M. Accurately Computing log(1 −
    ## exp(− |a|)) Assessed by the Rmpfr package Cran, The
    ## Comprehensive R Archive Network.
    def __log1mexp_-(self, a):
        if a > np.log(2):
