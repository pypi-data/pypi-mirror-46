...
class OneiroiHandler:
    ...
    NodeConnections={}
    AutoReConnect=False
    messageText = """<?xml version="1.0" encoding="utf-16"?><testMessage xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" name="TestMessage" sendFrom="NodeA" sendTo="NodeA" sourceClient="capp" hide=""><test>MessagetOSend</test></testMessage>"""
    oneiroi=None
    ...
    def create_Node_Implementation(self,NodeName): 
        ...
        if currentNodeDefinition:
            ...
            f.write("from becos.becosOneiroi import oneiroiReceiver\n")
            ...
            f.write("\texcAttributes=['original_tagname_','name','sendFrom','sendTo','sourceClient','hide','ttargetClient','sourceClient','targetClient']\n")
            ...
            for messagebinding in currentNodeDefinition['allMessageBindings']:
                if messagebinding['MessageFlow'].lower()=='inbound':
                    ...
                    f.write(f"\t\tmessage_received.sourceClient=root.attrib['sourceClient']\n")
...
\t\txmlMessage= xmlMessage +f"&%!&%$<{inputMessage.name}&%$name='{inputMessage.name}'&%$sourceClient='{inputMessage.sourceClient}'&%$sendFrom='{inputMessage.sendFrom}'&%$sendTo='{inputMessage.sendTo}'&%$hide=''&%$targetClient='{inputMessage.targetClient}'>"
