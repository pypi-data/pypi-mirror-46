        ...
        if self.normals:
            ...
            if np.any(magnitudes == 0):
                ...
                for index in zero_indices:
                    if self.indices:
                        index_in_indices = np.where(self.indices.data == index)[0][0]
                        mod = index_in_indices % 3
                        a = self.positions.data[index_in_indices - mod]
                        b = self.positions.data[1 + index_in_indices - mod]
                        c = self.positions.data[2 + index_in_indices - mod]
                    else:
                        mod = index % 3
