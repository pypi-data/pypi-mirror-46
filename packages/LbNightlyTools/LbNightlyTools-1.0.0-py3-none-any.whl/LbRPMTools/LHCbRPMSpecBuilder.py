...
""").substitute(buildarea = self._buildarea,
    ...
    def _createInstall(self):
        ...
        if self._sharedTar != None:
            ...
            spec += "cd  ${RPM_BUILD_ROOT}/opt/LHCbSoft/lhcb/ && tar jxf %s --strip-components=1 -C %%{projectUp}/%%{projectUp}_%%{lbversion}" % self._sharedTar
        else:
            spec += "cd  ${RPM_BUILD_ROOT}/opt/LHCbSoft/lhcb/%%{projectUp}/%%{projectUp}_%%{lbversion} && getpack --no-eclipse-config --no-config -P -r % s %s && mv %%{project}/ ./ && rm %%{project}" % (self._project, self._version)
...
\n""").substitute(buildarea = self._buildarea,
    ...
    def _createInstall(self):
        ...
        spec += "rsync -arL %{buildlocation}/%{project}/InstallArea/%{cmtconfig} ${RPM_BUILD_ROOT}/opt/LHCbSoft/lhcb/%{projectUp}/%{projectUp}_%{lbversion}/InstallArea/\n"
        spec += "\n\n"
        return spec
...
""").substitute(buildarea = self._buildarea,
    ...
    def _createInstall(self):
        ...
        spec = "%install\n"
        spec += "mkdir -p ${RPM_BUILD_ROOT}/opt/LHCbSoft/lhcb/%{projectUp}/%{projectUp}_%{lbversion}\n"
        spec += "cd  ${RPM_BUILD_ROOT}/opt/LHCbSoft/lhcb/ && tar jxf %s --strip-components=1 -C %%{projectUp}/%%{projectUp}_%%{lbversion}\n" % self._sharedTar
...
def getBuildInfo(manifestFileName):
    ...
    if len(splitPath) < 4 or splitPath[-3] != 'InstallArea':
        ...
        # The version is not anymore in the path
        return(realFilename, barea , None, splitPath[-2])
