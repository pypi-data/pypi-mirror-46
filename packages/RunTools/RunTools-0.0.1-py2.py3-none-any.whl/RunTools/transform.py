...
Notes:
    1. python中nan十分神奇，
        a. set([float("nan"), float("nan")]) -> {nan, nan}
        b. set([np.nan, np.nan]) -> {nan}
        pd.dataframe中的nan不管是哪种，set(df.x)都会视为是情形a，df.x.unique()都会视为是情形b
    2.
        1. {key: y, ...} 注意：如果key中有重复项或NaN的话，后面的y值可能会覆盖前面的y值
    ...
    3. 版本更新：y可以是多个字段组成的tuple或者list，如 (y1, y2) 或者 [y1, y2, y3] todo
    4. 关于warnings.warn()的使用有些疑问, 在console中重复运行同一句代码，只有第一次会输出相应警告，后续不再会重复输出。考虑使用logging模块进行替代 todo
"""
...
class Transformer:
    ...
    def df2dict(cls, df, format_str=None, keys=None, value=None, func=None, dropna=False,
        ...
        e.g.1. {(a1, a2, a3): {b: {(c1, c2): set(d), ...}, ...}, ...}
            第一种传参方式：df2dict(df, "{(a1, a2, a3): {b: {(c1, c2): set(d), ...}, ...}, ...}")
            第二种传参方式：df2dict(df, keys=[('a1', 'a2', 'a3'), 'b', ('c1', 'c2')], value='d', func='set')
        :param df:
        ...
        if format_str is not None:  # 第一种传参方式
            assert isinstance(format_str, str)
            if ':' not in format_str:
                raise Exception("too simple, please do this directly")
            format_str = format_str.strip()
            count = check_parentheses(format_str)  # 嵌套的层级
            keys, value, func = parse_format_str(format_str, count, cols_set)
        else:  # 第二种传参方式
            count = len(keys)

        if store_info:  # todo merge to bottom
            format_str = gen_format_str(keys, value, func)
        ...
        if count == 1:  # 一层嵌套
            keys = keys[0]
            if func is None:  # 1_1
                res = match_1_1(df, df_len, keys, value, with_warnings, print_codes)
            else:
                if isinstance(func, str):  # single function
                    if func == 'set':  # 1_2
                        res = match_1_2(df, keys, value, with_warnings, print_codes)
                    elif func == 'list':  # 1_3
                        res = match_1_3(df, keys, value, with_warnings, print_codes)
                    elif func == 'count':  # 1_4
                        res = match_1_4(df, keys, value, dropna, with_warnings, print_codes)
                    elif func in {'unique', 'nunique', 'max', 'min', 'median', 'sum', 'mean'}:
                        res = match_1_common(df, keys, value, func, dropna, with_warnings, print_codes)
                    else:
                        res = None
                    ...
                    if func == ['len', 'set']:
                        ...
                        res = match_1_nested_func_list_set(df, keys, value, with_warnings, print_codes)
                    else:
                        res = None
        else:  # 多层嵌套
            if func is None:  # multi_1
                res = match_multi_1(df, df_len, keys, value, with_warnings, print_codes)
            else:
                if isinstance(func, str):  # single function
                    if func == 'set':  # multi_2
                        res = match_multi_2(df, keys, value, with_warnings, print_codes)
                    elif func == 'list':  # multi_3
                        res = match_multi_3(df, keys, value, with_warnings, print_codes)
                    elif func == 'count':  # multi_4
                        res = match_multi_4(df, keys, value, with_warnings, print_codes)
                    elif func in {'unique', 'nunique', 'count', 'max', 'min', 'median', 'sum', 'mean'}:
                        res = match_multi_common(df, keys, value, func, with_warnings, print_codes)
                    else:
                        res = None
                else:  # functions list
                    if func == ['len', 'set']:
                        ...
                        res = match_multi_nested_func_list_set(df, keys, value, with_warnings, print_codes)
                    else:
                        res = None
        ...
        # todo recover df, delete tmp_col
        if delete_flag:
...
def parse_format_str(strs, colon_num, fields):
    """
...
def gen_format_str(keys, value, func):
    # value
    vtype = value.__class__
    if vtype == str:
        pass
...
def match_1_1(df, df_len, keys, value, with_warnings, print_codes):
...
def match_1_2(df, keys, value, with_warnings, print_codes):
...
def match_1_3(df, keys, value, with_warnings, print_codes):
    if with_warnings:
...
def match_1_4(df, keys, value, dropna, with_warnings, print_codes):
    if value == '':
        if isinstance(keys, str):  # single column as key
            # if print_codes:
...
def match_1_common(df, keys, value, func, dropna, with_warnings, print_codes):
    keys = [keys] if isinstance(keys, str) else list(keys)
    if print_codes:
...
def match_1_nested_func_list_set(df, keys, value, with_warnings, print_codes):
    # todo too slow
    keys = [keys] if isinstance(keys, str) else list(keys)
    if with_warnings and df[value].isnull().any():
        warnings.warn("column '{}' exists NaN, please drop or fill it".format(value))
...
def match_multi_1(df, df_len, keys, value, with_warnings, print_codes):
    if with_warnings:
        keys_list = flatten_lists(keys)
    ...
    return nested_dict_set_value_from_df(df, keys, value)


def match_multi_2(df, keys, value, with_warnings, print_codes):
    if with_warnings:
    ...
    return nested_dict_gen_set_from_df(df, keys, value)


def match_multi_3(df, keys, value, with_warnings, print_codes):
    if with_warnings:
        warnings.warn("you might need set('{0}') or list(set('{0}')) instead".format(value))
        #
        if df[value].isnull().any():
    ...
    return nested_dict_gen_list_from_df(df, keys, value)


def match_multi_4(df, keys, value, with_warnings, print_codes):
    if value != '':
    ...
    return nested_dict_counter_from_df(df, keys)


def match_multi_common(df, keys, value, func, with_warnings, print_codes):
    if with_warnings:
        for item in keys:
            if isinstance(item, str) and df[item].isnull().any():
                warnings.warn("column '{}' exists NaN".format(item))
    ...
    if func == 'unique':
        if with_warnings and df[value].isnull().any():
            warnings.warn("column '{}' exists NaN, please drop or fill it".format(value))
        df = df.groupby(flatten_lists(keys))[value].unique().reset_index()
    elif func == 'nunique':
        if with_warnings and df[value].isnull().any():
            warnings.warn("column '{}' exists NaN, nunique doesn't contain number of NaNs".format(value))
        df = df.groupby(flatten_lists(keys))[value].nunique().reset_index()
    elif func == 'count':
        if with_warnings and df[value].isnull().any():
            warnings.warn("column '{}' exists NaN, count doesn't contain number of NaNs".format(value))
        df = df.groupby(flatten_lists(keys))[value].count().reset_index()
    elif func == "max":
        df = df.groupby(flatten_lists(keys))[value].max().reset_index()
    elif func == "min":
        df = df.groupby(flatten_lists(keys))[value].min().reset_index()
    elif func == "median":
        df = df.groupby(flatten_lists(keys))[value].median().reset_index()
    elif func == "sum":
        df = df.groupby(flatten_lists(keys))[value].sum().reset_index()
    elif func == "mean":
        if with_warnings and df[value].isnull().any():
            warnings.warn("column '{}' exists NaN, mean = sum/count, count doesn't contain number of NaNs".format(value))
        df = df.groupby(flatten_lists(keys))[value].mean().reset_index()

    return nested_dict_set_value_from_df(df, keys, value)


def match_multi_nested_func_list_set(df, keys, value, with_warnings, print_codes):
    # todo too slow
    return


def nested_dict_set_value(d, lists):
    k = lists[0]
    if len(lists) == 2:
        d[k] = lists[1]
    else:
        if k not in d:
            d[k] = {}
        nested_dict_set_value(d[k], lists[1:])


def nested_dict_set_value_from_df(df, keys, value):
    d = {}
    tmp = zip(*[df[item] if isinstance(item, str) else zip(*[df[k] for k in item]) for item in keys], df[value])
    for lists in tmp:
        nested_dict_set_value(d, lists)

    return d


def nested_dict_gen_set(d, lists):
    k = lists[0]
    if len(lists) == 2:
        if k not in d:
            d[k] = {lists[1]}
        ...
        if k not in d:
            d[k] = {}
        nested_dict_gen_set(d[k], lists[1:])


def nested_dict_gen_set_from_df(df, keys, value):
    d = {}
    tmp = zip(*[df[item] if isinstance(item, str) else zip(*[df[k] for k in item]) for item in keys], df[value])
    for lists in tmp:
        nested_dict_gen_set(d, lists)

    return d


def nested_dict_gen_list(d, lists):
    k = lists[0]
    if len(lists) == 2:
        if k not in d:
            d[k] = [lists[1]]
        ...
        if k not in d:
            d[k] = {}
        nested_dict_gen_list(d[k], lists[1:])


def nested_dict_gen_list_from_df(df, keys, value):
    d = {}
    tmp = zip(*[df[item] if isinstance(item, str) else zip(*[df[k] for k in item]) for item in keys], df[value])
    for lists in tmp:
        nested_dict_gen_list(d, lists)

    return d


def nested_dict_counter(d, lists):
    k = lists[0]
    if len(lists) == 1:
        if k not in d:
            d[k] = 1
        ...
        if k not in d:
            d[k] = {}
        nested_dict_counter(d[k], lists[1:])


def nested_dict_counter_from_df(df, keys):
    d = {}
    tmp = zip(*[df[item] if isinstance(item, str) else zip(*[df[k] for k in item]) for item in keys])
    for lists in tmp:
        nested_dict_counter(d, lists)
