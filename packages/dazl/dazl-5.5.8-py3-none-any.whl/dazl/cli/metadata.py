...
from ..client.config import configure_parser, get_config, merge_configurations, LedgerConfiguration
from ..client.config_yaml import parse_yaml_config_file
...
class PrintMetadataCommand(CliCommand):
    ...
    def execute(self, args) -> int:
        ...
        configs = []
        if args.config:
            configs.append(parse_yaml_config_file(args.config))
        configs.append(get_config(args))

        final_config = merge_configurations(*configs)
        return self.execute_runtime_metadata(final_config, options)
    ...
    @staticmethod
    def execute_runtime_metadata(config: LedgerConfiguration, options: PrettyOptions) -> int:
        network = Network()
