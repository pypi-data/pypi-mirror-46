...
from .. import LOG
from .config import LedgerConfiguration, LedgerPartyConfiguration
...
class _PartyClientImpl:
    def __init__(self, invoker: Invoker, party: Party):
        self.invoker = invoker
        self.party = party

        self._config_values = dict()
        self._config = None  # type: Optional[LedgerConfiguration]
    ...
    def connect_in(self, pool: LedgerNetwork, base_config: dict) -> Future:
        self._config = config = LedgerPartyConfiguration(**{**base_config, **self._config_values})
