...
class _NetworkImpl:
    ...
    def party_impl(self, party: Party, ctor: 'Callable[[_PartyClientImpl], T]') -> T:
        with self._lock:
            ...
            if impl is None:
                impl = _PartyClientImpl(self.invoker, party)
...
class _NetworkRunner:
    ...
    async def _init(self, party_impls: 'Collection[_PartyClientImpl]', first_offset: Optional[str], base_config: dict) -> Tuple[str, Future]:
        # region log
        ...
        if party_impls:
            ...
            # fetched
            party_impl = _PartyClientImpl(self._network_impl.invoker, Party('\x00'))
