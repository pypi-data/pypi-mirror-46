...
from .config import get_config, parse_kwargs, validate_config, LedgerConfiguration
...
class LedgerClientManager:
    ...
    def __init__(self, config: LedgerConfiguration, loop=None, executor=None):
        # import warnings
        ...
        for subconfig in config.nodes:
            for party in subconfig.parties:
                party_config = dict(**subconfig)
                del party_config['parties']
                self.new_client(party, **party_config)
