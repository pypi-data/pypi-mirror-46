...
class behavior_manager():
    ...
    def __init__(self, middleware = "nanomsg",  pattern = "pubsub", blackboard = "sharo"):
        ...
        if  middleware == "ROS":
            ...
            if pattern == "pubsub":
                ...
                self.sur = oldne.client("127.0.0.1", "7050")     # Robot Action Manager and Monitor
                self.sur_cond = oldne.client("127.0.0.1", "7052")  # Blackboard 
                time.sleep(1)
                success= True
    ...
    def new_action(self, primitives, robots):
        ...
        if not bool(primitives) and not bool(robots):
            ...
            # call oldnepki.nodes.action
            action = oldnepki.action(primitives, robots)
