...
class interaction():
    ...
    def new_action(self, primitives, robots, output="robot"):
        ...
        action = oldnepki.action(primitives, robots, False, output)
        return action
    ...
    def new_sequence(self, name = "main", action_list = []):
        ...
        """
        sequence = oldnepki.sequence(action_list)
        return sequence

    def new_answer(self, yes_r, no_r, other_r):
        cond1 = oldnepki.condition({"primitive":"speech","input":"yes"})
        cond2 = oldnepki.condition({"primitive":"speech","input":"no"})
        se_y = oldnepki.sequence([cond1,yes_r])
        se_n = oldnepki.sequence([cond2,no_r])
        sel = oldnepki.selector([se_y,se_n,other_r])
        return sel;
    ...
    def new_module(self, name = "main", action_list = [], node_type = "sequence"):
        ...
        if node_type == "sequence":
            ...
            selector = oldnepki.selector(action_list)
            return selector

        elif node_type == "random":
            selector = oldnepki.random_selector(action_list)
            return selector
    ...
    def new_reaction(self,name = "reaction", primitive = {}, action_list = [], reaction_source = "blackboard", memory = False):
        ...
        cond = oldnepki.condition(primitive, memory, reaction_source) 
        sequence_actions = oldnepki.sequence(action_list, memory)
        l_tree = [cond, sequence_actions] 
        sequence_reaction = oldnepki.sequence(l_tree, memory)
    ...
    def new_goal(self, name = "goal", activation = {}, cancelation = {}, time_number = 1, time_format = "seconds", action_list = []):
        ...
        """
            ...
            #cond = oldnepki.condition(activation, True,  "blackboard",) 
            #sequence_actions = oldnepki.sequence(action_list, False)
            #l_tree = [cond, sequence_actions] 
            #sequence_reaction = oldnepki.sequence(l_tree, True)
        ...
        cond = oldnepki.condition(activation, False, "blackboard") 
        sequence_actions = oldnepki.sequence(action_list, False)
        l_tree = [cond, sequence_actions] 
        sequence_reaction = oldnepki.sequence(l_tree, False)


        #reaction_start = self.new_reaction("reaction", activation, action_list, False) 
        cond_stop = oldnepki.condition(cancelation, True, "blackboard")
        cond_negado =  oldnepki.negation([cond_stop],True)
        cond_goal = oldnepki.condition(goal_primitive, False, "goal") 
        ...
        cond_stop_del = oldnepki.condition(cancelation, False, "blackboard", "any", "forgot")
        ss_ = [set_goal_time,cond_stop_del] 
        seq_del =  oldnepki.sequence(ss_, False)
        jeje = oldnepki.always_failure([seq_del],False)
        reaction_stop = oldnepki.selector([cond_negado,jeje])


        lista1 = [cond_goal,reaction_stop,sequence_reaction,set_goal_time, cond_stop_del]
        #lista2 = [sequence_reaction,set_goal_time]

        selector_actions1 = oldnepki.sequence(lista1, False)
        #selector_actions2 = oldnepki.sequence(lista2, False)

        #selector_actions3 =  oldnepki.sequence([selector_actions1,selector_actions2], False)
    ...
    def new_until_detected(self,name = "reaction", primitive = {}, while_list = [], do_list = [], reaction_source = "internal"):
        ...
        cond = oldnepki.condition(primitive, reaction_source) 
        sequence_while = oldnepki.always_failure([oldnepki.sequence(while_list)])
        sequence_do = oldnepki.sequence(do_list)
        
        l_tree = [cond, sequence_do] 
        sequence_reaction = oldnepki.sequence(l_tree)
        selector_reaction = oldnepki.until_success([oldnepki.selector([sequence_reaction,sequence_while])])
