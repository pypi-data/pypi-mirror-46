...
class blockly_files:
    ...
    """
    def __init__(self, name_files, name_toolbox, path_blockly, path_toolbox, default_blocks = "none"):
        ...
        self.file_t = open(path_toolbox + "/"+ name_toolbox + ".js","w") # Envrioment toolbox description

        self.file_d = open(path_toolbox + "/databases.js","w")
        self.file_d.write("var database = ")
        self.file_d.write("{")

        if default_blocks == "none":
            print ("not custom blocks")
            custom_blocks =  ""
        else:
            file_ct = open(default_blocks, "r") 
            custom_blocks =  file_ct.read() 
        self.toolbox = toolbox_creator(custom_blocks, name_toolbox)
