...
class launcher():
    ...
    def oldne_launch(self, node_type, input_, options_=""):
        ...
        node_name = input_
        node_type_path = node_type
        parameters = ""


        if node_type == "robot_action" or node_type == "robot":
            try:
                name = options_['robot_name'].encode("UTF-8")
                ip = options_['robot_ip'].encode("UTF-8")
                port = options_['robot_port'].encode("UTF-8")
                mid = options_['middleware'].encode("UTF-8")
                pattern = options_['pattern'].encode("UTF-8")
                parameters = " " + name + " " + ip +  " " + port + " " + mid  +" " + pattern 
                node_type_path = "robots"
            except: 
                print ("robot action node not found")
                pass

        elif node_type == "robot_sensors":
            try:
                name = options_['robot_name'].encode("UTF-8")
                ip = options_['robot_ip'].encode("UTF-8")
                port = options_['robot_port'].encode("UTF-8")
                ...
                pattern = options_['pattern'].encode("UTF-8")
                parameters = " " + name + " " + ip +  " " + port + " " + mid +" " + pattern
                node_name = node_name + "_sensors"
                node_type_path = "robots"
            except: 
                print ("robot sensor node not found")
                pass
        ...
        folder = "nodes/"+ node_type_path + "/" + node_name
