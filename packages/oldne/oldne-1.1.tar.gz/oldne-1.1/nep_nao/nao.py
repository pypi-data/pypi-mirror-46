...
class nao():
    ...
    def __init__(self,ip,port, robot="nao"):
        ...
        robot_json = oldne.read_json("C:/RobotHome/conf.json")
        robot_info = oldne.json2dict(robot_json)

        self.language = robot_info["language"]
        self._connection()
        self.robot = robot
...
##        print "TURN not jet implemente"
    ...
    def _animation_query(self,name_animation, flip = False, reverse = False): #1
        ...
        db = TinyDB('C:/RobotHome/database/nao_animation_db.json')
        q = Query()
        try:
