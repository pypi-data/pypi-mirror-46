...
async def unblock_func(func_name:object, *args, logger=None, default_res=None):
    """
    异步函数非阻塞
    :param func_name: def 函数对象名
    :param args: 请求参数可迭代对象(必须遵循元素入参顺序!)
    :param logger:
    ...
    loop = get_event_loop()
    try:
        default_res = await loop.run_in_executor(None, func_name, *args)
    except Exception as e:
        _print(msg='遇到错误:', logger=logger, log_level=2, exception=e)
    finally:
        # loop.close()
