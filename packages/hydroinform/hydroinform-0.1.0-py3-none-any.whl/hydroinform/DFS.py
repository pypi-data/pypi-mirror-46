...
#ItemInfo
    ...
    def dfsGetItemElements(ItemPointer):
        ...
        hllDll.dfsGetItemElements.restype = c_short
    ...
    def dfsSetItemInfo(HeaderPointer, ItemPointer, ItemType, Name, Eum, Data_Type):
        return hllDll['dfsSetItemInfo'](HeaderPointer, ItemPointer, ItemType.value, Name, Eum.value, Data_Type.value)
    @staticmethod
...
#GeoInfo
    ...
    def dfsSetGeoInfoUTMProj(HeaderPointer, projection, XOrigin, YOrigin, Orientation):
      return hllDll['dfsSetGeoInfoUTMProj'](HeaderPointer, projection.encode(encoding='UTF-8'), c_float(XOrigin), c_float(YOrigin), c_float(Orientation))
...
class DFSBase(object):     
    ...
    def get_time_of_timestep(self, TimeStep):
        if TimeStep<len(self.time_steps): #TimeStep have been read or created
            ...
            if self.time_axis is TimeAxisType.CalendarNonEquidistant:
                self.read_item_timestep(TimeStep,1)
    ...
    def get_timestep(self, TimeStamp):
    ...
    def read_item_timestep(self, TimeStep, Item):
    ...
    def __move_to_item_timestep(self, TimeStep, Item):
        ...
        if TimeStep != self._currentTimeStep or Item != self._currentItem:
            ...
            if TimeStep == self.number_of_time_steps:
                #Spools to last item
                DFSDLLWrapper().dfsFindItemDynamic(self._header_pointer, self._file_pointer, c_int(TimeStep - 1), c_int(self.number_of_items))  
                #Skip Item to get to end                
                DFSDLLWrapper().dfsSkipItem(self._header_pointer, self._file_pointer) 
                self._currentItem = 1
            else:
    ...
    def __increment_item_timestep(self):
...
#        self.AppendTimeStep(self.TimeSteps[self.NumberOfTimeSteps-1] + self.TimeStepSize)
      ...
      self.__increment_item_timestep()
...
class DFS0(DFSBase):
    ...
    def from_file(cls, FileName):
        cls = DFS0(FileName)
        super(DFS0, cls).from_file(FileName)
        return cls

    @classmethod
    def new_file(cls, FileName, NumberOfItems):
        cls = DFS0(FileName)
        ...
        super(DFS0, cls).new_file(FileName, NumberOfItems)
    ...
    def get_data(self, TimeStep, Item):
        return super(DFS0, self).read_item_timestep(TimeStep, Item)[0]
...
class DFS2(DFSBase):
    ...
    def from_file(cls, FileName):
        cls = DFS2(FileName)
        super(DFS2, cls).from_file(FileName)
        return cls
    ...
    def new_file(cls, FileName, NumberOfItems):
        ...
        super(DFS2, cls).new_file(FileName, NumberOfItems)
    ...
    def from_template_file(cls, FileName, TemplateFileName):
        ...
        cls = DFS2.new_file(FileName, TemplateFile.number_of_items)
    ...
    def get_data(self, TimeStep, Item):
        return np.matrix(super(DFS2,self).read_item_timestep(TimeStep, Item))
...
class DFS3(DFSBase):
    ...
    def from_file(cls, FileName):
        cls = DFS3(FileName)
        super(DFS3, cls).from_file(FileName)
        return cls
    ...
    def new_file(cls, FileName, NumberOfItems):
        cls = DFS3(FileName)
        DFSDLLWrapper().dfsHeaderCreate(FileType.EqtimeFixedspaceAllitems, NumberOfItems, StatType.NoStat, cls._header_pointer)
        super(DFS3, cls).new_file(FileName, NumberOfItems)
        cls.time_axis = TimeAxisType.CalendarEquidistant
    ...
    def get_data(self, TimeStep, Item):
        data =super(DFS3, self).read_item_timestep(TimeStep, Item)
        toreturn =[]
...
class RES11(DFSBase):
    def __init__(self, FileName):
        super(RES11, self).__init__(FileName)
...
class RES1DBase(DFSBase):
    ...
    def from_file(self, FileName):
        ...
        super(RES1DBase, self).from_file(FileName)
        fioerror = c_int( 0)
        i = 1
        self.StaticItems=[]
    ...
    def get_data(self, TimeStep, Item):
        return super(RES1DBase, self).read_item_timestep(TimeStep, Item)
