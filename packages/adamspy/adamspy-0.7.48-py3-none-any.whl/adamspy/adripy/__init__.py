...
if 'ADRILL_USER_CFG' not in os.environ:
    if os.path.isfile(os.path.join(os.path.expanduser('~'), '.adrill.cfg')):
        ...
        print('ADRILL_USER_CFG environment variable is not set!')
        
elif not os.path.exists(os.environ['ADRILL_USER_CFG']):
    print('The configuration file {} does not exist!  You must set the ADRILL_USER_CFG environment variable to an existing cfg file before using adripy.'.format(os.environ['ADRILL_USER_CFG']))

# Test that the user shared file exists
if 'ADRILL_SHARED_CFG' not in os.environ:    
    print('ADRILL_SHARED_CFG environment variable is not set!')
elif not os.path.exists(os.environ['ADRILL_SHARED_CFG']):
    print('The configuration file {} does not exist!  You must set the ADRILL_SHARED_CFG environment variable to an existing cfg file before using adripy.'.format(os.environ['ADRILL_SHARED_CFG']))
...
if 'ADAMS_LAUNCH_COMMAND' not in os.environ:
    ...
    print('The adams launch file {} does not exist!  You must set the ADRILL_SHARED_CFG environment variable to an existing cfg file before using adripy.'.format(os.environ['ADAMS_LAUNCH_COMMAND']))
