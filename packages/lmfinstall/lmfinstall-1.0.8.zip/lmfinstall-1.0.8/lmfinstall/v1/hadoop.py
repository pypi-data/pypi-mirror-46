...
def home(pin,hadoopfile):
    for conp in pin:
        ...
        c.put("E:/运维之王/file/hadoop-2.7.3.tar.gz","/root")
...
def init_config(pin,**krg):
    ...
    for conp in pin:
        c=Connection(conp[0],connect_kwargs={"password":conp[1]})
        for name in ["core-site.xml",'hdfs-site.xml','mapred-site.xml','yarn-site.xml','salves']:
...
def profile(pin):
    for conp in pin:
        c=Connection(conp[0],connect_kwargs={"password":conp[1]})
        c.run("""echo "export PATH=\$PATH:/opt/hadoop-2.7.3/bin" && source /etc/profile""",pty=True)
