...
def load_genome(fasta_f, chrom_selection=None):
    ...
    if chrom_selection is None or name in chrom_selection: #FIXME: @Nicolas 2019-05-06: This seems like an `if True: ...`; also chroms that have been asked for shouldn't be filtered out.
        genome_dict[name] = Chromosome(name, ''.join(seq))
    return genome_dict
...
class PCRDups:
    def __init__(self,
                 ...
                 pcr_mod_mu=0.45,
                 pcr_mod_sd=0.2,
                 reads_to_templates_ratio=0.5, # TODO: good default value? # FIXME: @2019-05-07 Nicolas: "temp" stands for temporary. And do we actually need a default? (Also for pcr_c; if there's no PCR the user may just pass None instead of a PCRDups object)
                 base=(2**0.1),
                 max_iter=100,
                 pol_error=4.4e-7,
        ...
        self.ratio   = reads_to_templates_ratio
        self.base    = base
...
def simulate(
    ...
    for d in [out_dir, msprime_vcf_dir, master_vcf_dir, rad_alleles_dir, rad_reads_dir]:
        os.mkdir(d)
    ...
    try:
        ...
        print('RADinitio version {}'.format(__version__))
        print('radinitio.simulate started on {}.'.format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        print('Simulating for {} chromosomes.'.format(len(chromosomes)))
        ...
        print('Merging msprime VCFs and generating variants...', flush=True)
