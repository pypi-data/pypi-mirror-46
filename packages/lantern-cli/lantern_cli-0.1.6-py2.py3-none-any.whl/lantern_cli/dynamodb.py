...
def delete_method():
    ...
    for filter in filter_options:
        ...
        type = filter['type']
        value1 = Decimal(filter['value1']) if type == 'N' and filter['value1'] else filter['value1']
        value2 = Decimal(filter['value2']) if type == 'N' and filter['value2'] else filter['value2']
        if operator == 'eq':
        ...
        with table.batch_writer() as batch:
            for row in data_bar:
                ...
                for key in primary_keys.split(','):
                    key = key.replace(' ', '')
                    pk[key] = row[key]
                pk = json_float_to_decimal(pk)
                batch.delete_item(Key=pk)
    
    click.echo(click.style("You just deleted {} rows in {}".format(data_cont, dynamodb_table)))
