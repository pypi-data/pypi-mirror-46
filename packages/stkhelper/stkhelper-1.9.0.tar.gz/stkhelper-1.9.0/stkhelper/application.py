...
#%% init Function
    ...
    def __init__(self):
        ...
        self.areaTargetList = []    # [ID, areaTarget, patterns, midCoord]
        self.satelliteList = []     # [name, satellite]
...
#%% Methods to add objects
    ...
    def AddAreaTarget(self, parsedLine):
        ...
        Parameters:
            parsedLine (str): Parsed line from the target list csv.
        ...
        midCoord = Processing.ComputeCenterTarget(parsedLine)
        ...
        self.areaTargetList.append([ID,areaTarget,patterns,midCoord])
...
#%% Access Functions
    ...
    def ComputeAccess(self, cameraArray, areaTargetArray):
        ...
        Returns:
            accessIntervals (list): List containing ID, mid-point
            of areaTarget, and the computed intervals.
            
        """
        
        self.root.BeginUpdate()
        
        access = cameraArray[1].GetAccessToObject(areaTargetArray[1])
        access.ComputeAccess()
        intervalCollection = access.ComputedAccessIntervalTimes
        try:
            computedIntervals = intervalCollection.ToArray(0,-1)
            return [areaTargetArray[0],areaTargetArray[3],computedIntervals]
    ...
    def ComputeAllAccess(self, cameraArray):
        ...
        for i in range(len(self.areaTargetList)):
            accessIntervals.append(self.ComputeAccess(cameraArray,self.areaTargetList[i]))
    ...
    def ComputeDSInfo(self, areaTargetArray, satArray, passArray):
        
        """
        
        Computes all info needed for the Dynamic Simulations.
        
        Parameters:

            areaTargetArray (list): Array containing the area target and info
            from areaTargetList for the area target that contains the pass.
            satArray (list): Array containing info for satellite being used 
            from the satelliteList.
            passArray (list): Array containing the time of the start and end
            of a pass.
            
        Returns:
            An list in the following format:
                [areaTargetName, midCoord, keplerians, (startTime,endTime)]
        ...
        areaTargetName = areaTargetArray[0]
        midCoord = areaTargetArray[3]
        
        keplerians = self.ComputeKeplerians(satArray, startTime)
        
        return [areaTargetName, midCoord, keplerians, (startTime,endTime)]
