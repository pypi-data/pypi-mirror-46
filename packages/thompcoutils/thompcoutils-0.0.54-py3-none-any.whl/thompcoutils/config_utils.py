...
class ConfigManager:
    ...
    def read_entry(self, section, entry, default_value, notes=None, value_type=None):
        ...
        if self.create:
            ...
            try:
                if value_type == str or isinstance(default_value, str):
                    value = self.config.get(section, entry)
                elif value_type == bool or isinstance(default_value, bool):
                    value = self.config.getboolean(section, entry)
                elif value_type == int or isinstance(default_value, int):
                    value = self.config.getint(section, entry)
                elif value_type == float or isinstance(default_value, float):
                    value = self.config.getfloat(section, entry)
                elif value_type == dict or isinstance(default_value, dict):
                    value = ast.literal_eval(self.config.get(section, entry))
                elif value_type == list or isinstance(default_value, list):
                    value = ast.literal_eval(self.config.get(section, entry))
                else:
...
def main():
    for write in [True, False]:
        ...
        if not write:
            ...
            print(test1)
            test2 = cfg_mgr.read_entry("User 1", "cat name", None, value_type=str)
            print(test2)
