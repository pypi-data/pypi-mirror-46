...
def get_hostname():
    p1 = subprocess.Popen(["cat", "/proc/sys/kernel/hostname"], stdout=subprocess.PIPE)
    output1 = p1.communicate()[0]
    return output1.split(".")[0]


def get_modules_status():
    p = subprocess.Popen(["netstat", "-ntulp"], stdout=subprocess.PIPE)
...
def get_data_from_unbound_stat():
    p = subprocess.Popen(["unbound-control", "stats"], stdout=subprocess.PIPE)
    output = p.communicate()[0]
