...
class BertVectorsFeaturizer(Featurizer):
    ...
    @staticmethod
    def _replace_punc(text):
        delete_ch_punc = re.sub(r"[%s]+" % punc_cn, '', text)
        delete_ch_en_punc = re.sub(r"[%s]+" % string.punctuation, '', delete_ch_punc)
        return delete_ch_en_punc

    def _get_message_text(self, message):
        all_tokens = []

        for msg in message:
            text = self._replace_punc(msg.text)
