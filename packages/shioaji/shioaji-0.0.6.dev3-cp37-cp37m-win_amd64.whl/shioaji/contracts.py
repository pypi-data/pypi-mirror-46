...
class MultiContract(BaseIterContracts):
    ...
    def __init__(self, name, contracts):
        ...
        self.__slots__ = [(cont['symbol'],
                          ...
                          self._code2contract.update({cont['code']: cont['symbol']}))[0]
                          for cont in contracts] + ['_name'] + ['_code2contract']

    def __getitem__(self, key):
        return getattr(self, key, getattr(self, self._code2contract.get(key, ''), None))

    def get(self, key, default=None):
        return getattr(self, key, getattr(self, self._code2contract.get(key, ''), default))
