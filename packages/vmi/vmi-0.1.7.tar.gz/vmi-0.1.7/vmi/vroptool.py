        ...
        if self._Bind is not None:
            ...
            def func():
                ...
                while not it.IsAtEnd():
                    if it.IsInStencil():
                        vmi.imScalar(self._Bind.data(), it.GetIndex(), target)
                    it.Next()
                    count += 1
                    p[0] = 100 * count / sum
    ...
    def apply(self):
        if self._Bind is not None:
            ...
            if spc is not None:
                ...
                resample.SetInputData(self._Bind.data())
                resample.SetInterpolationModeToCubic()
                resample.SetOutputOrigin(ori)
                resample.SetOutputExtent(ext)
                resample.SetOutputSpacing(spc)
                resample.Update()
                self._Bind.clone(resample.GetOutput())
                self.view.updateInTime()
