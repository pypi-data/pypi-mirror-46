...
class Shape(QObject, vmi.Menu):
    Types = {'compound': TopAbs_COMPOUND,
    ...
    toType = {'compound': topods.Compound,
    ...
    def expShapes(self, t=None):
        if t is None:
            shapes, exp = [], TopExp_Explorer(self._Shape)
            while exp.More():
                shapes.append(self.toType[t](exp.Current()))
                exp.Next()
            return shapes
        else:
            t = t.lower()
            if t in self.Types:
                shapes, exp = [], TopExp_Explorer(self._Shape, self.Types[t])
                while exp.More():
                    shapes.append(self.toType[t](exp.Current()))
                    exp.Next()
                return shapes
    ...
    def subShapes(self):
        shapes, it = [], TopoDS_Iterator(self._Shape)
        while it.More():
...
def mkBSplineCurveK(pts, closed=False, tol=precision.Confusion()):
    ...
    curve = mkBSplineCurve(pts, closed=closed, tol=tol)

    obb = vmi.ptsOBB(points)
    if obb['size'][2] < precision.Confusion():
