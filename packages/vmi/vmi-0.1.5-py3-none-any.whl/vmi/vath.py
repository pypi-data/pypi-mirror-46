...
class Shape(QObject, vmi.Menu):
    ...
    def __setstate__(self, s):
        ...
        self._Dataset.DeepCopy(mkPoly(self._Shape))
...
def rayCastShape(pt, vt, shape):
    ...
    vtmax = [d * _ for _ in vtNormalized(vt)]
...
def mkBSplineKochanek(pts, closed=False, tol=precision.Confusion()):
    ...
    curve = mkBSpline(pts, closed=closed, tol=tol)

    obb = vmi.ptsOBB(points)
    if 0 < obb['size'][2] < precision.Confusion():
...
def mkSolid(shells):
    ...
    if mk.IsDone() and mk.Solid().Closed():
        return mk.Solid()
...
def mkFuse(s0, s1):
    ...
    if isinstance(s1, Shape):
        s1 = s1.shape()
    if not isinstance(s0, list) or not isinstance(s0, tuple):
        s0 = [s0]
    if not isinstance(s1, list) or not isinstance(s1, tuple):
        s1 = [s1]
...
def mkPrism(shape, vt, l):
    if isinstance(shape, Shape):
        shape = shape.shape()
    vec = [l * i for i in vmi.vtNormalized(vt)]
