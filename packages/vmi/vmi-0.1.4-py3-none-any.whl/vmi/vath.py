...
def expShape(s, t):
    ...
    t = t.lower()
    if t in ShapeType:
        shapes, exp = [], TopExp_Explorer(s, ShapeType[t])
        while exp.More():
...
def exportSTL(s, file=None):
    ...
    if file is None:
        ...
        if file is None:
            return
    return stlapi.Write(s, file, False)
