...
class Configuration:
    ...
    configuration implement will overwrite application.qatech properties when option is equal.
    """
    _instance_lock = threading.Lock()
    config = {}     # 暂存所有的配置文件信息

    DEFAULT_CONFIG_FILE_NAME = 'application.qatech'     # 默认配置文件
    DEFAULT_DEPLOY_RESOURCE_DIR = './resource'  # 默认部署路径

    def __init__(self, config_path):
        """
        :param config_path: D:\qatech\workspace\integration_core_src\resource/application.qatech
        """
        if not hasattr(Configuration, "_instance"):
            if not os.path.exists(config_path):
                raise RuntimeError('Can not find about application qatech.')
            ...
            if profiles_active:
                ...
                if not os.path.exists(os.path.join(self.resources_dir, profiles_active_config_file_name)):
                    raise RuntimeError('Can not read profiles active qatech file name:' +
